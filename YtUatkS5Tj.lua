...
local Mouse = LocalPlayer:GetMouse() -- Explicit mouse resolution for the new tracking layer
...
-- Fixed logic: Make sure it ignores intersections against target's parts
...
-- [[ UPGRADED AND RE-ENGINEERED MULTI-HOOK INTERCEPTION FRAMEWORK ]]

-- LAYER 1: Dual Index hook for tools accessing Mouse.Hit / Mouse.Target directly
...
OriginalIndexHook = hookmetamethod(game, "__index", newcclosure(function(self, index)
    if not checkcaller() and Config.Enabled and CurrentTargetPart and CurrentTargetPosition ~= Vector3.new(0, 0, 0) then
        if (self == Mouse or (IsA(self, "Mouse") and self.Parent == LocalPlayer)) then
            if index == "Hit" then
...
-- LAYER 2: Re-optimized Raycast & Ray intercept logic inside Namecall
