...
getgenv().BleedSettings = getgenv().BleedSettings or {
    ...
    BlatantBaseScalerX = 0.145, -- Slightly higher starting push than Standard
    BlatantBaseScalerY = 0.145,
    BlatantDistanceFactor = 0.0008, -- Adjusts prediction automatically based on distance spikes
    BlatantVelocityCap = 75 -- Prevents prediction breaking when velocity spikes/spoofs
}
...
-- [[ RE-ENGINEERED PREDICTION LAYER ]]
...
local function calculatePrediction(targetChar, isPlayer)
    ...
    if currentTrackedEntity ~= targetChar then
        ...
        trackedVelocity = trackedVelocity:Lerp(rawVelocity, 0.25)
    end
    
    local velocity = trackedVelocity
    local speed = velocity.Magnitude
    
    -- Anti-spoof/Cap max velocity multiplier
    ...
    ---------------------------------------------------------
    -- FIXED BLATANT SPEED-DISTANCE INTERPOLATOR MODE
    ---------------------------------------------------------
    if getgenv().BleedSettings.PredictionMode == "Blatant" then
        local myChar = LocalPlayer.Character
        if myChar and myChar:FindFirstChild("HumanoidRootPart") then
            local dist = (currentPosition - myChar.HumanoidRootPart.Position).Magnitude
            
            -- Generates dynamic scale: Base Scaler + (Distance * Distance Factor)
            -- This pushes crosshairs aggressively further out the faster and farther away they run.
            finalPredictionX = getgenv().BleedSettings.BlatantBaseScalerX + (dist * getgenv().BleedSettings.BlatantDistanceFactor)
            finalPredictionY = getgenv().BleedSettings.BlatantBaseScalerY + (dist * getgenv().BleedSettings.BlatantDistanceFactor * 0.8)
        else
            finalPredictionX = getgenv().BleedSettings.BlatantBaseScalerX
            finalPredictionY = getgenv().BleedSettings.BlatantBaseScalerY
        end
    end
...
RunService.RenderStepped:Connect(function()
    ...
    -- Continuous live updating for the input fields when Blatant Mode scales up
    if getgenv().BleedSettings.PredictionMode == "Blatant" and isLocked and targetEntity and targetEntity.Character and not updatingTargetLock then
        local myChar = LocalPlayer.Character
        local targetChar = targetEntity.Character
        if myChar and myChar:FindFirstChild("HumanoidRootPart") and targetChar:FindFirstChild("HumanoidRootPart") then
            ...
            local currentX = getgenv().BleedSettings.BlatantBaseScalerX + (dist * getgenv().BleedSettings.BlatantDistanceFactor)
            local currentY = getgenv().BleedSettings.BlatantBaseScalerY + (dist * getgenv().BleedSettings.BlatantDistanceFactor * 0.8)
            XInput.Text = string.format("%.4f", currentX)
            YInput.Text = string.format("%.4f", currentY)
        end
    end
