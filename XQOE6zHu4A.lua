...
getgenv().BleedSettings = getgenv().BleedSettings or {
    ...
    WallCheck = false,          
    ResolverActive = true,
    ResolverMode = "recalculate",        -- Shifted default from Delta-Time Fallback
    ...
    PriorityTarget = "None",
    CameraMode = "Normal"                -- "Normal" (Camlock Position) or "Instant" (Predicted Position)
}
...
local basePanelHeight = 217 -- Expanded from 191 to fit new switcher layout
...
local InputsContainer = UIBuilder:Create("Frame", { Name = "InputsContainer", Size = UDim2.new(0, 171, 0, 22), BackgroundTransparency = 1, LayoutOrder = 1, Parent = PredictionPanel })
...
local AutoPredToggle = UIBuilder:Create("TextButton", { Name = "AutoPredToggle", Size = UDim2.new(0, 171, 0, 22), BackgroundColor3 = Color3.fromRGB(115, 20, 25), Text = "Auto Prediction: OFF", TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 2, Parent = PredictionPanel })
...
local CameraModeToggle = UIBuilder:Create("TextButton", { Name = "CameraModeToggle", Size = UDim2.new(0, 171, 0, 22), BackgroundColor3 = Color3.fromRGB(115, 20, 25), Text = "Camera Mode: NORMAL", TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 5, Parent = PredictionPanel })
UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 5), Parent = CameraModeToggle })

-- SORT STAGE 3: INTERACTIVE DROPDOWNS
local HitpartContainer = UIBuilder:Create("Frame", { Name = "HitpartContainer", Size = UDim2.new(0, 171, 0, 22), BackgroundTransparency = 1, ClipsDescendants = true, LayoutOrder = 6, Parent = PredictionPanel })
...
local PriorityContainer = UIBuilder:Create("Frame", { Name = "PriorityContainer", Size = UDim2.new(0, 171, 0, 22), BackgroundTransparency = 1, ClipsDescendants = true, LayoutOrder = 7, Parent = PredictionPanel })
...
-- [[ FILTERED RESOLVER DROP DOWN MODULE ]]
local resolverModesList = {"LookVector", "recalculate", "MoveDirection"}
local ResolverContainer = UIBuilder:Create("Frame", { Name = "ResolverContainer", Size = UDim2.new(0, 171, 0, 22), BackgroundTransparency = 1, ClipsDescendants = true, LayoutOrder = 8, Parent = PredictionPanel })
...
ArrowCircle.MouseButton1Click:Connect(function()
    ...
    if not uiIsExpanded then
        ...
        TweenService:Create(ResolverContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0, 171, 0, 22)}):Play()
...
-- [[ CAMERA MODE TRACKING SWITCH ]]
CameraModeToggle.MouseButton1Click:Connect(function()
    if getgenv().BleedSettings.CameraMode == "Normal" then
        getgenv().BleedSettings.CameraMode = "Instant"
        CameraModeToggle.BackgroundColor3 = Color3.fromRGB(40, 150, 45)
        CameraModeToggle.Text = "Camera Mode: INSTANT"
    else
        getgenv().BleedSettings.CameraMode = "Normal"
        CameraModeToggle.BackgroundColor3 = Color3.fromRGB(115, 20, 25)
        CameraModeToggle.Text = "Camera Mode: NORMAL"
    end
end)
    ...
    if resolverDropdownOpen then
        ...
        for i, modeName in ipairs(resolverModesList) do
            ...
            optionBtn.MouseButton1Click:Connect(function()
                ...
                local t = TweenService:Create(ResolverContainer, TweenInfo.new(0.18, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0, 171, 0, 22)})
                t:Play()
                updatePanelHeight()
                task.spawn(function()
                    t.Completed:Wait()
        ...
        TweenService:Create(ResolverContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0, 171, 0, targetSizeY)}):Play()
    else
        ResolverToggle.Text = "Resolver: " .. getgenv().BleedSettings.ResolverMode .. " ▾"
        local t = TweenService:Create(ResolverContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0, 171, 0, 22)})
        t:Play()
        task.spawn(function()
            t.Completed:Wait()
        ...
        elseif mode == "recalculate" then
            ...
            if cachedData and cachedData.LastPosition then
                ...
                if dT > 0 then
                    ...
                    velocity = calculatedVel.Magnitude > maxVel and (calculatedVel.Unit * maxVel) or calculatedVel
                end
            end
    ...
    if isLocked and targetEntity and not updatingTargetLock then
        ...
        if isStillVisible then
            ...
            if targetPosition then 
                -- [[ CAMERA INTERPOLATION TRACKER SWITCHER ]]
                if getgenv().BleedSettings.CameraMode == "Instant" then
                    Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, targetPosition)
                else
                    Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, corePart.Position)
                end
            end
        end
