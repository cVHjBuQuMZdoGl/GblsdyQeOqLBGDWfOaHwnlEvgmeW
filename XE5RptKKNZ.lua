getgenv().ExtractCC = function()
    ...
    -- [ OBSIDIAN UI LIBRARY REPOSITORY SOURCE ] --
    local repo = "https://raw.githubusercontent.com/deividcomsono/Obsidian/main/"
    local Library = loadstring(game:HttpGet(repo .. "Library.lua"))()
    local ThemeManager = loadstring(game:HttpGet(repo .. "addons/ThemeManager.lua"))()
    local SaveManager = loadstring(game:HttpGet(repo .. "addons/SaveManager.lua"))()

    -- Map Obsidian backend storage arrays directly to globals to preserve backend logic stability
    ...
    local Window = Library:CreateWindow({
        ...
        Footer = 'Extract.cc - Obsidian Conversion',
        Center = true,
        AutoShow = true,
    })
    ...
    local OrbitActive, CFrameActive, AntiLockActive, CFrameFlyActive = false, false, false, false
    ...
    -- [ TABS SETUP WITH RE-BRANDED READABLE SIDE BAR HOOKS ] --
    local Tabs = {
    ...
    -- [[ PIXEL TEXT STYLING FOR EXTERNAL EXECUTOR UI BUTTONS ]] --
    ...
    MainGroup:AddInput('FallOffset', { Default = '0.27', Text = 'Fall Offset' })
    ...
    local FlyGroup = Tabs.Rage:AddLeftGroupbox('Velocity Fly Control')
    FlyGroup:AddInput('FlySpeed', { Default = '50', Text = 'Fly Speed' })
    FlyGroup:AddButton({
        ...
        Func = function()
            ...
            local Btn = Instance.new("TextButton", ScreenGui)
            Btn.Size = UDim2.new(0, 180, 0, 50)
            Btn.Position = UDim2.new(0.5, -90, 0.80, 0)
            StylePixelText(Btn, "Fly: OFF")
            Btn.Active, Btn.Draggable = true, true
            
            Btn.MouseButton1Click:Connect(function()
                CFrameFlyActive = not CFrameFlyActive
                Btn.TextColor3 = CFrameFlyActive and Color3.fromRGB(255, 0, 0) or Color3.new(1, 1, 1)
                Btn.Text = "  Fly: " .. (CFrameFlyActive and "ON" or "OFF") .. "  "
                
                if not CFrameFlyActive then
    ...
    FlyGroup:AddToggle('NoclipToggle', { Text = 'Noclip', Default = false })
    ...
    -- [[ INSTANT BACKEND NOCLIP LOOP ]] --
    RunService.Stepped:Connect(function()
        if Toggles.NoclipToggle and Toggles.NoclipToggle.Value and LocalPlayer.Character then
    ...
    RunService.Heartbeat:Connect(function(dt)
        ...
        -- Spinbot / Anti-Aim / CFrame Mechanics
        ...
        -- Mobile Supported Velocity Fly 
        if CFrameFlyActive and root then
            ...
            local speed = tonumber(Options.FlySpeed.Value) or 50
            ...
            if hum then
                ...
                if moveDir.Magnitude > 0 then
                    ...
                    flyDir = (camLook * forwardDot) + (camRight * rightDot)
                end
            end
            
            flyGyro.CFrame = Camera.CFrame
            if flyDir.Magnitude > 0 then
                flyVelocity.Velocity = flyDir.Unit * speed
            else
                flyVelocity.Velocity = Vector3.zero
            end
        else
        ...
        -- Camera Lock / Prediction Framework
        ...
        if _tick - last_render > crosshair.refreshrate then
            ...
            if Toggles.CrosshairToggle.Value then
                ...
                for idx = 1, 4 do
                    ...
                    inline.From = position + solve(angle, crosshair.radius); inline.To = position + solve(angle, crosshair.radius + length)
                    inline.Thickness = crosshair.width
        
                    outline.Visible = true; outline.From = position + solve(angle, crosshair.radius - 1)
                    outline.To = position + solve(angle, crosshair.radius + length + 1); outline.Thickness = crosshair.width + 1.5    
                end
            else
    ...
    -- [[ SILENT AIM METATABLE INTERCEPTION ]] --
    ...
    -- [[ AUTO-SAVE & THEME GENERATOR FOR OBSIDIAN ]] --
    ...
    Library:Notify('Extract.cc | Ported successfully to Obsidian UI')
end
