...
getgenv().BleedSettings = getgenv().BleedSettings or {
    PredictionMode = "Standard", -- "Standard" or "Blatant"
    ...
    -- Blatant Physics Parameters
    BlatantProjectileSpeed = 250, 
    BlatantTimeOffset = 0.135,
    BlatantGravityComp = 0.5,

    -- New resolver and anti-ground parameters
    ResolverThreshold = 25,   -- degrees difference to trigger resolver
    GroundOffset = 2          -- studs above ground to keep aim point
}
...
-- [[ RETRACTABLE PREDICTION PANEL ENGINE ]]
local basePanelHeight = 192 
local normalPanelSize = UDim2.new(0, 190, 0, basePanelHeight)
...
local PanelLayout = UIBuilder:Create("UIListLayout", { Padding = UDim.new(0, 4), SortOrder = Enum.SortOrder.LayoutOrder, HorizontalAlignment = Enum.HorizontalAlignment.Center, VerticalAlignment = Enum.VerticalAlignment.Top, Parent = PredictionPanel })
...
-- SORT STAGE 1: PREDICTION INPUT LABELS
local InputsContainer = UIBuilder:Create("Frame", { Name = "InputsContainer", Size = UDim2.new(0.9, 0, 0, 22), BackgroundTransparency = 1, LayoutOrder = 1, Parent = PredictionPanel })
...
local XInput = UIBuilder:Create("TextBox", { Name = "XInput", Size = UDim2.new(0.48, -2, 1, 0), BackgroundColor3 = getgenv().Colors.DropdownBg, Text = tostring(getgenv().BleedSettings.PredictionX), PlaceholderText = "X Pred", TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 1, Parent = InputsContainer })
UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 4), Parent = XInput })

local YInput = UIBuilder:Create("TextBox", { Name = "YInput", Size = UDim2.new(0.48, -2, 1, 0), BackgroundColor3 = getgenv().Colors.DropdownBg, Text = tostring(getgenv().BleedSettings.PredictionY), PlaceholderText = "Y Pred", TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 2, Parent = InputsContainer })
UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 4), Parent = YInput })

-- SORT STAGE 2: PANEL TOGGLES
local AutoPredToggle = UIBuilder:Create("TextButton", { Name = "AutoPredToggle", Size = UDim2.new(0.9, 0, 0, 22), BackgroundColor3 = Color3.fromRGB(115, 20, 25), Text = "Auto Prediction: OFF", TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 2, Parent = PredictionPanel })
UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 5), Parent = AutoPredToggle })

-- MODE SWITCHER BUTTON
local ModeSwitcher = UIBuilder:Create("TextButton", { Name = "ModeSwitcher", Size = UDim2.new(0.9, 0, 0, 22), BackgroundColor3 = Color3.fromRGB(35, 35, 35), Text = "Mode: STANDARD", TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 3, Parent = PredictionPanel })
UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 5), Parent = ModeSwitcher })

local VisualizerToggle = UIBuilder:Create("TextButton", { Name = "VisualizerToggle", Size = UDim2.new(0.9, 0, 0, 22), BackgroundColor3 = getgenv().ESP.Enabled and Color3.fromRGB(40, 150, 45) or Color3.fromRGB(115, 20, 25), Text = "Visualizer ESP: " .. (getgenv().ESP.Enabled and "ON" or "OFF"), TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 4, Parent = PredictionPanel })
UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 5), Parent = VisualizerToggle })

local WallCheckToggle = UIBuilder:Create("TextButton", { Name = "WallCheckToggle", Size = UDim2.new(0.9, 0, 0, 22), BackgroundColor3 = getgenv().BleedSettings.WallCheck and Color3.fromRGB(40, 150, 45) or Color3.fromRGB(115, 20, 25), Text = "Wall Check: " .. (getgenv().BleedSettings.WallCheck and "ON" or "OFF"), TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 5, Parent = PredictionPanel })
UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 5), Parent = WallCheckToggle })

-- SORT STAGE 3: INTERACTIVE DROPDOWNS
local HitpartContainer = UIBuilder:Create("Frame", { Name = "HitpartContainer", Size = UDim2.new(0.9, 0, 0, 22), BackgroundTransparency = 1, ClipsDescendants = true, LayoutOrder = 6, Parent = PredictionPanel })
...
-- PRIORITY SYSTEM
local PriorityContainer = UIBuilder:Create("Frame", { Name = "PriorityContainer", Size = UDim2.new(0.9, 0, 0, 22), BackgroundTransparency = 1, ClipsDescendants = true, LayoutOrder = 7, Parent = PredictionPanel })
...
local function updatePanelHeight()
    ...
    local currentCalculatedHeight = basePanelHeight + (hPartHeight - 22) + (pTargetHeight - 22)
    TweenService:Create(PredictionPanel, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0, 190, 0, currentCalculatedHeight)}):Play()
end
...
ArrowCircle.MouseButton1Click:Connect(function()
    ...
    if not uiIsExpanded then
        ...
        TweenService:Create(HitpartContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, 22)}):Play()
        TweenService:Create(PriorityContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, 22)}):Play()
        ...
        local t3 = TweenService:Create(PredictionPanel, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = retractedPanelSize})
...
-- [[ FIXED MODE SWITCHER REGISTRY CONNECTIONS ]]
ModeSwitcher.MouseButton1Click:Connect(function()
    if getgenv().BleedSettings.PredictionMode == "Standard" then
        getgenv().BleedSettings.PredictionMode = "Blatant"
        ModeSwitcher.Text = "Mode: BLATANT"
        ModeSwitcher.BackgroundColor3 = getgenv().Colors.Accent
        -- UI Visual Response for Blatant calculations running dynamically
        XInput.Text = "DYNAMIC"
        YInput.Text = "DYNAMIC"
    else
        getgenv().BleedSettings.PredictionMode = "Standard"
        ModeSwitcher.Text = "Mode: STANDARD"
        ModeSwitcher.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
        XInput.Text = tostring(getgenv().BleedSettings.PredictionX)
        YInput.Text = tostring(getgenv().BleedSettings.PredictionY)
    end
end)
    ...
    if hitpartDropdownOpen then
        ...
        for i, partName in ipairs(componentsList) do
            ...
            optionBtn.MouseButton1Click:Connect(function()
                ...
                local t = TweenService:Create(HitpartContainer, TweenInfo.new(0.18, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, 22)})
                t:Play()
                updatePanelHeight()
                task.spawn(function()
                    t.Completed:Wait()
        ...
        TweenService:Create(HitpartContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, targetSizeY)}):Play()
    else
        HitpartToggle.Text = "Hitpart: " .. getgenv().BleedSettings.TargetPart .. " ▾"
        local t = TweenService:Create(HitpartContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, 22)})
        t:Play()
        task.spawn(function()
            t.Completed:Wait()
    ...
    if priorityDropdownOpen then
        ...
        for i, playerName in ipairs(pool) do
            ...
            optionBtn.MouseButton1Click:Connect(function()
                ...
                local t = TweenService:Create(PriorityContainer, TweenInfo.new(0.18, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, 22)})
        ...
        TweenService:Create(PriorityContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, targetSizeY)}):Play()
        ...
        PriorityToggle.Text = "Priority: " .. getgenv().BleedSettings.PriorityTarget .. " ▾"
        local t = TweenService:Create(PriorityContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, 22)})
...
local function assignManualPredictionHook(box, settingKey)
    box.FocusLost:Connect(function()
        if not AutoPredActive and getgenv().BleedSettings.PredictionMode == "Standard" then
...
local function generatePredictionsFromPing(pingValue)
    ...
    for pingMin = basePing, basePing + 100, 10 do
        ...
        local standardValX = (averagePing * getgenv().BleedSettings.StandardDelayCoefficient) + (math.random(-5, 5) / 10000)
        local standardValY = (averagePing * getgenv().BleedSettings.StandardDelayCoefficientY) + (math.random(-5, 5) / 10000)
        standardPredictions = standardPredictions .. string.format("ping%d_%d_X = %.4f, ping%d_%d_Y = %.4f,\n", pingMin, pingMin + 10, standardValX, pingMin, pingMin + 10, standardValY)
        
        local zeroValX = getgenv().BleedSettings.ZeroDelayValue + (math.random(-2, 2) / 10000)
        local zeroValY = getgenv().BleedSettings.ZeroDelayValueY + (math.random(-2, 2) / 10000)
        zeroDelayPredictions = zeroDelayPredictions .. string.format("ping%d_%d_X = %.4f, ping%d_%d_Y = %.4f,\n", pingMin, pingMin + 10, zeroValX, pingMin, pingMin + 10, zeroValY)
    end
    ...
    while true do
        ...
        pcall(function()
            ...
            if pingItem then
                ...
                if parsedPing then 
                    currentPing = parsedPing 
                    cachedLivePing = parsedPing / 1000
                end
            end
        end)
        ...
        -- Auto prediction only runs in Standard mode
        if AutoPredActive and getgenv().BleedSettings.PredictionMode == "Standard" then
            ...
            getgenv().BleedSettings.PredictionX = liveStandardX
            XInput.Text = string.format("%.5f", liveStandardX)
            getgenv().BleedSettings.PredictionY = liveStandardY
            YInput.Text = string.format("%.5f", liveStandardY)
        end
        task.wait(3) 
    end
end)
...
-- [[ ENHANCED PREDICTION LAYER WITH RESOLVER & ANTI-GROUND SHOT ]]
...
local function calculatePrediction(targetChar, isPlayer)
    ...
    ---------------------------------------------------------
    -- BLATANT MODE (unchanged)
    ---------------------------------------------------------
    if getgenv().BleedSettings.PredictionMode == "Blatant" then
        local myChar = LocalPlayer.Character
        if not myChar or not myChar:FindFirstChild("HumanoidRootPart") then return currentPosition end
        
        local myPosition = myChar.HumanoidRootPart.Position
        local distance = (currentPosition - myPosition).Magnitude
        
        local flightTime = (distance / getgenv().BleedSettings.BlatantProjectileSpeed) + getgenv().BleedSettings.BlatantTimeOffset
        local PredPos = currentPosition + (velocity * flightTime)
        
        if humanoid and (humanoid.FloorMaterial == Enum.Material.Air or humanoid:GetState() == Enum.HumanoidStateType.Freefall) then
            local grav = workspace.Gravity
            PredPos = PredPos - Vector3.new(0, 0.5 * grav * (flightTime ^ 2) * getgenv().BleedSettings.BlatantGravityComp, 0)
        end
        
        PredPos = PredPos + (velocity * cachedLivePing)
        return PredPos
    end

    ---------------------------------------------------------
    -- STANDARD MODE (enhanced resolver + anti‑ground shot)
    ---------------------------------------------------------
    local scaleX = getgenv().BleedSettings.PredictionX
    local scaleY = getgenv().BleedSettings.PredictionY
    
    -- ========== RESOLVER (anti‑desync / anti‑aim) ==========
    if getgenv().BleedSettings.ResolverActive and humanoid then
        -- Get current look direction (visual)
        local lookDir = corePart.CFrame.LookVector
        local horizLook = Vector3.new(lookDir.X, 0, lookDir.Z).Unit
        
        -- Get actual movement direction from velocity (horizontal)
        local moveDir = Vector3.new(velocity.X, 0, velocity.Z)
        local moveAngle = 0
        if moveDir.Magnitude > 0.1 then
            moveDir = moveDir.Unit
            moveAngle = math.acos(horizLook:Dot(moveDir)) * (180 / math.pi)
        else
            moveAngle = 0
        end
        
        -- If target is moving and angle difference exceeds threshold, override velocity direction
        if moveDir.Magnitude > 0.1 and moveAngle > getgenv().BleedSettings.ResolverThreshold then
            local speed = velocity.Magnitude
            if speed < humanoid.WalkSpeed then speed = humanoid.WalkSpeed end
            velocity = moveDir * speed
        end
        
        -- Legacy high‑speed detection (keep original behavior)
        if velocity.Magnitude > 95 then
            velocity = corePart.CFrame.LookVector * humanoid.WalkSpeed
        end
    end
    
    -- Horizontal & vertical prediction offsets
    local horizontalOffset = Vector3.new(velocity.X * scaleX, 0, velocity.Z * scaleX)
    local verticalOffset = Vector3.new(0, velocity.Y * scaleY, 0)
    
    local PredPos = currentPosition + horizontalOffset + verticalOffset
    
    -- Jump / fall offset (still honored)
    if humanoid and (humanoid.FloorMaterial == Enum.Material.Air or humanoid:GetState() == Enum.HumanoidStateType.Freefall or humanoid:GetState() == Enum.HumanoidStateType.Jumping) then
        ...
        if velocity.Y > 0 then 
            PredPos = PredPos + Vector3.new(0, jumpOffset, 0)
        else 
            PredPos = PredPos + Vector3.new(0, fallOffset, 0)
        end
    end
    
    -- ========== ANTI‑GROUND SHOT ==========
    if getgenv().BleedSettings.AntiGroundShot and humanoid then
        ...
        if isGrounded then
            -- Raycast downward from predicted position to find ground level
            local rayParams = RaycastParams.new()
            rayParams.FilterType = Enum.RaycastFilterType.Blacklist
            rayParams.FilterDescendantsInstances = {targetChar, LocalPlayer.Character, Camera}
            rayParams.IgnoreWater = true
            
            local rayOrigin = PredPos + Vector3.new(0, 10, 0)  -- start above
            local rayDirection = Vector3.new(0, -20, 0)
            local hit = workspace:Raycast(rayOrigin, rayDirection, rayParams)
            
            if hit then
                local groundY = hit.Position.Y
                local minY = groundY + getgenv().BleedSettings.GroundOffset
                if PredPos.Y < minY then
                    PredPos = Vector3.new(PredPos.X, minY, PredPos.Z)
                end
            else
                -- Fallback: clamp to root part Y - 1 (avoid going into floor)
                local rootY = corePart.Position.Y
                if PredPos.Y < rootY - 1 then
                    PredPos = Vector3.new(PredPos.X, rootY - 1, PredPos.Z)
                end
            end
        end
    end
