...
-- Button Creation Helper (Premium Theme)
...
local ESPBtn = createButton("PURE CHAMS: OFF", UDim2.new(0, 20, 0, 155), function(btn)
    ...
    if not ESPEnabled then ChamFolder:ClearAllChildren() end
end)

-- Distance Config inside GUI
...
local function createESP(player)
    ...
    local function ApplyHighlight()
        ...
        if character and not character:FindFirstChild("KhizarCham_Tag") then
    ...
    if AimbotEnabled then
        ...
        if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if ClosestPlayerEnabled then
                -- Pure Lock: Direct smooth frame interpolation camera focus
                Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, targetPos)
            else
                -- Assist Mode: Thoda soft tracking (lerp) taake automatic systems trigger na hon
