...
local int = lib:CreateInterface("99 Nights in the Forest","script made by hellocun","https://discord.gg/ATmzh2ut2s","bottom left","royal")
...
main:AddToggle("Show Safe Zone", function(enabled)
...
local storyDropdown = gametp:AddDropdown("Teleports")
...
itemtp:AddToggle("Item ESP", function(state)
	...
	local customFont = Font.new("rbxassetid://90912080224117", Enum.FontWeight.Regular, Enum.FontStyle.Normal)
...
local dropdown = itemtp:AddDropdown("Teleport to Item")
...
local bringitemtoyou = itemtp:AddDropdown("Teleport Item (Bulk):")
local sources = {
    itemsFolder,
...
local bringCharacterToYou = charactertp:AddDropdown("Teleport Mob:")
...
-- JumpPower Slider
plr:AddSlider("jumppower", 700, 50, function(value)
...
plr:AddSlider("walkspeed", 700, 16, function(value)
...
plr:AddToggle("walkspeed toggle (50)",function(toggle)
    if toggle == true then 
...
vis:AddToggle("Chams", function(state)
	ChamsEnabled = state
	if not state then
		cleanupChamsESP()
	else
...
vis:AddToggle("FOV Circle", function(state)
...
local civDropdown2 = misc:AddDropdown("Extra Scripts", function() end)
...
main:AddSlider("Kill Aura Radius", 500, 20, function(value)
...
for groupName, itemList in pairs(bracket) do
    ...
    local dropdown = itemtp:AddDropdown(label)
...
local function AddDropdownWithCheckboxes(title, itemList, enabledTable)
    local dropdown = autofarmss:AddDropdown(title)
    for _, itemName in ipairs(itemList) do
...
AddDropdownWithCheckboxes("Auto Feed Campfire (ignores HP)", campfireFuelItems, alwaysFeedEnabledItems)
AddDropdownWithCheckboxes("Auto Feed Campfire (HP Based)", campfireFuelItems, autoFuelEnabledItems)
AddDropdownWithCheckboxes("Auto Cook Food", autocookItems, autoCookEnabledItems)
AddDropdownWithCheckboxes("Auto Machine Grind", autoGrindItems, autoGrindEnabledItems)
AddDropdownWithCheckboxes("Auto Biofuel Processor", biofuelItems, autoBiofuelEnabledItems)

-- Auto Eat
local eatDropdown = autofarmss:AddDropdown("Auto Eat (3 sec interval)")
eatDropdown:AddCheckbox("Enable Auto Eat", function(checked)
    autoEatEnabled = checked
end)

-- Auto Eat HP Based
local eatHPDropdown = autofarmss:AddDropdown("Auto Eat (HP Bar Based)")
...
-- Tree toggle
local miscdropdown = autofarmss:AddDropdown("Auto Misc Features")
...
local strongholdDropdown = main:AddDropdown("Stronghold Clients")
