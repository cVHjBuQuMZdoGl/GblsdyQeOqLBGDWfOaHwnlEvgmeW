...
getgenv().BleedSettings = getgenv().BleedSettings or {
    ...
    WallCheck = false,          
    ResolverActive = true,
    ResolverMode = "Delta-Time Fallback", -- Default Advanced Mode
    MaxRealisticVelocity = 120,          -- Upper limit for validation math
    AntiGroundShot = true,
    MinHeightAboveGround = 2.5,          -- Strict ground safety buffer floor
    JumpOffset = 0,            
    FallOffset = 0,
...
local basePanelHeight = 191 -- Adjusted up from 165 to clear collapsed layout configurations
...
local resolverDropdownOpen = false
...
local VisualizerToggle = UIBuilder:Create("TextButton", { Name = "VisualizerToggle", Size = UDim2.new(0.9, 0, 0, 22), BackgroundColor3 = getgenv().ESP.Enabled and Color3.fromRGB(40, 150, 45) or Color3.fromRGB(115, 20, 25), Text = "Visualizer ESP: " .. (getgenv().ESP.Enabled and "ON" or "OFF"), TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 3, Parent = PredictionPanel })
UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 5), Parent = VisualizerToggle })

local WallCheckToggle = UIBuilder:Create("TextButton", { Name = "WallCheckToggle", Size = UDim2.new(0.9, 0, 0, 22), BackgroundColor3 = getgenv().BleedSettings.WallCheck and Color3.fromRGB(40, 150, 45) or Color3.fromRGB(115, 20, 25), Text = "Wall Check: " .. (getgenv().BleedSettings.WallCheck and "ON" or "OFF"), TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 11, LayoutOrder = 4, Parent = PredictionPanel })
UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 5), Parent = WallCheckToggle })

-- SORT STAGE 3: INTERACTIVE DROPDOWNS
local HitpartContainer = UIBuilder:Create("Frame", { Name = "HitpartContainer", Size = UDim2.new(0.9, 0, 0, 22), BackgroundTransparency = 1, ClipsDescendants = true, LayoutOrder = 5, Parent = PredictionPanel })
...
local PriorityContainer = UIBuilder:Create("Frame", { Name = "PriorityContainer", Size = UDim2.new(0.9, 0, 0, 22), BackgroundTransparency = 1, ClipsDescendants = true, LayoutOrder = 6, Parent = PredictionPanel })
...
-- [[ EXTENDED RESOLVER DROP DOWN MODULE ]]
local resolverModesList = {"Delta-Time Fallback", "LookVector Clamp", "No Vertical", "Static Zero", "Predictive Average"}
local ResolverContainer = UIBuilder:Create("Frame", { Name = "ResolverContainer", Size = UDim2.new(0.9, 0, 0, 22), BackgroundTransparency = 1, ClipsDescendants = true, LayoutOrder = 7, Parent = PredictionPanel })
local ResolverLayout = UIBuilder:Create("UIListLayout", { Padding = UDim.new(0, 2), SortOrder = Enum.SortOrder.LayoutOrder, Parent = ResolverContainer })
local ResolverToggle = UIBuilder:Create("TextButton", { Name = "ResolverToggle", Size = UDim2.new(1, 0, 0, 22), BackgroundColor3 = getgenv().Colors.DropdownBg, Text = "Resolver: " .. getgenv().BleedSettings.ResolverMode .. " ▾", TextColor3 = Color3.fromRGB(255, 255, 255), Font = Enum.Font.Code, TextSize = 10, LayoutOrder = 1, Parent = ResolverContainer })
...
-- [[ DYNAMIC MULTI-DROPDOWN EXPANSION RESOLVER ]]
local function updatePanelHeight()
    ...
    local rModeHeight = resolverDropdownOpen and (22 + (#resolverModesList * 20)) or 22
    
    local currentCalculatedHeight = basePanelHeight + (hPartHeight - 22) + (pTargetHeight - 22) + (rModeHeight - 22)
...
ArrowCircle.MouseButton1Click:Connect(function()
    ...
    if not uiIsExpanded then
        ...
        ResolverToggle.Text = "Resolver: " .. getgenv().BleedSettings.ResolverMode .. " ▾"
        ...
        TweenService:Create(ResolverContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, 22)}):Play()
...
-- [[ RESOLVER TYPE DROPDOWN CONROLLER ]]
local activeResolverButtons = {}
local function clearResolverDropdown()
    for _, btn in ipairs(activeResolverButtons) do btn:Destroy() end
    table.clear(activeResolverButtons)
end

ResolverToggle.MouseButton1Click:Connect(function()
    resolverDropdownOpen = not resolverDropdownOpen
    if resolverDropdownOpen then
        ResolverToggle.Text = "Resolver: " .. getgenv().BleedSettings.ResolverMode .. " ▴"
        clearResolverDropdown()
        for i, modeName in ipairs(resolverModesList) do
            local optionBtn = UIBuilder:Create("TextButton", { Name = "Option_" .. modeName, Size = UDim2.new(1, 0, 0, 18), BackgroundColor3 = getgenv().Colors.DropdownOption, Text = modeName, TextColor3 = Color3.fromRGB(225, 225, 225), Font = Enum.Font.Code, TextSize = 10, LayoutOrder = i + 1, Parent = ResolverContainer })
            UIBuilder:Create("UICorner", { CornerRadius = UDim.new(0, 4), Parent = optionBtn })
            
            optionBtn.MouseButton1Click:Connect(function()
                updatingTargetLock = true
                getgenv().BleedSettings.ResolverMode = modeName
                ResolverToggle.Text = "Resolver: " .. modeName .. " ▾"
                resolverDropdownOpen = false
                
                local t = TweenService:Create(ResolverContainer, TweenInfo.new(0.18, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, 22)})
                t:Play()
                updatePanelHeight()
                task.spawn(function()
                    t.Completed:Wait()
            ...
            table.insert(activeResolverButtons, optionBtn)
        end
        local targetSizeY = 22 + (#resolverModesList * 20)
        TweenService:Create(ResolverContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, targetSizeY)}):Play()
    else
        ResolverToggle.Text = "Resolver: " .. getgenv().BleedSettings.ResolverMode .. " ▾"
        local t = TweenService:Create(ResolverContainer, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2.new(0.9, 0, 0, 22)})
        t:Play()
        task.spawn(function()
            t.Completed:Wait()
            if not resolverDropdownOpen then clearResolverDropdown() end
        end)
    end
    updatePanelHeight()
end)
...
local function calculatePrediction(targetChar, isPlayer)
    ...
    local maxVel = getgenv().BleedSettings.MaxRealisticVelocity or 120
    
    -- [[ EXTENDED RESOLVER STRUCTURAL MATRIX ]]
    if getgenv().BleedSettings.ResolverActive then
        local mode = getgenv().BleedSettings.ResolverMode
        
        if mode == "Delta-Time Fallback" then
            -- Safely catch anti-aim velocity exploits or NaN breaks
            if velocity.Magnitude > maxVel or velocity.Magnitude ~= velocity.Magnitude then
                local cachedData = PositionCache[targetChar]
                local currentTime = os.clock()
                
                if cachedData and cachedData.LastPosition then
                    local dT = currentTime - cachedData.LastTime
                    if dT > 0 then
                        local calculatedVel = (currentPosition - cachedData.LastPosition) / dT
                        cachedData.LastPosition = currentPosition
                        cachedData.LastTime = currentTime
                        
                        -- If delta positioning is also tracking anomalies, enforce structural fallback bounds
                        if calculatedVel.Magnitude > maxVel then
                            velocity = corePart.CFrame.LookVector * (humanoid and humanoid.WalkSpeed or 16)
                        else
                            velocity = calculatedVel
                        end
                    end
                else
                    PositionCache[targetChar] = { LastPosition = currentPosition, LastTime = currentTime }
                    velocity = Vector3.new()
                end
            else
                -- Maintain hot-cache states during valid velocity frames
                PositionCache[targetChar] = { LastPosition = currentPosition, LastTime = os.clock() }
            end
            
        elseif mode == "LookVector Clamp" then
            if velocity.Magnitude > maxVel then
                velocity = corePart.CFrame.LookVector * (humanoid and humanoid.WalkSpeed or 16)
            end
            
        elseif mode == "No Vertical" then
            velocity = Vector3.new(velocity.X, 0, velocity.Z)
            
        elseif mode == "Static Zero" then
            if velocity.Magnitude > maxVel then
                velocity = Vector3.zero
            end
            
        elseif mode == "Predictive Average" then
            if velocity.Magnitude > maxVel then
                velocity = velocity.Unit * (humanoid and humanoid.WalkSpeed or 16)
            end
        end
        
        -- Structural Y-jitter smoothing filter
    ...
    if getgenv().BleedSettings.AntiGroundShot then
        ...
        -- Downward check raycast from 5 studs above prediction boundary down 15 studs
        ...
        if raycastResult then
            ...
            -- Clamp vector calculations out of the floor mapping if vector forces clipping
...
RunService.RenderStepped:Connect(function()
    ...
    if isLocked and targetEntity and not updatingTargetLock then
        ...
        -- [[ DYNAMIC TRACKING LOGIC COUPLING ]]
