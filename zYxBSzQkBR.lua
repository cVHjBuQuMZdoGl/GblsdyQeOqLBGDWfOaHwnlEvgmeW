-- Blazed - Fixed UI Toggle + Movement
...
UserInputService.InputBegan:Connect(function(input)
    if input.KeyCode == Enum.KeyCode.RightShift then
        ...
        Window:SetVisibility(MenuOpen)  -- This depends on your library, might need adjustment
...
local function updateESP()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        ...
        if not espNames[player] then
            ...
            espNames[player].Size = 14; espNames[player].Center = true; espNames[player].Outline = true; espNames[player].Color = ESPColor
        end
        ...
        if EspEnabled then
            ...
            local p = {Vector2.new(x-size/2,y-size), Vector2.new(x+size/2,y-size), Vector2.new(x+size/2,y+size*1.8), Vector2.new(x-size/2,y+size*1.8)}
            for i = 1, 4 do
                local nxt = i % 4 + 1
...
print("✅ Blazed Loaded - Press RIGHT SHIFT to toggle menu")