...
-- ╔══════════════════════════════════════════════════════════╗
-- ║     Axver Hub PREMIUM SCRIPT v1.2.0                          ║
-- ║     Watermark: 5847da3a                                  ║
-- ║     Token: f36e5c01c47f5f9faf22b459d1ec28ef                          ║
-- ║     Generated: 2026-05-29T06:29:25.939Z                  ║
-- ║     Protected by Cloudflare + Netlify                    ║
-- ║     Domain: loader.qzz.io                          ║
-- ║     © 2024 Axver Hub. All Rights Reserved.            ║
-- ╚══════════════════════════════════════════════════════════╝
-- ============================================
-- ANTI-LEAK PROTECTION v1.2.0
-- ============================================
if getgenv().AXVER_HUB_LOADED then return end

getgenv().AXVER_HUB_LOADED = true
getgenv().AXVER_HUB_VERSION = "1.2.0"
getgenv().AXVER_HUB_WATERMARK = "5847da3a"
getgenv().AXVER_HUB_TOKEN = "f36e5c01c47f5f9faf22b459d1ec28ef"
getgenv().AXVER_HUB_TIMESTAMP = 1780036165939

local function axverIntegrityCheck()
    if getgenv().AXVER_HUB_TOKEN ~= "f36e5c01c47f5f9faf22b459d1ec28ef" then
        local player = game:GetService("Players") and game:GetService("Players").LocalPlayer
        if player then
            player:Kick("Script integrity check failed! Contact Axver Hub.")
        end
        return false
    end
    return true
...
if not axverIntegrityCheck() then return end
...
InfoTab:AddButton({
	Title = "Tham gia nhóm của mình nhé",
	Callback = function()
		setclipboard("https://tiktok.me/group/ZSxqo1T4a/")
		Library:Notify({
			Title = "Group TikTok",
			Content = "Link group TikTok has been copied",
			Duration = 4
		})
	end
})
...
InfoTab:AddParagraph({
	Title = "Những người làm script này",
	Desc = "Fias Hub,Axver Hub"
})
InfoTab:AddParagraph({
	Title = "những người chung tay",
	Desc = "Teams"
})
InfoTab:AddParagraph({
	Title = "Functions",
	Desc = "KhanggVTX,Axver Hub"
})
InfoTab:AddParagraph({
	Title = "Cảm ơn vì tât cả!",
	Desc = ""
})

InfoTab:AddSection("Axver Hub Version: 1.2")

InfoTab:AddParagraph({
	Title = "Những lỗi fix và chức mới trong bản này:",
	Desc = "Fix: Auto farm levels\nFix: Tween và TP\nFix: Không raid được\nFix: ,...\nNew: Fast Attack\n"
})
InfoTab:AddParagraph({
	Title = "Fix/Bug",
	Desc = "Có lỗi gì nhớ liên hệ mình nhé <3"
})
InfoTab:AddParagraph({
	Title = "Follow My Account TikTok!",
	Desc = "Follow Account TikTok Của Mình Để Biết Thêm Nhiều Script Nhé!! <3"
})
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động tiêu diệt trùm Saw Sword",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động hoàn thành chuỗi nhiệm vụ Chuyên gia Kiếm - Biển 1",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động kill người chơi gần để lấy Mũ Usoap",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động tiêu diệt Greybeard cho Bisento - Biển 1",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động kill Darkbeard - nhảy server nếu không tìm thấy",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động tiêu diệt Trưởng cai ngục để lấy Warden Sword",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Đến Diablo/Urban/Deandre, kill elite boss, nhảy server nếu không thấy",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động làm nhiệm vụ Công Dân để mở Ken V2",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động kill Rip Indra - nhảy server nếu không tìm thấy",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động farm Áo Marine - nhảy server nếu không thấy Đô Đốc Vice",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động kill Don Swan để lấy Áo Thiên Nga - Biển 2, Phòng Swan",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động farm Chén Thánh - nhảy server khi không thấy boss",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Phát hiện và farm: 250 Ectoplasm (Tàu Ma Biển 2), 500 Xương (Lâu Đài Ma Biển 2), 1 Mảnh Bóng Tối (Darkbeard Biển 2). Thứ tự: Darkbeard → Ectoplasm → Xương.",
	Value = false,
	Callback = function(state)
...
MainTab:AddDropdown({
	...
	Desc = "Level = theo nhiệm vụ/cấp độ | Bone = Lâu Đài Ma | Cake Prince = Hoàng Tử Bánh | Tyrant Of The Skies = Bạo Chúa Bầu Trời | Nearest = tấn công quái gần nhất",
...
AutoLevelFarmToggle = MainTab:AddToggle({
	Title = "Start Farm",
	Desc = "Bắt đầu chế độ farm đã chọn",
...
-- ============================================
-- AUTO FARM LEVEL - TWEEN CẢI TIẾN
-- ============================================
	...
	while task.wait(0.15) do
		if _G.EclipseLevel and _G.EclipseStartFarm then
			pcall(function()
				...
				-- Xử lý Submerged Island (Level 2600+)
				local isSubQuest = LevelQuest and NameQuest and string.find(tostring(NameQuest or ""), "Submerged")
				if isSubQuest or playerLevel >= 2600 then
					local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
					if not hrp then return end
					...
					if hrp.Position.Y > -200 then
						...
						until (hrp.Position - SubWorkerCF.Position).Magnitude < 15 or timeout > 25
						...
						repeat
							...
							timeout = timeout + 0.3
							hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart")
						until (hrp and hrp.Position.Y < -200) or timeout > 20
				...
				-- Kiểm tra và abandon quest nếu sai
				...
				if questVisible and questTitle ~= "" and not string.find(questTitle, NameMon or "") then
				...
				if not plr.PlayerGui.Main.Quest.Visible then
					-- Tween đến NPC quest
					local questNPC = CFrameQuest
					local distToNPC = (questNPC.Position - plr.Character.HumanoidRootPart.Position).Magnitude
					
					if distToNPC > 10 then
						TweenPlayer(questNPC)
						task.wait(0.5)
					end
				...
				if plr.PlayerGui.Main.Quest.Visible then
					...
					for _, mob in pairs(workspace.Enemies:GetChildren()) do
						if mob.Name == Mon and mob:FindFirstChild("HumanoidRootPart") then
							local hum = mob:FindFirstChild("Humanoid")
							if hum and hum.Health > 0 then
								local dist = myHRP and (mob.HumanoidRootPart.Position - myHRP.Position).Magnitude or 0
								if dist < closestDist then
									closestDist = dist
									targetMob = mob
								end
					...
					if targetMob then
						-- Tween đến quái
						local mobPos = targetMob.HumanoidRootPart.CFrame * CFrame.new(0, _G.Settings.Setting["Farm Distance"] or 15, 0)
						local distToMob = (mobPos.Position - myHRP.Position).Magnitude
						
						if distToMob > 8 then
							TweenPlayer(mobPos)
						end
						
						-- Trang bị vũ khí và bật Haki
						EquipWeapon(_G.Settings.Main["Selected Weapon"])
						AutoHaki()
						
						-- Setup quái để kéo về
						PosMon = targetMob.HumanoidRootPart.Position
						MonFarm = targetMob.Name
						_B = true
						BringEnemy()
						
						-- Disable quái để tránh chạy lung tung
						pcall(function()
							targetMob.Humanoid.WalkSpeed = 0
							targetMob.HumanoidRootPart.CanCollide = false
							targetMob.HumanoidRootPart.Size = Vector3.new(1, 1, 1)
						end)
						
						-- Tấn công liên tục
						local startTime = tick()
						while _G.EclipseLevel and _G.EclipseStartFarm 
							and targetMob.Parent 
							and targetMob.Humanoid.Health > 0 
							and plr.PlayerGui.Main.Quest.Visible do
							
							FastAttack()
							task.wait(0.05)
							
							-- Kiểm tra timeout để tránh bug
						...
						local spawnPoint = CFrameMon
						if spawnPoint then
							TweenPlayer(spawnPoint)
						end
					end
				end
			end)
...
AutoMonFarmToggle = MainTab:AddToggle({
	Title = "Auto Farm Mob",
	Desc = "Tự động tiêu diệt quái vật khi chúng xuất hiện",
...
AutoFarmBossToggle = FarmingHopTab:AddToggle({
	...
	Desc = "Tự động tiêu diệt trùm khi xuất hiện",
...
AutoFactoryRaidToggle = OthersTab:AddToggle({
	...
	Desc = "Vào nhà máy qua cổng Dom Flamingo (Biển 2), lên đỉnh và farm quái.",
...
local function _vStage5_Raid()
	...
	if _G.VolcanicCollectEgg and _G.FullyVolcanicActive then
		Library:Notify({Title="Volcanic Kaitun", Content="Đang thu thập Trứng Rồng...", Icon="egg", Duration=4});
	...
	if _G.VolcanicCollectBone and _G.FullyVolcanicActive then
		Library:Notify({Title="Volcanic Kaitun", Content="Đang thu thập Xương Khủng Long...", Icon="box", Duration=4});
		local bt=0;
...
_volcanicMainLoop = function()
	while _G.FullyVolcanicActive do
		...
		if not hasMagnet then
			...
			if _vGetMat("Scrap Metal") < 10 then
				Library:Notify({Title="Volcanic Kaitun", Content="[1/9] Đang farm Kim Loại Phế Liệu (".. _vGetMat("Scrap Metal") .."/10)...", Icon="pickaxe", Duration=4});
				local scrapTries = 0;
			...
			if _vGetMat("Blaze Ember") < 15 then
				Library:Notify({Title="Volcanic Kaitun", Content="[2/9] Đang farm Blaze Ember (".. _vGetMat("Blaze Ember") .."/15)...", Icon="flame", Duration=4});
				local blazeTries = 0;
			...
			Library:Notify({Title="Volcanic Kaitun", Content="[3/9] Đang chế tạo Nam Châm Núi Lửa...", Icon="hammer", Duration=4});
			_vStage3_CraftMagnet();
			if not _G.FullyVolcanicActive then break; end;
			hasMagnet = _vHasItem("Volcanic Magnet");
			if not hasMagnet then task.wait(1); continue; end;
			Library:Notify({Title="Volcanic Kaitun", Content="[3/9] Đã chế tạo Nam Châm Núi Lửa thành công!", Icon="check", Duration=3});
		end;

		if not workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
			_G._volcanicPhase = "sailing";
			Library:Notify({Title="Volcanic Kaitun", Content="[4/9] Đang đến Tiền Đồn Tiki và đi thuyền ra Biển 6+...", Icon="anchor", Duration=5});
			_vStopTween();
			shouldTween = false; _G.StopTween = true;
			local boatName = _G.VolcanicSelectedBoat or "Guardian";
			local hrp = game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart");
			...
			Library:Notify({Title="Volcanic Kaitun", Content="[5/9] Đang chèo thuyền... chờ Đảo Tiền Sử xuất hiện...", Icon="ship", Duration=6});
			_vSailToIsland(700);
		end;
		if not _G.FullyVolcanicActive then break; end;

		if workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island") then
			Library:Notify({Title="Volcanic Kaitun", Content="[6/9] Đã tìm thấy Đảo Tiền Sử! Đang di chuyển đến...", Icon="map-pin", Duration=4});
			_G._volcanicPhase = "tweening";
			_vStage4_GoToIsland();
			Library:Notify({Title="Volcanic Kaitun", Content="[7/9] Đã đến đảo! Đang kích hoạt raid núi lửa...", Icon="zap", Duration=3});
		end;
		if not _G.FullyVolcanicActive then break; end;

		_G._volcanicPhase = "solving";
		Library:Notify({Title="Volcanic Kaitun", Content="[8/9] RAID ĐÃ KÍCH HOẠT! Đang bịt lỗ hổng + tiêu diệt Golem...", Icon="shield", Duration=5});
		_vStage5_Raid();
		if not _G.FullyVolcanicActive then break; end;

		Library:Notify({Title="Volcanic Kaitun", Content="[9/9] ĐÃ HOÀN THÀNH VOLCANIC! Đang chờ reset...", Icon="trophy", Duration=8});
		...
		if _G.VolcanicAutoReset then
			...
			if _G.FullyVolcanicActive and _G.VolcanicAutoReset then
				Library:Notify({Title="Volcanic Kaitun", Content="Đang reset nhân vật để bắt đầu chu kỳ mới...", Icon="refresh-cw", Duration=3});
				pcall(function()
	...
	_G._volcanicPhase = "idle";
	Library:Notify({Title="Volcanic Kaitun", Content="Auto Fully Volcanic đã tắt.", Icon="x", Duration=4});
end;
...
MultiFarmTab:AddDropdown({
	...
	Desc    = "Chọn thuyền để di chuyển đến Đảo Tiền Sử",
...
MultiFarmTab:AddToggle({
	...
	Desc  = "Chờ 10 giây và reset sau khi raid xong.",
...
MultiFarmTab:AddToggle({
	...
	Desc  = "Thu thập Trứng Rồng sau khi raid.",
...
MultiFarmTab:AddToggle({
	...
	Desc  = "Thu thập Xương Khủng Long sau khi raid.",
...
MultiFarmTab:AddButton({
	...
	Desc  = "Xóa toàn bộ dung nham trên Đảo Tiền Sử.",
	Callback = function()
		pcall(function()
			for _, v in pairs(workspace:GetDescendants()) do
...
task.spawn(function()
	...
	plr.CharacterAdded:Connect(function(char)
		...
		task.wait(4);
		if not _G.FullyVolcanicActive then return; end;
		Library:Notify({Title="Volcanic Kaitun", Content="Nhân vật đã hồi sinh - tiếp tục chu kỳ...", Icon="refresh-cw", Duration=4});
		task.spawn(_volcanicMainLoop);
	end);
end);
...
MultiFarmTab:AddParagraph({
	...
	Desc = "Vào Dungeon, đánh bại toàn bộ quái mỗi tầng (gồm Kitsune F10 và Gas F15), tự Skip Hub và lặp lại. Chỉ hoạt động ở Dungeon World (ID: 73902483975735)."
});

local _AutoFullyDungeonToggle = MultiFarmTab:AddToggle({
	Title = "Auto Fully Dungeon",
	Desc = "Cho phép/vô hiệu hóa chu trình hầm ngục tự động hoàn toàn.",
...
task.spawn(function()
	...
	if _G.Settings.Main["Auto Fully Dungeon"] then
		...
		Library:Notify({Title = "Dungeon", Content = "Auto Fully Dungeon đã được tự động kích hoạt lại!", Icon = "zap", Duration = 4});
	end;
end);
...
task.spawn(function()
	...
	while true do
		...
		if game.PlaceId ~= _DUNGEON_PID then
			Library:Notify({Title = "Auto Fully Dungeon", Content = "Không ở trong Thế Giới Dungeon! ID Bản Đồ: " .. _DUNGEON_PID, Icon = "alert-triangle", Duration = 5});
			task.wait(10);
			continue;
		end;
		pcall(function()
...
AutoFarmChestTweenToggle = OthersTab:AddToggle({
	...
	Desc = "Tween chính xác đến từng rương. Xác nhận đã nhặt qua Beli trước khi chuyển rương.",
	...
	while true do
		...
		pcall(function()
			...
			if _isSpecialChestItem() then
				...
				Library:Notify({Title = "Axver Hub", Content = "Đã tìm thấy vật phẩm đặc biệt! Tạm dừng Auto Chest.", Icon = "bell", Duration = 6});
				return;
			end;
			local chests = {};
			...
			if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
				_G.ChestHopCount = 0;
				Library:Notify({Title = "Axver Hub", Content = "Chest Hop: đã đạt giới hạn, đang chuyển server...", Icon = "bell", Duration = 4});
				task.wait(2);
				local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
				module:Teleport(game.PlaceId);
			end;
...
AutoFarmChestInstantToggle = OthersTab:AddToggle({
	...
	Desc = "Teleport và chính xác đến từng rương. Chờ xác nhận đã nhặt trước khi chuyển.",
...
task.spawn(function()
	while true do
		...
		if _G.ChestHopActive and _G.ChestHopCount >= _G.ChestHopLimit then
			_G.ChestHopCount = 0;
			Library:Notify({Title = "Axver Hub", Content = "Chest Hop: đang chuyển server...", Icon = "bell", Duration = 4});
			task.wait(2);
			local module = (loadstring(game:HttpGet("https://raw.githubusercontent.com/raw-scriptpastebin/FE/main/Server_Hop_Settings")))();
			module:Teleport(game.PlaceId);
		end;
...
OthersTab:AddToggle({
	Title = "Chest Hop",
	Desc = "Chuyển máy chủ sau khi lấy X rương (đặt số lượng bên dưới)",
	Value = false,
	Callback = function(state)
...
OthersTab:AddDropdown({
	...
	Desc = "Số rương cần lấy trước khi chuyển máy chủ",
...
OthersTab:AddParagraph({
	Title = "Rương đã thu thập",
	Desc = "0"
});
task.spawn(function()
	local para = nil;
	for _, v in pairs(Tabs.OthersTab._elements or {}) do
		if type(v) == "table" and v.Title == "Rương đã thu thập" then para = v; break; end;
	end;
	while true do
		task.wait(1);
...
AutoStopItemsToggle = OthersTab:AddToggle({
	...
	Desc = "Dừng lại khi nhận được Chén Thánh hoặc đuốc Râu Đen",
...
AutoKatakuriToggle = OthersTab:AddToggle({
	Title = "Auto Katakuri",
	Desc = "Tự động farm + Tiêu diệt Cake Prince [Chỉ dành cho Sea 3] ]",
...
local function ShowResetConfirm()
	pcall(function()
		...
		msg.Text = "Bạn có chắc chắn muốn reset tất cả cài đặt?";
...
SettingsTab:AddToggle({
	...
	Desc = "Bật theo mặc định. Dùng kỹ thuật làm mờ chuyển động, ngẫu nhiên thời gian và mô phỏng hành động người để tránh phát hiện từ Anti Cheat.",
	Value = true,
	Callback = function(state)
...
ChooseWeaponDropdown = SettingsTab:AddDropdown({
	Title = "Choose Weapon",
	Desc = "Loại vũ khí nên trang bị khi cày cuốc (Vũ khí cận chiến, Kiếm, Trái cây hoặc Súng)",
	Values = WeaponList,
...
SettingsTab:AddSlider({
	...
	Desc = "Khoảng cách tối đa (tính bằng stud) để tấn công các NPC gần đó. Mặc định trên đảo = 150",
	Min = 20,
	Max = 600,
...
ResetSettingsButton = SettingsTab:AddButton({
	...
	Desc = "Xóa tất cả cài đặt đã lưu (yêu cầu xác nhận)",
	Callback = function()
		ShowResetConfirm();
	end
});
...
SettingsTab:AddToggle({
	...
	Desc = "Bật điều khiển tốc độ di chuyển thủ công",
...
SettingsTab:AddSlider({
	Title = "Walk Speed",
	Desc = "Tốc độ di chuyển trực tiếp. Mặc định = 16. Tối đa = 300",
	Min = 16,
	Max = 300,
...
SettingsTab:AddToggle({
	...
	Desc = "Bật điều khiển độ cao nhảy tùy chỉnh",
...
SettingsTab:AddSlider({
	Title = "Jump Power %",
	Desc = "100% = nhảy thường (50). 500% = 250. Tối đa: 1000%",
	Min = 100,
	Max = 1000,
...
BringMobToggle = SettingsTab:AddToggle({
	Title = "Bring Mob",
	Desc = "Sử dụng BodyPosition để kéo quái vật về vị trí của bạn. Chỉ kích hoạt khi đang farm.",
...
SettingsTab:AddToggle({
	Title = "Fast Attack",
	Desc = "Cho phép tấn công nhanh. Tự động kích hoạt khi có trang trại quái vật. Sử dụng phương thức RegisterHit mới.",
...
SettingsTab:AddToggle({
	Title = "Fast M1 Fruit",
	Desc = "Spam click trái cây rất nhanh khi chọn Vũ Khí = Fruit và đang farm. Với Sea Beast và trái đặc biệt (Pain, T-Rex, Kitsune, Control, Dragon) chỉ dùng M1 không skill.",
...
SettingsTab:AddToggle({
	Title = "Fix Lag",
	Desc = "Giảm lag và giật hình nhưng không xóa kết cấu game",
	Value = false,
	Callback = function(state)
...
SettingsTab:AddToggle({
	Title = "FPS Boost",
	Desc = "Xóa hiệu ứng hình ảnh để tăng FPS và giảm ping",
	Value = false,
	Callback = function(state)
...
FarmingHopTab:AddToggle({
	...
	Desc = "Auto CDK: tiếp tục nhiệm vụ và đánh bại Trùm Bộ xương bị nguyền rủa. Biển 3.",
	Value = false,
	Callback = function(state)
...
FarmingHopTab:AddToggle({
	Title = "Auto Yama CDK",
	Desc = "Tự động hoàn thành tất cả các giai đoạn của nhiệm vụ Yama cho CDK. Sea 3.",
	Value = false,
	Callback = function(state)
...
AutoGetYamaToggle = FarmingHopTab:AddToggle({
	Title = "Auto Get Yama",
	Desc = "Cần 30 Elite Hunter, Chức năng chỉ ở Sea 3",
...
AutoGetYamaHopToggle = FarmingHopTab:AddToggle({
	...
	Desc = "Nếu Elite Hunter không xuất hiện thì nhảy vào đây.",
...
AutoDarkDaggerToggle = FarmingHopTab:AddToggle({
	...
	Desc = "Cần Spawn Rip Indra, chức năng chỉ ở Sea 3",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Trang bị Haki Huyền Thoại (Hồng/Trắng/Đỏ) và kích hoạt các ô trong lâu đài. Thay đổi sau mỗi 7 giây.",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Nó tự động đưa Chén Thánh đến bia mộ để triệu hồi Rip Indra.",
...
FarmingHopTab:AddToggle({
	...
	Desc = "Tự động kill Rip Indra [Sea 3]",
	...
	while task.wait(0.5) do
		pcall(function()
			...
			if _G.Settings.Raid["Auto Raid"] then
				if raidActive then
					RaidStatusParagraph:SetDesc(" Đang trong raid! Tiêu diệt kẻ địch...");
				else
					if RaidGetBP("Special Microchip") then
						RaidStatusParagraph:SetDesc(" Microchip sẵn sàng. Bắt đầu raid...");
					else
						RaidStatusParagraph:SetDesc(" Đang mua Microchip...");
					end;
				end;
			else
				RaidStatusParagraph:SetDesc(" Đã tắt");
			end;
		end);
	end;
end);
...
AutoRaidToggle = RaidTab:AddToggle({
	...
	Desc = "Bắt đầu cuộc đột kích khi con chip nằm trong túi và tiêu diệt tất cả kẻ thù trên các hòn đảo.",
...
AutoBuyChipToggle = RaidTab:AddToggle({
	Title = " Auto Buy Chip",
	Desc = "Đổi một trái cây có sẵn trong túi đồ để lấy Microchip. Không lấy trái từ kho. Chỉ mua 1 chip mỗi lần.",
	Value = false,
	Callback = function(state)
...
AutoUnstoreDevilFruitToggle = RaidTab:AddToggle({
	...
	Desc = "Lấy trái cây dưới giá đã đặt ra khỏi kho (mặc định: dưới 1.000.000). Dùng cùng với Auto Buy Chip.",
...
if not _IS_DUNGEON then
	...
	DungeonTab:AddParagraph({
		Title = "⚠ Bạn không đang ở trong Dungeon World",
		Desc = "Vui lòng truy cập Dungeon World để sử dụng các tính năng này.\nDungeon Place ID: " .. tostring(_DUNGEON_PLACE_ID)
	});
else
	...
	DungeonTab:AddToggle({
		...
		Desc = "Cho phép tự động chọn hiệu ứng tăng sức mạnh khi màn hình thẻ bài xuất hiện trong hầm ngục.",
	...
	DungeonTab:AddToggle({
		...
		Desc = "Khi kết thúc hầm ngục, hãy nhấn nút để tự động quay lại sảnh chờ.",
...
local PvPPlayerDropdown = CombatTab:AddDropdown({
	Title = "Select Player",
	Desc = "Chọn người chơi mục tiêu cho chế độ PvP.",
...
CombatTab:AddButton({
	...
	Desc = "Cập nhật danh sách người chơi trên máy chủ.",
	Callback = function()
		RefreshPvPTargets();
		pcall(function()
...
CombatTab:AddButton({
	...
	Desc = "Dịch chuyển tức thời đến vị trí của người chơi đã chọn.",
	Callback = function()
		pcall(function()
...
CombatTab:AddToggle({
	Title = "AimLock Skill",
	Desc = "Giữ camera khóa vào mục tiêu đã chọn khi sử dụng kỹ năng. Không di chuyển người chơi.",
	Value = false,
	Callback = function(v)
		_G.AimLockSkill = v;
	end
...
CombatTab:AddToggle({
	Title = "AimLock NPC",
	Desc = "Tự động khóa camera vào NPC gần nhất.",
	Value = false,
	Callback = function(v)
		_G.AimLockNPC = v;
	end
...
local function BuyFightStyleFull(styleName)
	...
	if not data then
		Library:Notify({Title = "Axver Hub", Content = "Không tìm thấy phong cách: " .. tostring(styleName), Icon = "alert-triangle", Duration = 4});
		return;
	end;
	local plr = game.Players.LocalPlayer;
	local hrp = plr.Character and plr.Character:FindFirstChild("HumanoidRootPart");
	...
	Library:Notify({Title = "Axver Hub", Content = "Đã gửi yêu cầu mua: " .. styleName, Icon = "check", Duration = 4});
end;

ShopTab:AddButton({
	Title = " Go Buy Selected Fight Style",
	Desc = "Tween đã ăn thịt NPC, tham gia đối thoại và mua kiểu trang phục đã chọn.",
	Callback = function()
...
DragonDojoTab:AddParagraph({
	...
	Desc = "Tự động hoàn thành nhiệm vụ đai (Trắng -> Đen)"
});
...
DragonDojoTab:AddToggle({
	Title = "Auto Dojo Trainer",
	Desc = "Nhận và hoàn thành nhiệm vụ đai: Trắng, Vàng, Xanh Lá, Tím, Đỏ, Đen",
	Value = false,
	Callback = function(I)
		_G_Dojoo = I;
		_G.Dojoo = I;
...
AutoFarmBlazeEmberToggle = DragonDojoTab:AddToggle({
	...
	Desc = "Hoàn thành nhiệm vụ tự động + Thu thập Blaze Ember [Chỉ dành cho Sea 3]",
...
UpgradeDracoTrialToggle = DragonDojoTab:AddToggle({
	...
	Desc = "Chuyển đổi tự động sang Dragon Wizard và nâng cấp",
...
DracoV1Toggle = DragonDojoTab:AddToggle({
	Title = "Auto Draco V1",
	Desc = "Tự động làm nhiệm vụ thứ nhất + Sự kiện thời tiền sử + Thu thập trứng rồng",
...
DracoV2Toggle = DragonDojoTab:AddToggle({
	Title = "Auto Draco V2",
	Desc = "Tự động tiêu diệt hải tặc rừng + Thu thập hoa lửa",
...
DracoV3Toggle = DragonDojoTab:AddToggle({
	Title = "Auto Draco V3",
	Desc = "Sự kiện biển tự động Giết Terror Shark",
...
RelicDracoTrialToggle = DragonDojoTab:AddToggle({
	...
	Desc = "Chế độ chơi tự động V4 - Hãy tự mình thu thập di vật trước",
...
PrehistoricFarm:AddToggle({
	...
	Desc = "Tự động chế tạo Volcanic Magnet",
	Value = false,
	Callback = function(state)
...
PrehistoricFarm:AddToggle({
	...
	Desc = "Tự động hoàn thành sự kiện Đảo Thời Tiền sử",
	Value = false,
	Callback = function(state)
...
PrehistoricFarm:AddToggle({
	Title = "Auto Collect Bone",
	Desc = "Tự động thu thập xương khủng long",
	Value = false,
	Callback = function(state)
		_G.Prehis_DB = state
		task.spawn(function()
...
PrehistoricFarm:AddToggle({
	...
	Desc = "Bỏ qua bước chế tạo trong vòng lặp Fully Volcanic.",
	Value = false,
	Callback = function(state)
		_G.CraftVM = state
		task.spawn(function()
...
ChooseBoatDropdown = SeaEventTab:AddDropdown({
	Title = "Choose Boat",
	Desc = "Chọn thuyền nên mua và sử dụng",
...
ChooseZoneDropdown = SeaEventTab:AddDropdown({
	...
	Desc = "Cấp độ nguy hiểm để đi thuyền (Chỉ Biển 3)",
...
SeaEventTab:AddToggle({
	...
	Desc = "Áp dụng tốc độ tùy chỉnh cho thuyền theo thời gian thực",
	Value = false,
	Callback = function(v)
		_G.SpeedBoat = v;
	end
...
GrindSeaToggle = SeaEventTab:AddToggle({
	Title = "Grind Sea",
	Desc = "Đến Tiki, mua thuyền, lên thuyền, đi đến vùng biển đã chọn và farm quái biển.",
	Value = false,
	Callback = function(state)
		_G.GrindSea = state;
		(getgenv()).SaveSetting();
...
SeaEventTab:AddButton({
	Title = "Bring Boat",
	Desc = "Dịch chuyển thuyền đã chọn đến vị trí hiện tại của bạn.",
	Callback = function()
		pcall(function()
			local selectedBoat = _G.Settings.SeaEvent["Selected Boat"] or "Guardian";
...
SeaStackTab:AddDropdown({
	...
	Desc = "Chọn hòn đảo muốn đến -thuyền tự lái.",
			...
			if nightTime then
				for _, v in pairs(mystic:GetDescendants()) do
					if v:IsA("MeshPart") and v.Material == Enum.Material.Neon and v.BrickColor == BrickColor.new("Bright blue") then
						...
						task.wait(0.2);
						pcall(function()
							Library:Notify({Title = "Axver Hub", Content = "Đã hoàn thành nhiệm vụ!", Icon = "bell", Duration = 6});
						end);
...
TeleportToFruitToggle = FruitTab:AddToggle({
	...
	Desc = "NGUY CƠ BỊ BAN - Dịch chuyển tức thì đến quả. Sử dụng cẩn thận.",
	...
	while true do
		...
		pcall(function()
			...
			elseif World3 then
				...
				_seaStatusParagraph:SetDesc("Biển...");
			end;
		end);
	end;
end);
...
HoldAndSkillTab:AddSection("Skills Config");

HoldAndSkillTab:AddParagraph({
	Title = "Cách thức hoạt động",
	Desc = "Hãy chọn bên dưới những phím/kỹ năng nào sẽ được sử dụng trong TẤT CẢ các trang trại và chức năng trung tâm yêu cầu kỹ năng. Các tổ hợp Z X C Cận chiến, Z X C V F Trái cây, Z X Kiếm và Z X Súng xác định các nhóm nhanh."
});

HoldAndSkillTab:AddToggle({
	Title = "Z X C Melee",
	Desc = "Chỉ dùng Z, X, C của Melee (Fighting Style)",
	Value = false,
	Callback = function(state)
		if state then
...
HoldAndSkillTab:AddToggle({
	...
	Desc = "Dùng Z, X, C, V và F của Trái Ác Quỷ",
	Value = false,
	Callback = function(state)
		if state then
			getgenv().HoldSkillConfig["Z"] = true;
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill Z",
	Desc = "Bật sử dụng phím Z khi farm",
	Value = true,
	Callback = function(state)
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill X",
	Desc = "Bật sử dụng phím X khi farm",
	Value = true,
	Callback = function(state)
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill C",
	Desc = "Bật sử dụng phím C khi farm",
	Value = true,
	Callback = function(state)
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill V",
	Desc = "Bật sử dụng phím V khi farm",
	Value = false,
	Callback = function(state)
...
HoldAndSkillTab:AddToggle({
	Title = "Use Skill F",
	Desc = "Bật sử dụng phím F khi farm",
	Value = false,
	Callback = function(state)
