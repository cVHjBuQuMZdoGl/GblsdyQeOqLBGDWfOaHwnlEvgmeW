...
-- [[ ADVANCED ENGINE NAME CALL INTERCEPTOR ]]
...
OriginalNamecallHook = hookmetamethod(game, "__namecall", newcclosure(function(...)
    ...
    if not checkcaller() and Config.Enabled and CurrentTargetPart then
        -- Network Remote Spoofing Hook (Intercepts Direct Weapon Remotes)
        if method == "FireServer" and (self.Name == "MainEvent" or self.Name == "MainRemote" or self.Name == "BulletEvent") and CurrentTargetPosition ~= Vector3.new(0, 0, 0) then
            for i = 2, #args do
                if typeof(args[i]) == "Vector3" then
                    args[i] = CurrentTargetPosition
                end
            end
            return OriginalNamecallHook(unpack(args))
        end

        -- Traditional Engine Raycast Hooks
        ...
        if (method == "ScreenPointToRay" or method == "ViewportPointToRay") and self == Camera and args[2] then
            return Ray.new(Camera.CFrame.Position, (CurrentTargetPosition - Camera.CFrame.Position).Unit * 1000)
        end
    end
...
-- UI Controls - Remaining Elements
