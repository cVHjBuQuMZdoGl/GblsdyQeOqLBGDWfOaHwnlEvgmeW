...
-- Calculates Redirected Raycast Vector with Integrated Target Prediction & Optional Spread Factors
local function calculateAimDirection(origin, targetPos, spread)
    local direction = (targetPos - origin).Unit
    if spread and spread > 0 then
        local offsetMagnitude = spread * ((targetPos - origin).Magnitude / 100)
        local randomOffset = Vector3.new(
            math.random(-offsetMagnitude, offsetMagnitude),
            math.random(-offsetMagnitude, offsetMagnitude),
            math.random(-offsetMagnitude, offsetMagnitude)
        )
        direction = ((targetPos + randomOffset) - origin).Unit
    end
    return direction
end
    ...
    if not Config.Enabled then 
        ...
        if TargetPlayer and CurrentTargetPart then
            ...
            if rootPart then
                ...
                -- [[ BULLET DESYNC SOLUTION ]]
                -- Modulates arm motor / gun framework orientation directly to the calculated trajectory path vector.
                ...
                if myChar then
                    ...
                    if myRoot and tool then
                        ...
                        if handle then
                            local lookAtTarget = CFrame.lookAt(handle.Position, CurrentTargetPosition)
                            local torso = myChar:FindFirstChild("Torso") or myChar:FindFirstChild("UpperTorso")
                            local rightShoulder = torso and (torso:FindFirstChild("Right Shoulder") or torso:FindFirstChild("RightShoulder"))
                            
                            if rightShoulder then
                                local jointOrigin = rightShoulder.Part0 and rightShoulder.Part0.CFrame * rightShoulder.C0 or myRoot.CFrame
                                rightShoulder.Transform = jointOrigin:ToObjectSpace(lookAtTarget)
                            end
                        end
                    end
                end
...
-- Metamethod Hooking Engine (Forcing Predicted Silent Aim Redirection)
...
OriginalNamecallHook = hookmetamethod(game, "__namecall", newcclosure(function(...)
    ...
    if not checkcaller() and Config.Enabled and TargetPlayer and CurrentTargetPart then
        -- Intercept standard game weapon raycasts with updated prediction/spread logic
        if method == "Raycast" and self == Workspace and args[2] and args[3] then
            local origin = args[2]
            
            if EquippedToolName == "Double Barrel" or EquippedToolName == "Tactical Shotgun" then
                args[3] = calculateAimDirection(origin, CurrentTargetPosition, 3) * args[3].Magnitude
            else
                args[3] = calculateAimDirection(origin, CurrentTargetPosition) * args[3].Magnitude
            end
        ...
        if (method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist") and args[2] then
            ...
            if EquippedToolName == "Double Barrel" or EquippedToolName == "Tactical Shotgun" then
                newDirection = calculateAimDirection(ray.Origin, CurrentTargetPosition, 3) * 9999
            else
                newDirection = calculateAimDirection(ray.Origin, CurrentTargetPosition) * 9999
            end
        ...
        if (method == "ScreenPointToRay" or method == "ViewportPointToRay") and self == Camera and args[2] then
            local calculatedDirection = calculateAimDirection(Camera.CFrame.Position, CurrentTargetPosition)
            return Ray.new(Camera.CFrame.Position, calculatedDirection)
        end
    end
