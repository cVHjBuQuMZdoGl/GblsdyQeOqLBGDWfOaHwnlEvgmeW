...
local basePanelHeight = 192 -- Extended to account for Mode Switcher button
...
-- NEW: BLATANT / STANDARD SWITCHER BUTTON
...
-- [[ OPTIMIZED SCROLLABLE PRIORITY SYSTEM DESIGN ]]
...
-- [[ MODE SWITCHER REGISTRY CONNECTIONS ]]
...
-- Storage for reading actual live ping value
...
task.spawn(function()
    while true do
        ...
        -- Continuous Auto Prediction tracking (Only acts if Mode is Standard)
...
-- [[ HYBRID BLATANT / STANDARD PREDICTION LOGIC LAYER ]]
...
local function calculatePrediction(targetChar, isPlayer)
    ...
    -- Dynamic physics filtering for cleaner tracking updates
    ...
    ---------------------------------------------------------
    -- CHOICE A: BLATANT HIGH-ACCURACY ADVANCED PHYSICS MODE
    ---------------------------------------------------------
    if getgenv().BleedSettings.PredictionMode == "Blatant" then
        ...
        -- Dynamic flight latency parsing
        ...
        -- Physical deceleration and tracking checks (Freefall / Jumping interception algorithms)
        ...
        -- Dynamic High-Precision Server Ping Multiplier Buffer
        PredPos = PredPos + (velocity * cachedLivePing)
        return PredPos
    end

    ---------------------------------------------------------
    -- CHOICE B: RE-ENGINEERED COMPATIBLE DECOUPLED STANDARD MODE
    ---------------------------------------------------------
