...
getgenv().BleedSettings = getgenv().BleedSettings or {
    ...
    -- [UPDATED Standard Predictor Architecture]
    SmoothingFactor = 0.25,          -- Exponential moving average filter weight
    BasePingWeight = 0.0135,         -- High-accuracy engine scale coefficient
    ...
    -- Blatant Hybrid Scaling Parameters
    BlatantBaseScalerX = 0.145, 
    BlatantBaseScalerY = 0.145,
    BlatantDistanceFactor = 0.0008, 
    BlatantVelocityCap = 75 
}
...
-- [[ SWITCH REGISTRY WITH DYNAMIC LIVE-READOUT UPDATE ]]
...
-- [[ LATENCY & ENGINE CALIBRATION CONTROLLER ]]
task.spawn(function()
    while true do
        local currentPing = 0.04 -- Default safe fallback (40ms in seconds)
        pcall(function()
            -- Utilizing high-fidelity engine call for precise current ping data
            currentPing = LocalPlayer:GetNetworkPing()
        end)
        
        -- Master auto prediction loop updates when targets change or mode modifies
        if AutoPredActive and not isLocked then
            if getgenv().BleedSettings.PredictionMode == "Standard" then
                -- Fallback static display calculations when no target is locked
                local rawX = currentPing * getgenv().BleedSettings.BasePingWeight * 10
                local rawY = rawX * 0.8
                
                getgenv().BleedSettings.PredictionX = rawX
                getgenv().BleedSettings.PredictionY = rawY
                
                XInput.Text = string.format("%.4f", rawX)
                YInput.Text = string.format("%.4f", rawY)
            end
        end
        task.wait(1) 
    end
...
-- [[ RE-ENGINEERED PREDICTION LAYER WITH DYNAMIC SMOOTHING ]]
...
local function calculatePrediction(targetChar, isPlayer)
    ...
    -- Exponential Moving Average Filter implementation for smooth tracking transitions
    if currentTrackedEntity ~= targetChar then
        ...
        trackedVelocity = trackedVelocity:Lerp(rawVelocity, getgenv().BleedSettings.SmoothingFactor)
    end
    
    local velocity = trackedVelocity
    local speed = velocity.Magnitude
    
    if speed > getgenv().BleedSettings.BlatantVelocityCap then
        velocity = velocity.Unit * getgenv().BleedSettings.BlatantVelocityCap
    end

    local finalPredictionX = getgenv().BleedSettings.PredictionX
    local finalPredictionY = getgenv().BleedSettings.PredictionY

    local myChar = LocalPlayer.Character
    local myHrp = myChar and myChar:FindFirstChild("HumanoidRootPart")
    local distance = myHrp and (currentPosition - myHrp.Position).Magnitude or 0
    local ping = LocalPlayer:GetNetworkPing()

    ---------------------------------------------------------
    -- AUTOMATED ADVANCED PING SIZERS (STANDARD HYBRID MODE)
    ---------------------------------------------------------
    if AutoPredActive then
        if getgenv().BleedSettings.PredictionMode == "Standard" then
            -- Base ping mapping scale
            local dynamicPing = ping * getgenv().BleedSettings.BasePingWeight
            
            -- Intelligent Distance Scaling modifiers
            if distance > 100 then
                dynamicPing = dynamicPing * (1 + (distance / 450)) -- Buff target lead dynamically over range
            elseif distance < 30 then
                dynamicPing = dynamicPing * 0.85 -- Tighten down tracking up close to avoid missing
            end
            
            finalPredictionX = dynamicPing
            finalPredictionY = dynamicPing * 0.8
            
            -- Mirror configurations back into global engine memory state safely
            getgenv().BleedSettings.PredictionX = finalPredictionX
            getgenv().BleedSettings.PredictionY = finalPredictionY
        elseif getgenv().BleedSettings.PredictionMode == "Blatant" then
            -- Blatant Distance Scaling calculations 
            finalPredictionX = getgenv().BleedSettings.BlatantBaseScalerX + (distance * getgenv().BleedSettings.BlatantDistanceFactor)
            finalPredictionY = getgenv().BleedSettings.BlatantBaseScalerY + (distance * getgenv().BleedSettings.BlatantDistanceFactor * 0.8)
        end
    end
    ...
    local horizontalOffset = Vector3.new(velocity.X * finalPredictionX, 0, velocity.Z * finalPredictionX)
    local verticalOffset = Vector3.new(0, velocity.Y * finalPredictionY, 0)
...
-- [[ MASTER RENDERING AND CALCULATION LOOP ]]
...
RunService.RenderStepped:Connect(function()
    ...
    -- Continuous real-time frame updating for the input text displays
    if AutoPredActive and isLocked and targetEntity and targetEntity.Character and not updatingTargetLock then
        XInput.Text = string.format("%.4f", getgenv().BleedSettings.PredictionX)
        YInput.Text = string.format("%.4f", getgenv().BleedSettings.PredictionY)
    end
