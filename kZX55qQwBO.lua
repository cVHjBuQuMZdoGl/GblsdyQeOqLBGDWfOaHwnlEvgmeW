...
print(" Detected: " .. detectedGame .. " | ULTRA")
...
-- MOUSE POSITION BYPASS (NO RECURSION)
local Mouse = LocalPlayer:GetMouse()
local grm = getrawmetatable(game)
setreadonly(grm, false)
local oldIndex = grm.__index

-- Cache for target position to avoid recursion
local cachedTargetX = nil
local cachedTargetY = nil
local cachedTargetCF = nil
local lastCacheUpdate = 0

-- Function to update cached target position
local function updateTargetCache()
    local now = tick()
    if now - lastCacheUpdate < 0.033 then return end -- 30fps cache update
    lastCacheUpdate = now
    
    local target = nil
    if silentLockEnabled and silentLockedTarget then
        target = silentLockedTarget
    elseif silentAim then
        target = forceTarget or (findClosest and findClosest())
    end
    ...
    if target and target.Character then
        ...
        if hrp then
            ...
            if onScreen then
                cachedTargetX = screenPos.X
                cachedTargetY = screenPos.Y
            else
                cachedTargetX = nil
                cachedTargetY = nil
            end
            
            -- Update cached CFrame for Mouse.Hit
            if getAimPos then
                local aimPos = getAimPos(target)
                if aimPos and aimPos ~= Vector3.new() then
                    cachedTargetCF = CFrame.new(aimPos)
                else
                    cachedTargetCF = nil
                end
            end
...
-- Override Mouse.X, Mouse.Y,
grm.__index = function(self, key)
    if not checkcaller() and self == Mouse then
        if key == "X" then
            updateTargetCache()
            return cachedTargetX or oldIndex(self, key)
        elseif key == "Y" then
            updateTargetCache()
            return cachedTargetY or oldIndex(self, key)
        elseif key == "Hit" then
            updateTargetCache()
            return cachedTargetCF or oldIndex(self, key)
        end
    end
...
-- Also override UserInputService:GetMouseLocation
local UIS = game:GetService("UserInputService")
local Camera = workspace.CurrentCamera

-- Method 1: 
if hookfunction then
    local original = hookfunction(UIS.GetMouseLocation, function(self)
        local target = nil
        
        if silentLockEnabled and silentLockedTarget then
            target = silentLockedTarget
        elseif silentAim then
            target = forceTarget or findClosest()
        end
    ...
    if UIS.GetMouseLocation then
        local originalGetMouseLocation = UIS.GetMouseLocation
...
findClosest = function(customRadius)
...
-- ГЛАВНАЯ ФУНКЦИЯ С ДИНАМИЧЕСКИМИ DAMPENING, PREDY, OFFSETY

getAimPos = function(plr)
    ...
    if not isAir then
        local damp = 0
        if currentPing > 95 then damp = damp + 0.012 end
        if vel.Magnitude > 33 then damp = damp + 0.02 end
        damp = math.clamp(damp, 0.09, 0.15)
        vel = vel * (1 - damp)
    end

    local predY_coef = 0
    if vel.Y > 15 then predY_coef = predY_coef + 0.002
    elseif vel.Y < -15 then predY_coef = predY_coef - 0.002 end

    local offsetY = -0.03
    if isAir then offsetY = 0.03 + (vel.Y * 0.0026) end
    pos = pos + Vector3.new(0, offsetY, 0)

    local predXZ = Vector3.new(vel.X * getgenv().PredX, 0, vel.Z * getgenv().PredX)
    local predY
    if isAir then
        predY = vel.Y * predY_coef - 0.5 * g * predY_coef * predY_coef
        if vel.Y > 0 then predY = predY + 0.24 * predY_coef end
    else
        predY = vel.Y * predY_coef
    end
...
RunService.Heartbeat:Connect(function()
    ...
    lbl.Text = "Game: "..detectedGame.." | PRED X: "..string.format("%.4f", getgenv().PredX).." | PING: "..currentPing.."ms\n"..
...
RunService.Heartbeat:Connect(function()
    ...
    if autoShoot then
        if tick() - lastAutoShoot >= getgenv().TriggerFireRate then
            local target = silentLockedTarget or lockedTarget or (silentAim and findClosest())
            if target and isVisible(target) then
                local tool = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Tool")
                if tool and not isKatana(tool) then
                    forceTarget = target
