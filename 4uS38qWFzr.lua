...
Notifier.new({
    Title = "Slik",
    Content = "Hello! " .. display_name .. " – Hope You Enjoy",
    Duration = 3,
...
TextButton.Image = "rbxassetid://112305316108408"
...
TextButton.MouseButton1Click:Connect(function()
    ...
    if not (N.Camlock.enabled or N.Targetaim.enabled) then
        ...
        locking = true
        TextButton.Image = "rbxassetid://133008043001387"
...
local function CreateWalkspeed()
    if not getgenv().WalkSpeed.Showbutton then return end
    if sextoy then sextoy:Destroy() end
    
    sextoy = Instance.new("ScreenGui")
    sextoy.Name = "NiggaNigger"
    sextoy.ResetOnSpawn = false
    sextoy.Parent = CoreGui

    nigga = Instance.new("TextButton")
    nigga.Name = "fuckoffnihha"
    nigga.Size = UDim2.new(0, 75, 0, 50)
    nigga.Position = UDim2.new(0.5, -100, 0.5, -25)
    nigga.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    nigga.BackgroundTransparency = 0.4
    nigga.Text = "WalkSpeed"
    nigga.TextColor3 = Color3.fromRGB(255, 255, 255)
    nigga.Font = Enum.Font.Code
    nigga.TextScaled = true
    nigga.TextWrapped = true
    nigga.Parent = sextoy
    ...
    local UICorner = Instance.new("UICorner")
    UICorner.CornerRadius = UDim.new(0, 10)
    UICorner.Parent = nigga

    nigga.MouseButton1Click:Connect(function()
            getgenv().WalkSpeed.Enabled = not getgenv().WalkSpeed.Enabled
            if getgenv().WalkSpeed.Enabled then
                nigga.TextColor3 = Color3.fromRGB(0, 255, 0) 
            else
                nigga.TextColor3 = Color3.fromRGB(255, 255, 255) 
            end
        end)
    ...
    nigga.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            startPos = nigga.Position
            startMousePos = input.Position
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    dragging = false
    ...
    nigga.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - startMousePos
            nigga.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)
end

local function DestroyWalkspeed()
    if sextoy then
        sextoy:Destroy()
        sextoy = nil
        nigga = nil
...
getgenv().highjump = getgenv().highjump or {
    enabled = false,
    Showbutton = false,
    power = 120
}
...
local function CreateJP()
    if not getgenv().highjump.Showbutton then return end
    if fucku then fucku:Destroy() end
    
    fucku = Instance.new("ScreenGui")
    fucku.Name = "JPGui"
    fucku.ResetOnSpawn = false
    fucku.Parent = CoreGui

    vexx = Instance.new("TextButton")
    vexx.Name = "JPNiggaChad"
    vexx.Size = UDim2.new(0, 75, 0, 50)
    vexx.Position = UDim2.new(0.5, -95, 0.5, -35)
    vexx.BackgroundColor3 = Color3.fromRGB(0, 0, 0)
    vexx.BackgroundTransparency = 0.4
    vexx.Text = "Jump Power"
    vexx.TextColor3 = Color3.fromRGB(255, 255, 255)
    vexx.Font = Enum.Font.Code
    vexx.TextScaled = true
    vexx.TextWrapped = true
    vexx.Parent = fucku
    ...
    local UICorner = Instance.new("UICorner")
    UICorner.CornerRadius = UDim.new(0, 10)
    UICorner.Parent = vexx

    vexx.MouseButton1Click:Connect(function()
            getgenv().highjump.enabled = not getgenv().highjump.enabled
            if getgenv().highjump.enabled then
                vexx.TextColor3 = Color3.fromRGB(0, 255, 0) 
            else
                vexx.TextColor3 = Color3.fromRGB(255, 255, 255) 
            end
        end)
    ...
    vexx.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            dragging = true
            startPos = vexx.Position
            startMousePos = input.Position
            input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    dragging = false
    ...
    vexx.InputChanged:Connect(function(input)
        if dragging and (input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch) then
            local delta = input.Position - startMousePos
            vexx.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y)
        end
    end)
end

local function DestroyJP()
    if fucku then
        fucku:Destroy()
        fucku = nil
        vexx = nil
...
local ScreenGui
local Frame, PFPImage, HealthFill
local CachedThumbs = {} 

local function createGUI()
    if mfs then return end

    mfs = Instance.new("ScreenGui")
    mfs.Parent = game.CoreGui
    ...
    PFPImage.Position = UDim2.new(0, 10, 0, 15)
    PFPImage.BackgroundTransparency = 1
    PFPImage.Parent = Frame

    local HealthBack = Instance.new("Frame")
    HealthBack.Size = UDim2.new(0, 180, 0, 15)
    HealthBack.AnchorPoint = Vector2.new(0, 0.5) 
    HealthBack.Position = UDim2.new(0, 110, 0.5, 0) 
    ...
    HealthFill.BackgroundColor3 = Color3.fromRGB(0, 180, 0)
    HealthFill.BorderSizePixel = 0
    HealthFill.Parent = HealthBack
    
    local Ahahah = Instance.new("UICorner")
    Ahahah.Parent = HealthFill
    Ahahah.CornerRadius = UDim.new(0, 12)
end
...
local function destroyGUI()
    if mfs then
        mfs:Destroy()
        mfs = nil
        Frame, PFPImage, HealthFill = nil
    end
end

RunService.RenderStepped:Connect(function()
    local camEnabled = getgenv().x06y and (
        (getgenv().x06y.Camlock and getgenv().x06y.Camlock.enabled) or
        (getgenv().x06y.Targetaim and getgenv().x06y.Targetaim.enabled)
    )
    local target = getgenv().x06y and (
        getgenv().x06y.Targetaim and getgenv().x06y.Targetaim.target or
        getgenv().x06y.Camlock and getgenv().x06y.Camlock.target
    )
    ...
    if getgenv().EnemyStats.Enabled and camEnabled and target then
        ...
        if player and player.UserId then
            if not CachedThumbs[player.UserId] then
                ...
                if isReady then
                    CachedThumbs[player.UserId] = thumb
                end
            end
            PFPImage.Image = CachedThumbs[player.UserId] or ""
        end
        ...
        if humanoid and humanoid.MaxHealth > 0 then
            ...
            HealthFill.BackgroundColor3 = Color3.fromRGB(0, math.floor(180 * hpPercent), 0)
            Ahahah.Parent = HealthFill
            Ahahah.CornerRadius = UDim.new(0, 12)
        end
    else
...
local function stripGameTextures()
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") then
            backups[obj] = backups[obj] or {}
            if backups[obj].Material == nil then backups[obj].Material = obj.Material end
            obj.Material = Enum.Material.Plastic

            local sa = obj:FindFirstChildOfClass("SurfaceAppearance")
            if sa then
                backups[obj].SurfaceAppearance = sa
                sa.Parent = nil
            end
        elseif obj:IsA("MeshPart") then
            ...
            if backups[obj].Material == nil then backups[obj].Material = obj.Material end
            obj.Material = Enum.Material.Plastic
            if backups[obj].TextureID == nil then backups[obj].TextureID = obj.TextureID end
            obj.TextureID = ""
        elseif obj:IsA("SpecialMesh") then
            backups[obj] = backups[obj] or {}
            if backups[obj].TextureId and backups[obj].TextureId ~= "" then
                backups[obj].TextureId = obj.TextureId
                obj.TextureId = ""
            end
        end
    end
    ...
    backups.GlobalShadows = backups.GlobalShadows or Lighting.GlobalShadows
    Lighting.GlobalShadows = false
end

local function restoreGameTextures()
    for obj, data in pairs(backups) do
        if obj and obj.Parent then
            if data.Material then obj.Material = data.Material end
            if data.TextureID then obj.TextureID = data.TextureID end
            if data.TextureId then obj.TextureId = data.TextureId end
            if data.SurfaceAppearance then data.SurfaceAppearance.Parent = obj end
        end
    end
    backups = {}
    if backups.GlobalShadows ~= nil then
        Lighting.GlobalShadows = backups.GlobalShadows
    end
end

RunService.Heartbeat:Connect(function()
    if getgenv().fpsboost.enabled and not getgenv().__fpsboost_active then
        getgenv().__fpsboost_active = true
        stripGameTextures()
    elseif not getgenv().fpsboost.enabled and getgenv().__fpsboost_active then
        getgenv().__fpsboost_active = false
        restoreGameTextures()
    end
end)
...
local function createCircle()
    ...
    for i = 1, getgenv().circlez.Segments do
        local line = Drawing.new("Line")
        line.Visible = false
        line.Color = getgenv().circlez.Color
        line.Thickness = getgenv().circlez.Thickness
        line.Transparency = 1
        drawings[i] = line
    end
end
...
RunService.RenderStepped:Connect(function()
    if not getgenv().circlez.Enabled then
        for _, v in ipairs(drawings) do
            v.Visible = false
        end
        return
    ...
    local radius = getgenv().circlez.Radius
    local segments = getgenv().circlez.Segments
    ...
    for i = 1, segments do
        ...
        local point1 = pos + Vector3.new(math.cos(angle1) * radius, 0, math.sin(angle1) * radius)
        local point2 = pos + Vector3.new(math.cos(angle2) * radius, 0, math.sin(angle2) * radius)

        local p1, onscreen1 = cam:WorldToViewportPoint(point1)
        local p2, onscreen2 = cam:WorldToViewportPoint(point2)
...
getgenv().selfHighlight = {
    Enabled = false,
    FillColor = Color3.fromRGB(255, 255, 255),
    FillTransparency = 0.5,
    OutlineColor = Color3.fromRGB(170,170,170),
    OutlineTransparency = 0.5,
    DepthMode = Enum.HighlightDepthMode.AlwaysOnTop -- or Occluded
}
...
highlight.Adornee = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
highlight.FillColor = getgenv().selfHighlight.FillColor
highlight.FillTransparency = 0.5
highlight.OutlineColor = getgenv().selfHighlight.OutlineColor
highlight.OutlineTransparency = 0.5
highlight.DepthMode = getgenv().selfHighlight.DepthMode
...
task.spawn(function()
    while task.wait(0.1) do
        if getgenv().selfHighlight.Enabled then
...
getgenv().HRPTrail = getgenv().HRPTrail or {
    Enabled = false,
    Color = Color3.fromRGB(255, 255, 255), 
    Lifetime = 2,
    Width = 0.35,
...
if hrp:FindFirstChild("HRPVerticalTrail") then
    hrp.HRPVerticalTrail:Destroy()
end

local folder = Instance.new("Folder")
folder.Name = "HRPVerticalTrail"
folder.Parent = hrp

local top = Instance.new("Attachment")
top.Position = Vector3.new(0, getgenv().HRPTrail.Height/2, 0)
top.Parent = hrp

local bottom = Instance.new("Attachment")
bottom.Position = Vector3.new(0, -getgenv().HRPTrail.Height/2, 0)
bottom.Parent = hrp

local trail = Instance.new("Trail")
trail.Name = "VerticalTrail"
trail.Attachment0 = top
trail.Attachment1 = bottom
trail.Lifetime = getgenv().HRPTrail.Lifetime
trail.Color = ColorSequence.new(getgenv().HRPTrail.Color)
trail.WidthScale = NumberSequence.new(getgenv().HRPTrail.Width)
trail.Enabled = getgenv().HRPTrail.Enabled
trail.MinLength = 0.1
trail.Parent = folder

RunService.Heartbeat:Connect(function()
    if trail then
        local cfg = getgenv().HRPTrail

        trail.Color = ColorSequence.new(cfg.Color)
        trail.WidthScale = NumberSequence.new(cfg.Width)

        if cfg.Enabled and not trail.Enabled then
            trail.Lifetime = cfg.Lifetime
            trail.Enabled = true
        elseif not cfg.Enabled and trail.Enabled then
            local tween = TweenService:Create(trail, TweenInfo.new(0.6), {Lifetime = 0})
            tween:Play()
...
local ws = cah:DrawSection({Name = "Walkspeed", Position = "left"})
local jp = cah:DrawSection({Name = "Jump Power", Position = "left"})
local game = cah:DrawSection({Name = "Game Setting", Position = "right"})
local chm = cah:DrawSection({Name = "Self Esp", Position = "right"})
...
chamz:AddDropdown({
    ...
    Multi = false,
    Flag = "choose_hitchams_material",
    Callback = function(a)
        if Enum.Material[a] then
            getgenv().HitChams.material = Enum.Material[a]
        end
    end,
});
csync:AddToggle({
...
ws:AddToggle({
    ...
    Flag = "toggle_ws",
    Callback = function(a)
        getgenv().WalkSpeed.Enabled = a
    end,
});
ws:AddToggle({
    Name = "Show The Button",
    Default = getgenv().WalkSpeed.Showbutton,
    Flag = "ws_showbutton",
    Callback = function(a)
        getgenv().WalkSpeed.Showbutton = a
        if a then
            CreateWalkspeed()
        else
           DestroyWalkspeed()
...
ws:AddTextBox({
    Name = "Value",
    Default = getgenv().WalkSpeed.Value,
    Placeholder = "Insert Value..",
    Callback = function(a)
        getgenv().WalkSpeed.Value = a
    end,
});
jp:AddToggle({
    Name = "Toggle JP",
    ...
    Flag = "toggle_jp",
    Callback = function(a)
        getgenv().highjump.enabled = a
    end,
});
jp:AddToggle({
    Name = "Show The Button",
    Default = getgenv().highjump.Showbutton,
    Flag = "jp_showbutton",
    Callback = function(a)
        getgenv().highjump.Showbutton = a
        if a then
            CreateJP()
        else
            DestroyJP()
...
game:AddToggle({
    Name = "Fpsboost",
    Default = getgenv().fpsboost.enabled,
    Flag = "toggle_fpsboost",
    Callback = function(a)
        getgenv().fpsboost.enabled = a
    end,
});
local chars = chm:AddToggle({
    Name = "Character Chams",
    Default = getgenv().Chams.Character.Enabled,
    Flag = "toggle_char_chams",
    Callback = function(a)
        getgenv().Chams.Character.Enabled = a
    end,
});
chars.Link:AddColorPicker({
    Default = getgenv().Chams.Character.Color,
    Callback = function(a)
        getgenv().Chams.Character.Color = a
    end,
});
local toolzs = chm:AddToggle({
    Name = "Tool Chams",
    Default = getgenv().Chams.Tool.Enabled,
    Flag = "toggle_tool_cham",
    Callback = function(a)
        getgenv().Chams.Tool.Enabled = a
    end,
});
toolzs.Link:AddColorPicker({
    Default = getgenv().Chams.Tool.Color,
    Callback = function(s)
        getgenv().Chams.Tool.Color = s
    end,
});
local circir = chm:AddToggle({
    Name = "3D Circle",
    Default = getgenv().circlez.Enabled,
    Flag = "toggle_3dcircle",
    Callback = function(a)
        getgenv().circlez.Enabled = a
    end,
});
circir.Link:AddColorPicker({
    Default = getgenv().circlez.Color,
    Callback = function(a)
        getgenv().circlez.Color = a
    end,
});
local selfhi = chm:AddToggle({
    Name = "Self Highlight",
    Default = getgenv().selfHighlight.Enabled,
    Flag = "toggle_self_highlight",
    Callback = function(a)
        getgenv().selfHighlight.Enabled = a
    end,
});
selfhi.Link:AddColorPicker({
    Default = getgenv().selfHighlight.FillColor,
    Callback = function(a)
        getgenv().selfHighlight.FillColor = a
    end,
});
selfhi.Link:AddColorPicker({
    Default = getgenv().selfHighlight.OutlineColor,
    Callback = function(a)
        getgenv().selfHighlight.OutlineColor = a
    end,
});
local trails = chm:AddToggle({
    Name = "Character Trail",
    Default = getgenv().HRPTrail.Enabled,
    Flag = "toggle_trail",
    Callback = function(a)
        getgenv().HRPTrail.Enabled = a
    end,
});
trails.Link:AddColorPicker({
    Default = getgenv().HRPTrail.Color,
    Callback = function(a)
        getgenv().HRPTrail.Color = a
     end,
});
