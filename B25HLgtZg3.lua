...
-- Re-engineered Rain and Blood Layer Container
...
-- [[ HIGH-VELOCITY RAIN & SLOW-DRIPPING BLOOD SIMULATOR ]]
task.spawn(function()
    while task.wait(0.04) do
        if not ButtonFrame or not ButtonFrame.Parent then break end
        
        -- 1. FAST CYBERPUNK ATMOSPHERIC RAIN (Spawns continuously in background)
        if math.random(1, 2) == 1 then
            local rainDrop = Instance.new("Frame")
            rainDrop.Name = "AtmosphericRain"
            rainDrop.BorderSizePixel = 0
            rainDrop.BackgroundColor3 = Color3.fromRGB(240, 245, 255)
            rainDrop.BackgroundTransparency = math.random(75, 90) / 100
            rainDrop.ZIndex = 1
            rainDrop.Parent = RainContainer
            
            -- High velocity parameters (Angle downwards-left slightly)
            rainDrop.Size = UDim2.new(0, 1, 0, math.random(14, 24))
            rainDrop.Rotation = 12
            
            local startX = math.random(-20, 110) / 100
            rainDrop.Position = UDim2.new(startX, 0, -0.3, 0)
            
            local fallSpeed = math.random(25, 45) / 100 -- Extremely fast sweep
            local rainTween = TweenService:Create(rainDrop, TweenInfo.new(fallSpeed, Enum.EasingStyle.Linear), {
                Position = UDim2.new(startX - 0.15, 0, 1.3, 0)
            })
            rainTween:Play()
            Debris:AddItem(rainDrop, fallSpeed)
        end
        
        -- 2. DETAILED SLOW DRIPPING & POOLING BLOOD (Spawns dynamically)
        if math.random(1, 6) == 1 then
            local bloodDrip = Instance.new("Frame")
            bloodDrip.Name = "ViscousBloodDrip"
            bloodDrip.BorderSizePixel = 0
            bloodDrip.BackgroundColor3 = Color3.fromRGB(math.random(130, 165), 0, math.random(5, 15)) -- Crimson Variance
            bloodDrip.ZIndex = 2
            bloodDrip.Parent = RainContainer
            
            local initialX = math.random(2, 98) / 100
            local dripWidth = math.random(2, 3)
            local initialHeight = math.random(4, 8)
            
            bloodDrip.Size = UDim2.new(0, dripWidth, 0, initialHeight)
            bloodDrip.Position = UDim2.new(initialX, 0, -0.1, 0)
            
            -- Slow visceral slide downwards
            local dripDuration = math.random(14, 26) / 10
            
            -- Step 1: Slow journey down to the floor edge
            local fallTween = TweenService:Create(bloodDrip, TweenInfo.new(dripDuration, Enum.EasingStyle.Cubic, Enum.EasingDirection.In), {
                Position = UDim2.new(initialX, 0, 1.0, -initialHeight)
            })
            fallTween:Play()
            
            -- Step 2: Pool and flatten out upon hitting the frame boundary
            task.delay(dripDuration, function()
                if bloodDrip and bloodDrip.Parent then
                    local poolTween = TweenService:Create(bloodDrip, TweenInfo.new(0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
                        Size = UDim2.new(0, dripWidth + math.random(3, 5), 0, math.random(1, 2)),
                        Position = UDim2.new(initialX - 0.01, 0, 1.0, -2),
                        BackgroundTransparency = 1
                    })
                    poolTween:Play()
                    Debris:AddItem(bloodDrip, 0.6)
                end
            end)
        end
    end
