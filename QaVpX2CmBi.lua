...
local success, err = pcall(function()
    ...
    gamerawmetatable.__namecall = newcclosure(function(self, ...)
        ...
        local blockedRemotes = {
            ...
            ["BANREMOTE"] = true,
            ["PERMAIDBAN"] = true,
            ["KIvoidhitCKREMOTE"] = true,
            ["BR_KICKPC"] = true,
            ["BR_KICKMOBILE"] = true
        }

        if blockedRemotes[revoidhitmoteName] then
            return
        end
...
warn("[+] void.hit : anticheat got mogged.")
end)()
...
if isOwner then
    Library:Notify("Owner  User! Welcome " .. plr.DisplayName .. " (@" .. plr.Name .. ")", 15)
end
if hasBypass then
...
tracerOutline.Color = voidhit.TargetAim.TracerOutlineColor
tracerOutline.Thickness = 4

local tracer = Drawing.new("Line")
tracer.Visible = false
tracer.Color = voidhit.TargetAim.TracerFillColor
tracer.Thickness = 2
...
local HitChams = LPH_NO_VIRTUALIZE(function(Player)
    if not voidhit.HitEffects.HitChams.Enabled then return end
    if Player and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        ...
        for _, BodyPart in ipairs(Cloned:GetChildren()) do
            if BodyPart:IsA("BasePart") then
                ...
                BodyPart.Transparency = voidhit.HitEffects.HitChams.Transparency
                BodyPart.Color = voidhit.HitEffects.HitChams.Color
                BodyPart.Material = voidhit.HitEffects.HitChams.Material
            end
        end
        if Cloned:FindFirstChild("Head") then
            local Head = Cloned.Head
            Head.Transparency = voidhit.HitEffects.HitChams.Transparency
            Head.Color = voidhit.HitEffects.HitChams.Color
            Head.Material = voidhit.HitEffects.HitChams.Material
        ...
        local tweenInfo = TweenInfo.new(
            voidhit.HitEffects.HitChams.Lifetime,
        ...
        task.delay(voidhit.HitEffects.HitChams.Lifetime, function()
...
local HitChamsSkeleton = LPH_NO_VIRTUALIZE(function(Player)
    if not voidhit.HitEffects.HitSkeleton.Enabled then return end  -- Thêm HitSkeleton vào voidhit.HitEffects
    if Player and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        local bones = {
            {"Head", "UpperTorso"},
        ...
        for _, bonePair in ipairs(bones) do
            ...
            if parentBone and childBone then
                ...
                line.Transparency = voidhit.HitEffects.HitChams.Transparency  -- Sử dụng chung transparency
                line.Color = voidhit.HitEffects.HitSkeleton.Color  -- Sử dụng Color mới
                line.Material = Enum.Material.Neon
                line.Parent = workspace
                local tweenInfo = TweenInfo.new(voidhit.HitEffects.HitChams.Lifetime / 0.2, Enum.EasingStyle.Sine, Enum.EasingDirection.Out)
...
local function GetClosestCharacter()
    ...
    for _, player in pairs(players:GetPlayers()) do
        ...
        if not isAlive(player) and voidhit.Checks.Alive then continue end
        if voidhit.Checks.Team and player.Team == localPlayer.Team then continue end
        if voidhit.Checks.Forcefield and player.Character:FindFirstChildWhichIsA("ForceField") then continue end
...
local function createHitSound()
    ...
    sound.SoundId = voidhit.HitEffects.HitSoundID
    sound.Volume = voidhit.HitEffects.HitSoundVolume
    sound:Play()
...
TargetAimTab:AddToggle('TargetAimEnabled', {
    ...
    Callback = function(Value)
        voidhit.TargetAim.Enabled = Value
        if not Value then
            voidhit.TargetAim.Target = "None"
            tracer.Visible = false
            tracerOutline.Visible = false
    ...
    Mode = 'Toggle',
    Callback = function(Value)
        if not voidhit.TargetAim.Enabled then return end
        if Value then
            local target = GetClosestCharacter()
            if target then
                voidhit.TargetAim.Target = target.Name
            else
                voidhit.TargetAim.Target = "None"
            end
        else
...
if UserInputService.TouchEnabled then
    ...
    ImageButton.Image = "http://www.roblox.com/asset/?id=71425889880001"
    ...
    ImageButton.MouseButton1Click:Connect(function()
        if voidhit.TargetAim.Enabled then
...
TargetAimTab:AddToggle('AutoSelect', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.AutoSelect = Value
        if Value then
...
TargetAimTab:AddToggle('AutoFire', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.AutoFire = Value
    end
})
...
TargetAimTab:AddToggle('TargetStatsEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.TargetStats = Value
    end
...
local voidhit_TargetStats = Instance.new("ScreenGui")
...
voidhit_TargetStats.Name = "voidhit_TargetStats"
voidhit_TargetStats.Parent = game.CoreGui
voidhit_TargetStats.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
Background.Name = "Background"
Background.Parent = voidhit_TargetStats
...
RunService.Heartbeat:Connect(function()
    if voidhit.TargetAim.TargetStats and voidhit.TargetAim.Enabled then
        local targetName = voidhit.TargetAim.Target
...
TargetAimTab:AddToggle('Highlight', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.Highlight = Value
    end
}):AddColorPicker('HighlightOutline', {
    Default = voidhit.TargetAim.HighlightOutlineColor,
    Title = 'Outline Color',
    Callback = function(Value)
        voidhit.TargetAim.HighlightOutlineColor = Value
    end
})
...
local function connectHealth(target)
    ...
    healthConnections[target] = hum.HealthChanged:Connect(function(newHP)
        local highlight = target.Character and target.Character:FindFirstChild("voidhitHighlight")
        ...
        task.delay(0.5,function()
            if highlight and voidhit.TargetAim.Highlight then
                highlight.OutlineColor = voidhit.TargetAim.HighlightOutlineColor
            end
        end)
    end)
end
...
RunService.Heartbeat:Connect(function()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        local hl = plr.Character and plr.Character:FindFirstChild("voidhitHighlight")
        if hl and (not target or not voidhit.TargetAim.Highlight or plr ~= target) then
            hl:Destroy()
    ...
    if not voidhit.TargetAim.Highlight or not target or not target.Character then return end

    local hum = target.Character:FindFirstChild("Humanoid")
    if not hum then return end
    
    local highlight = target.Character:FindFirstChild("voidhitHighlight")
    if not highlight then
        highlight = Instance.new("Highlight")
        highlight.Name = "voidhitHighlight"
        highlight.FillTransparency = 1
        highlight.OutlineColor = .TargetAim.HighlightOutlineColor
...
TargetAimTab:AddToggle('DotCircle', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.DotCircle = Value
        DotCircle.Visible = Value and voidhit.TargetAim.Enabled
    end
...
TargetAimTab:AddToggle('Tracer', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.Tracer = Value
        tracer.Visible = false
        tracerOutline.Visible = false
    end
}):AddColorPicker('TracerFill', {
    Default = voidhit.TargetAim.TracerFillColor,
    Title = 'Fill Color',
    Callback = function(Value)
        voidhit.TargetAim.TracerFillColor = Value
        tracer.Color = Value
    end
}):AddColorPicker('TracerOutline', {
    Default = voidhit.TargetAim.TracerOutlineColor,
    Title = 'Outline Color',
    Callback = function(Value)
        voidhit.TargetAim.TracerOutlineColor = Value
...
TargetAimTab:AddDropdown('TracerPosition', {
    ...
    Callback = function(Value)
        voidhit.TargetAim.TracerPosition = Value
    end
})
...
TargetAimTab:AddToggle('LookAt', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.LookAt = Value
    end
})
TargetAimTab:AddButton('Teleport to Target', function()
    local targetName = voidhit.TargetAim.Target  -- Sử dụng target từ TargetAim
    if not targetName or targetName == "" or targetName == "None" then
        Library:Notify("void.hit - No target from TargetAim!", 3)
        return
    end
    local targetPlayer = Players:FindFirstChild(targetName)
    if not targetPlayer or not targetPlayer.Character or not targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        Library:Notify("void.hit - Target not found or no character!", 3)
        return
    end
    local hrp = localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart")
    if not hrp then
        Library:Notify("void.hit - Your character not loaded!", 3)
        return
    end
...
TargetAimTab:AddToggle('AutoKillEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.AutoKill = Value
    end
...
TargetAimTab:AddToggle('AutoKillEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.Spoofer = Value
    end
})
TargetAimTab:AddToggle('ToggleStrafe', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.ToggleStrafe = Value
    end
    ...
    Mode = 'Toggle',
    Callback = function(Value)
        if voidhit.TargetAim.ToggleStrafe then
            voidhit.TargetAim.Strafe = Value
            if Value then
...
TargetAimTab:AddToggle('VisualizeStrafe', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.VisualizeStrafe = Value
    end
}):AddColorPicker('VisInline', {
    Default = voidhit.TargetAim.VisualizeStrafeInlineColor,
    Title = 'Inline Color',
    Callback = function(Value)
        voidhit.TargetAim.VisualizeStrafeInlineColor = Value
        BodyCloneHighlight.FillColor = Value
    end
}):AddColorPicker('VisOutline', {
    Default = voidhit.TargetAim.VisualizeStrafeOutlineColor,
    Title = 'Outline Color',
    Callback = function(Value)
        voidhit.TargetAim.VisualizeStrafeOutlineColor = Value
...
TargetAimTab:AddDropdown('StrafeMethod', {
    ...
    Text = 'Strafe Method',
    Callback = function(Value)
        voidhit.TargetAim.StrafeMethod = Value
    end
})
...
TargetAimTab:AddSlider('StrafePrediction', {
    ...
    Rounding = 2,
    Callback = function(Value)
        voidhit.TargetAim.StrafePrediction = Value
    end
})
...
TargetAimTab:AddToggle('AutoStomp', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.AutoStomp = Value
    end
})
...
TargetAimTab:AddToggle('SpectateTarget', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.SpectateTarget = Value

        if Value then
            if voidhit.TargetAim.Target == "None" then
                Library:Notify(
                    "void.hit - ples ked select a target before using 'Spectate Target'.",
                    3
                )
                Toggles.SpectateTarget.Value = false
                voidhit.TargetAim.SpectateTarget = false
                return
            end
...
HitEffectsGroup:AddToggle('HitSkeleton', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.HitEffects.HitSkeleton.Enabled = Value
    end
}):AddColorPicker('HitSkeletonColor', {
    Default = Color3.fromRGB(255, 255, 255),
    Title = 'Skeleton Color',
    Callback = function(Value)
        voidhit.HitEffects.HitSkeleton.Color = Value
    end
})
HitEffectsGroup:AddToggle('HitChamsEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.HitEffects.HitChams.Enabled = Value
    end
}):AddColorPicker('HitChamsColor', {
    Default = voidhit.HitEffects.HitChams.Color,
    Title = 'Hit Chams Color',
    Callback = function(Value)
        voidhit.HitEffects.HitChams.Color = Value
    end
})
...
HitEffectsGroup:AddSlider('HitChamsLifetime', {
    ...
    Min = 1,
    Max = 10,
    Default = voidhit.HitEffects.HitChams.Lifetime,
    Rounding = 1,
    Callback = function(Value)
        voidhit.HitEffects.HitChams.Lifetime = Value
    end
})
...
HitEffectsGroup:AddSlider('HitChamsTransparency', {
    ...
    Min = 0,
    Max = 1,
    Default = voidhit.HitEffects.HitChams.Transparency,
    Rounding = 2,
    Callback = function(Value)
        voidhit.HitEffects.HitChams.Transparency = Value
    end
})

HitEffectsGroup:AddDropdown('HitChamsMaterial', {
    Values = {'Neon', 'ForceField'},
    Default = voidhit.HitEffects.HitChams.Material == "Neon" and 1 or 2,
    Multi = false,
    Text = 'Hit Chams Material',
    Callback = function(Value)
        voidhit.HitEffects.HitChams.Material = Value
    end
})
...
HitEffectsGroup:AddToggle('HitEffectEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.HitEffects.HitEffect.Enabled = Value
    end
}):AddColorPicker('HitEffectColor', {
    Default = voidhit.HitEffects.HitEffect.Color,
    Title = 'Effect Color',
    Callback = function(Value)
        voidhit.HitEffects.HitEffect.Color = Value
    end
})
...
HitEffectsGroup:AddDropdown('HitEffectType', {
    ...
    Text = 'Effect Type',
    Callback = function(Value)
        voidhit.HitEffects.HitEffect.Type = Value
    end
})
...
local function TriggerHitEffect(Player)
    if not voidhit.HitEffects.HitEffect.Enabled then return end
    if Player and Player.Character and Player.Character:FindFirstChild("HumanoidRootPart") then
        local HitPart = Player.Character.HumanoidRootPart
        local EffectType = voidhit.HitEffects.HitEffect.Type
        local EffectColor = voidhit.HitEffects.HitEffect.Color
...
HitEffectsGroup:AddToggle('HitNotifications', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.HitEffects.HitNotifications = Value
    end
})
...
HitEffectsGroup:AddSlider('NotifyTime', {
    ...
    Rounding = 0,
    Callback = function(Value)
        voidhit.HitEffects.HitNotificationsTime = Value
    end
})
...
HitEffectsGroup:AddToggle('HitSounds', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.HitEffects.HitSounds = Value
    end
})
...
HitEffectsGroup:AddDropdown('HitSoundSelect', {
    ...
    Text = 'Hit Sound',
    Callback = function(Value)
        voidhit.HitEffects.HitSoundID = hitsounds[Value]
    end
})
...
HitEffectsGroup:AddSlider('HitSoundVolume', {
    ...
    Rounding = 0,
    Callback = function(Value)
        voidhit.HitEffects.HitSoundVolume = Value
    end
})
...
ChecksTab:AddToggle('CheckForcefield', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.Checks.Forcefield = Value
    end
})
...
OptionsTab:AddInput('PredictionInput', {
    ...
    Placeholder = '0.0000',
    Callback = function(Value)
        voidhit.TargetAim.Prediction = tonumber(Value) or 0
    end
})
...
OptionsTab:AddToggle('AutoPredictToggle', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.AutoPredict = Value
    end
})
...
OptionsTab:AddDropdown('PredictModeDropdown', {
    ...
    Callback = function(Value)
        voidhit.TargetAim.PredictMode = Value
    end
})
...
OptionsTab:AddDropdown('HitPartDropdown', {
    ...
    Text = 'Hit Part',
    Callback = function(Value)
        voidhit.TargetAim.HitPart = Value
    end
})
...
OptionsTab:AddInput('OffsetInput', {
    ...
    Placeholder = '0',
    Callback = function(Value)
        voidhit.TargetAim.Offset = tonumber(Value) or 0
    end
})
...
OptionsTab:AddInput('JumpOffsetInput', {
    ...
    Placeholder = '0',
    Callback = function(Value)
        voidhit.TargetAim.JumpOffset = tonumber(Value) or 0
    end
})
...
OptionsTab:AddToggle('AirPartToggle', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.AirPartEnabled = Value
    end
})
...
OptionsTab:AddDropdown('AirPartDropdown', {
    ...
    Text = 'Airshot Part',
    Callback = function(Value)
        voidhit.TargetAim.AirPart = Value
    end
})
...
OptionsTab:AddToggle('ResolverToggle', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.TargetAim.Resolver = Value
    end
})
...
KillAuraGroup:AddToggle('KillAuraEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.KillAura.Enabled = Value
        if not Value then
            voidhit.KillAura.Active = false
            ka_tracer.Transparency = 1
        end
    end
    ...
    Mode = 'Toggle',
    Callback = function(Value)
        if voidhit.KillAura.Enabled then
            voidhit.KillAura.Active = Value
        end
    end
})
...
KillAuraGroup:AddToggle('KillAuraVisualize', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.KillAura.Visualize = Value
    end
...
local function setupEquipAll()
    local char = localPlayer.Character
    local backpack = localPlayer.Backpack
    if not char or not backpack or not voidhit.EquipAllGuns.Enabled then return end
    ...
    EquipAllConnection = char.ChildRemoved:Connect(function(child)
        if voidhit.EquipAllGuns.Enabled and child:IsA("Tool") and child:FindFirstChild("Ammo") and (tick() - lastEquipTime) >= 0.5 then
            lastEquipTime = tick()
            task.spawn(function()
                task.wait(0.1)
                equipAllGuns()
    ...
    EquipAddedConnection = backpack.ChildAdded:Connect(function(child)
        if voidhit.EquipAllGuns.Enabled and child:IsA("Tool") and child:FindFirstChild("Ammo") then
            child.Parent = char
        end
    end)
end
...
localPlayer.CharacterAdded:Connect(function()
    task.wait(5)
    if voidhit.EquipAllGuns.Enabled then
        setupEquipAll()
    end
end)
...
GunModsGroup:AddToggle('EquipAllGunsEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.EquipAllGuns.Enabled = Value
        if Value then
            setupEquipAll()
        else
            if EquipAllConnection then
...
GunModsGroup:AddToggle('RapidFireEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.RapidFire.Enabled = Value
...
GunModsGroup:AddToggle('WallbangEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.Wallbang.Enabled = Value
        if getnamecallmethod then
...
HitboxGroup:AddToggle('HitboxEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.HitboxExpander.Enabled = Value
    end
})
...
HitboxGroup:AddToggle('HitboxVisualize', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.HitboxExpander.Visualize = Value
    end
}):AddColorPicker('HitboxColor', {
    Default = voidhit.HitboxExpander.Color,
    Title = 'Fill Color',
    Callback = function(Value)
        voidhit.HitboxExpander.Color = Value
    end
}):AddColorPicker('HitboxOutline', {
    Default = voidhit.HitboxExpander.OutlineColor,
    Title = 'Outline Color',
    Callback = function(Value)
        voidhit.HitboxExpander.OutlineColor = Value
    end
})
...
HitboxGroup:AddSlider('HitboxFillTrans', {
    ...
    Rounding = 2,
    Callback = function(Value)
        voidhit.HitboxExpander.FillTransparency = Value
    end
})
...
HitboxGroup:AddSlider('HitboxOutlineTrans', {
    ...
    Rounding = 2,
    Callback = function(Value)
        voidhit.HitboxExpander.OutlineTransparency = Value
    end
})
...
HitboxGroup:AddSlider('HitboxSize', {
    ...
    Rounding = 0,
    Callback = function(Value)
        voidhit.HitboxExpander.Size = Value
    end
})

getgenv().voidhit.Legit = {
    Aimlock = false,
    HitPart = "Head",
    Smoothing = false,
    SmoothingAmount = 0.1,
...
getgenv().voidhit.Legit.Triggerbot.HitParts = {
...
getgenv().voidhit.Legit.Triggerbot.SelectedHitParts = {
    "Head", "HumanoidRootPart", "UpperTorso", "LowerTorso",
    "LeftUpperArm", "RightUpperArm", "LeftLowerArm", "RightLowerArm",
    "LeftHand", "RightHand", "LeftUpperLeg", "RightUpperLeg",
    "LeftLowerLeg", "RightLowerLeg", "LeftFoot", "RightFoot",
...
TriggerBox:AddToggle('TriggerbotEnabled', {
    Text = 'Enabled',
    Default = false,
    Callback = function(v) getgenv().voidhit.Legit.Triggerbot.Enabled = v end
}):AddKeyPicker('TargetAimKey', {
    Default = 'none',
    Text = 'Target Aim',
    Mode = 'Toggle',
    Callback = function(Value) getgenv().voidhit.Legit.Triggerbot.Enabled = Value end 
})
TriggerBox:AddToggle('TriggerbotOnlyTarget', {
    Text = 'Only Target',
    Default = false,
    Tooltip = 'Chỉ bắn khi target đang được aimbot lock',
    Callback = function(v) getgenv().voidhit.Legit.Triggerbot.OnlyTarget = v end
})
...
TriggerBox:AddSlider('TriggerbotFOV', {
    ...
    Default = 20,
    Rounding = 1,
    Callback = function(v) getgenv().voidhit.Legit.Triggerbot.FOV = v end
})
TriggerBox:AddDropdown('TriggerbotHitParts', {
    ...
    Default = getgenv().voidhit.Legit.Triggerbot.HitParts, -- chọn hết mặc định
    Multi = true,
    Callback = function(selected)
        getgenv().voidhit.Legit.Triggerbot.SelectedHitParts = selected
    end
})
TriggerBox:AddSlider('TriggerbotDelay', {
    ...
    Default = 0,
    Rounding = 2,
    Callback = function(v) getgenv().voidhit.Legit.Triggerbot.Delay = v end
})
...
TriggerBox:AddDropdown('TriggerbotChecks', {
    ...
    Callback = function(selected)
        getgenv().voidhit.Legit.Triggerbot.CheckKO     = table.find(selected, "KO")      ~= nil
        getgenv().voidhit.Legit.Triggerbot.CheckKnife  = table.find(selected, "Knife")   ~= nil
        getgenv().voidhit.Legit.Triggerbot.CheckGrab   = table.find(selected, "Grab")    ~= nil
        getgenv().voidhit.Legit.Triggerbot.CheckTeam   = table.find(selected, "Team")    ~= nil
        getgenv().voidhit.Legit.Triggerbot.CheckFriend = table.find(selected, "Friend")  ~= nil
    end
})
...
local function isValidTriggerTarget(plr)
    ...
    if getgenv().voidhit.Legit.Triggerbot.OnlyTarget then
        if not voidhit.TargetAim.Target or plr.Name ~= voidhit.TargetAim.Target then
            return false
        end
    end

    if getgenv().voidhit.Legit.Triggerbot.CheckTeam and plr.Team == LocalPlayer.Team then return false end
    if getgenv().voidhit.Legit.Triggerbot.CheckFriend and LocalPlayer:IsFriendsWith(plr.UserId) then return false end
    if getgenv().voidhit.Legit.Triggerbot.CheckKO and isKO(plr) then return false end
    if getgenv().voidhit.Legit.Triggerbot.CheckGrab and isGrabbed(plr) then return false end
    if getgenv().voidhit.Legit.Triggerbot.CheckKnife and isHoldingKnife() then return false end
    return true
end
local function GetBestTriggerPart()
    local bestPart, bestDist = nil, getgenv().voidhit.Legit.Triggerbot.FOV

    for _, plr in Players:GetPlayers() do
        if isValidTriggerTarget(plr) and plr.Character then
            for _, partName in ipairs(getgenv().voidhit.Legit.Triggerbot.SelectedHitParts) do
...
RunService.RenderStepped:Connect(function()
    local trigCfg = getgenv().voidhit.Legit.Triggerbot
...
SilentBox:AddToggle('SilentAimEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().voidhit.Legit.SilentAim = Value
    end
})
...
SilentBox:AddDropdown('SilentAimMethod', {
    ...
    Callback = function(Value)
        getgenv().voidhit.Legit.SilentAimMethod = Value
    end
})
SilentBox:AddToggle('SilentAimEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().voidhit.Legit.SilentFOVEnabled = Value
    end
})
SilentBox:AddSlider('SilentFOV', {
    ...
    Rounding = 0,
    Callback = function(Value)
        getgenv().voidhit.Legit.SilentFOV = Value
    end
})
local camera = workspace.CurrentCamera
...
local function getClosestPlayerToMouse()  -- Hàm closest player từ code Rival
    local closestPlayer = nil
    local shortestDistance = getgenv().voidhit.Legit.SilentFOVEnabled and getgenv().voidhit.Legit.SilentFOV or 9999
...
RunService.Heartbeat:Connect(function()
    if getgenv().voidhit.Legit.SilentAim and getgenv().voidhit.Legit.SilentAimMethod == "Rival" and not isLobbyVisible() then
...
spawn(function()
    ...
    RunService.Heartbeat:Connect(function()
        if not getgenv().voidhit.Legit.SilentAim or getgenv().voidhit.Legit.SilentAimMethod ~= "Flick" then
        ...
        if not BulletHandler then
            ...
            if BulletHandler and BulletHandler.Fire then
                ...
                BulletHandler.Fire = newcclosure(function(data, ...)
                    if not getgenv().voidhit.Legit.SilentAim then
                        return OldFire(data, ...)
                    end

                    local closest = get_closest_target(getgenv().voidhit.Legit.SilentFOVEnabled and getgenv()..Legit.SilentFOV or 9999)
...
AimlockBox:AddToggle('AimlockEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().voidhit.Legit.Aimlock = Value
    end
})
AimlockBox:AddDropdown('HitPart', {
    ...
    Text = 'Hit Part',
    Callback = function(Value)
        getgenv().voidhit.Legit.HitPart = Value
    end
})
AimlockBox:AddToggle('SmoothingEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().voidhit.Legit.Smoothing = Value
    end
})
AimlockBox:AddSlider('SmoothingAmount', {
    ...
    Rounding = 2,
    Callback = function(Value)
        getgenv().voidhit.Legit.SmoothingAmount = Value
    end
})
AimlockBox:AddSlider('Prediction', {
    ...
    Rounding = 2,
    Callback = function(Value)
        getgenv().voidhit.Legit.Prediction = Value
    end
})
AimlockBox:AddSlider('JumpOffset', {
    ...
    Rounding = 1,
    Callback = function(Value)
        getgenv().voidhit.Legit.JumpOffset = Value
    end
})
AimlockBox:AddToggle('ResolverToggle', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().voidhit.Legit.Resolver = Value
    end
})
local VelocityData = {}
...
local function updateCamlock()
    local targetName = voidhit.TargetAim.Target
    if not getgenv().voidhit.Legit.Aimlock then return end
    ...
    local part = target.Character:FindFirstChild(getgenv().voidhit.Legit.HitPart)
    ...
    if getgenv().voidhit.Legit.Resolver and VelocityData[target] then
        velocity = VelocityData[target].Velocity
    end
    
    local pred = getgenv().voidhit.Legit.Prediction or 0
    local pos = part.Position + (velocity * pred)
    
    --// JUMP OFFSET (nếu bật và đang rơi/jump)
    if getgenv().voidhit.Legit.Offset and target.Character.Humanoid:GetState() == Enum.HumanoidStateType.Freefall then
        pos = pos + Vector3.new(0, getgenv().voidhit.Legit.JumpOffset or 0, 0)
    end
    
    local goal = CFrame.new(Camera.CFrame.Position, pos)
    
    if getgenv().voidhit.Legit.Smoothing then
        Camera.CFrame = Camera.CFrame:Lerp(goal, getgenv().voidhit.Legit.SmoothingAmount)
    else
        Camera.CFrame = goal
    end
end
RunService.RenderStepped:Connect(function()
    if getgenv().voidhit.Legit.Aimlock then
        updateCamlock()
    end
end)
...
local voidhitEsp = loadstring(game:HttpGet('https://raw.githubusercontent.com/alebinh60/asmobile/refs/heads/main/esplib'))()
local esp = voidhitEsp
...
local function bullettracerlol(startPos, endPos)
    ...
    beam.Color = ColorSequence.new(voidhit.BulletTracers.Color)
    beam.Texture = voidhit.BulletTracers.TextureID
    beam.LightEmission = 1
    beam.Transparency = NumberSequence.new(voidhit.BulletTracers.Transparency)
    beam.Width0 = voidhit.BulletTracers.Size
    beam.Width1 = voidhit.BulletTracers.Size

    task.delay(voidhit.BulletTracers.TimeAlive, function()
        ...
        __namecall = (function(self, ...)
            ...
            if getnamecallmethod() == "FireServer" then
                if args[1] == "ShootGun" then
                    ...
                    -- Bullet Tracers
                    if voidhit.BulletTracers.Enabled then
...
BulletTracerGroup:AddToggle('BulletTracersEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.BulletTracers.Enabled = Value
        if not getnamecallmethod then
            Library:Notify("Your executor does not support this feature")
        end
    end
}):AddColorPicker('BulletTracersColor', {
    Default = voidhit.BulletTracers.Color,
    Title = 'Color',
    Callback = function(Value)
        voidhit.BulletTracers.Color = Value
    end
})
...
BulletTracerGroup:AddDropdown('BulletTracersTexture', {
    ...
    Callback = function(Value)
        if Value == "Beam" then
            voidhit.BulletTracers.TextureID = "rbxassetid://12781852245"
        elseif Value == "Lightning" then
            voidhit.BulletTracers.TextureID = "rbxassetid://446111271"
        elseif Value == "Heartrate" then
            voidhit.BulletTracers.TextureID = "rbxassetid://5830549480"
        elseif Value == "Chain" then
            voidhit.BulletTracers.TextureID = "rbxassetid://9632168658"
        elseif Value == "Glitch" then
            voidhit.BulletTracers.TextureID = "rbxassetid://8089467613"
        elseif Value == "Swirl" then
            voidhit.BulletTracers.TextureID = "rbxassetid://5638168605"
        end
    end
})
...
BulletTracerGroup:AddSlider('BulletTracersSize', {
    ...
    Rounding = 2,
    Callback = function(Value)
        voidhit.BulletTracers.Size = Value
    end
})
...
BulletTracerGroup:AddSlider('BulletTracersTransparency', {
    ...
    Rounding = 2,
    Callback = function(Value)
        voidhit.BulletTracers.Transparency = Value
    end
})
...
BulletTracerGroup:AddSlider('BulletTracersTimeAlive', {
    ...
    Rounding = 0,
    Callback = function(Value)
        voidhit.BulletTracers.TimeAlive = Value
    end
})
...
RunService.RenderStepped:Connect(function(deltaTime)
    ...
    if isFollowTargetEnabled and voidhit and voidhit.TargetAim and voidhit.TargetAim.Target ~= "None" then
        
        local targetPlayer = Players:FindFirstChild(voidhit.TargetAim.Target)
...
MovementGroup:AddToggle('SpeedEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.Movement.Speed.Enabled = Value
    end
    ...
    Text = 'Speed Key',
    Callback = function(Value)
        voidhit.Movement.Speed.Keybind = Value
    end
})
...
MovementGroup:AddSlider('SpeedValue', {
    ...
    Rounding = 0,
    Callback = function(Value)
        voidhit.Movement.Speed.Speed = Value
    end
})
MovementGroup:AddToggle('FlyEnabled', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.Movement.Fly.Enabled = Value
    end
}):AddKeyPicker('FlyKey', {
    Default = 'none',
    SyncToggleState = true,
    ...
    Text = 'Fly Key',
    Callback = function(Value)
        voidhit.Movement.Fly.Keybind = Value
    end
})
...
MovementGroup:AddSlider('FlySpeed', {
    ...
    Rounding = 0,
    Callback = function(Value)
        voidhit.Movement.Fly.Speed = Value
    end
})
...
MovementGroup:AddToggle('WalkSpeedToggle', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.Movement.WalkSpeedEnabled = Value
    end
...
MovementGroup:AddSlider('WalkSpeedSlider', {
    ...
    Rounding = 0,
    Callback = function(Value)
        voidhit.Movement.WalkSpeed = Value
    end
})
...
MovementGroup:AddToggle('JumpPowerToggle', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.Movement.JumpPowerEnabled = Value
    end
...
MovementGroup:AddSlider('JumpPowerSlider', {
    ...
    Rounding = 0,
    Callback = function(Value)
        voidhit.Movement.JumpPower = Value
    end
})

getgenv().voidhit.BunnyHop = {
    Enabled = false,
    Keybind = false,
    Speed = 100
}
MovementGroup:AddToggle('BunnyHop_Enabled', {
    ...
    Default = false,
    Callback = function(value)
        getgenv().voidhit.BunnyHop.Enabled = value
    end
    ...
    Callback = function(state)
        getgenv().voidhit.BunnyHop.Keybind = state
    end
})
...
MovementGroup:AddSlider('BunnyHop_Speed', {
    ...
    Rounding = 0,
    Callback = function(value)
        getgenv().voidhit.BunnyHop.Speed = value
    end
})

-- Logic Bunny Hop (chạy liên tục)
RunService.RenderStepped:Connect(function()
    if not getgenv().voidhit.BunnyHop.Enabled or not getgenv().voidhit.BunnyHop.Keybind then return end
...
MovementGroup:AddToggle('SpinBot_Enabled', {
    ...
    Default = false,
    Callback = function(value)
        getgenv().voidhit.SpinBot.Enabled = value
    end
...
MovementGroup:AddSlider('SpinBot_Speed', {
    ...
    Rounding = 0,
    Callback = function(value)
        getgenv().voidhit.SpinBot.Speed = value
    end
})
...
game.Players.LocalPlayer.Character.Humanoid.Health = 0
RunService.Heartbeat:Connect(function(dt)
    if not getgenv().voidhit.SpinBot.Enabled then
    ...
    local spinSpeed = getgenv().voidhit.SpinBot.Speed or 300  
...
RunService.Heartbeat:Connect(function()
    ...
    if hum then
        if voidhit.Movement.WalkSpeedEnabled then
            hum.WalkSpeed = voidhit.Movement.WalkSpeed
        end
        if voidhit.Movement.JumpPowerEnabled then
            hum.JumpPower = voidhit.Movement.JumpPower
...
localPlayer.CharacterAdded:Connect(function(char)
    ...
    if voidhit.Movement.FlyEnabled then StartFly() end
end)
-- Integrate Movement Logic into Heartbeat
RunService.Heartbeat:Connect(function(dt)
    -- Speed
    if voidhit.Movement.Speed.Enabled and voidhit.Movement.Speed.Keybind then
        if localPlayer and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local move_direction = localPlayer.Character.Humanoid.MoveDirection
            local hrp = localPlayer.Character.HumanoidRootPart
            hrp.CFrame = hrp.CFrame + (move_direction * dt) * voidhit.Movement.Speed.Speed * 10
        end
    end
    
    -- Fly
    if voidhit.Movement.Fly.Enabled and voidhit.Movement.Fly.Keybind then
        if localPlayer and localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
            local move_direction = localPlayer.Character.Humanoid.MoveDirection
            local hrp = localPlayer.Character.HumanoidRootPart
            local add = Vector3.new(0, (UserInputService:IsKeyDown(Enum.KeyCode.Space) and voidhit.Movement.Fly.Speed / 8 or UserInputService:IsKeyDown(Enum.KeyCode.LeftShift) and -voidhit.Movement.Fly.Speed / 8) or 0, 0)
            hrp.CFrame = hrp.CFrame + (move_direction * dt) * voidhit.Movement.Fly.Speed * 10
...
RunService.Heartbeat:Connect(function(delta)
    if not voidhit.TargetAim.Enabled or voidhit.TargetAim.Target == "None" then
        DotCircle.Visible = false
        return
    end
    ...
    local hitPart = targetPlayer.Character:FindFirstChild(voidhit.TargetAim.HitPart)
    if not hitPart then
        DotCircle.Visible = false
        return
    end
    ...
    -- Resolver
    if voidhit.TargetAim.Resolver then
    ...
    local predictVal = voidhit.TargetAim.AutoPredict and voidhit.TargetAim.PredictMode == "Ping" and (Stats.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000 + 0.035) or voidhit.TargetAim.Prediction
    local predictedPos = hitPart.Position + hitPart.Velocity * predictVal
    local screenPos, onScreen = Camera:WorldToViewportPoint(predictedPos)

    if onScreen and voidhit.TargetAim.DotCircle then
	    ...
	    for _, tool in pairs(char:GetChildren()) do
	        if tool:IsA("Tool") then
	            tool.Activated:Connect(function()
	                if not voidhit.TargetAim.Enabled or voidhit.TargetAim.Target == "None" or not CurrentGame then return end
	                ...
	                local predictVal = voidhit.TargetAim.Prediction
	                if voidhit.TargetAim.AutoPredict then
	                    if voidhit.TargetAim.PredictMode == "Ping" then
	                        predictVal = (game.Stats.Network.ServerStatsItem["Data Ping"]:GetValue() / 1000) + 0.035
	                    elseif voidhit.TargetAim.PredictMode == "Calculate" then
...
if getnamecallmethod and not isMobile then
    ...
    Meta.__index = function(t, k)
        if t == Mouse and (k == "Hit" or k == "hit") then
            if voidhit.TargetAim.Enabled and voidhit.TargetAim.Target ~= "None" and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                local TargetPlayer = Players:FindFirstChild(voidhit.TargetAim.Target)
                if TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Humanoid") and TargetPlayer.Character.Humanoid.Health > 0 and not TargetPlayer.Character:FindFirstChild("ForceField") then
                    ...
                    -- Team check
                    if voidhit.Checks.Team and TargetPlayer.Team == LocalPlayer.Team then return backupindex(t, k) end
                    ...
                    local partName = voidhit.TargetAim.HitPart
                    if voidhit.TargetAim.AirPartEnabled then
                        ...
                        if targetInAir or selfInAir then
                            partName = voidhit.TargetAim.AirPart
                        end
                    end
                    ...
                    if targetPart then
                        ...
                        -- Prediction logic
                        local pred = voidhit.TargetAim.Prediction -- manual default
                        if voidhit.TargetAim.AutoPredict then
                            if voidhit.TargetAim.PredictMode == "Ping Sets" then
                                pred = currentPred
                            elseif voidhit.TargetAim.PredictMode == "Calculate" then
...
if getnamecallmethod and game.GameId == 9825515356 then
    ...
    OldNamecall = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if method == "FireServer" and self.Name == "MainEvent" and args[1] == "Shoot" then
            if voidhit.TargetAim.Enabled and voidhit.TargetAim.AutoFire and voidhit.TargetAim.Target ~= "None" and localPlayer and localPlayer.Character then
                local TargetPlayer = players:FindFirstChild(voidhit.TargetAim.Target)
...
players.PlayerRemoving:Connect(function(plr)
    if plr.Name == voidhit.TargetAim.Target then
...
RunService.Heartbeat:Connect(function(dt)
    pcall(function()
        if getnamecallmethod and voidhit.RapidFire.Enabled then
        ...
        local useDesync = voidhit.TargetAim.Strafe or voidhit.TargetAim.AutoStomp or voidhit.KillAura.StompAura or voidhit.TargetAim.AutoKill or headshots.AutoLoadout.Enabled or BuyingSingle or BuyingAmmoActive
        local TargetPlayer = players:FindFirstChild(voidhit.TargetAim.Target)
        ...
        if useDesync and hrp then
            if voidhit.TargetAim.Strafe and voidhit.TargetAim.Target ~= "None" and TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Head") and not (AutoKillActive or AutoArmorActive or AutoLoadoutActive or BuyingSingleActive or AutoHealActive) then
                if not TargetPlayer.Character:FindFirstChild("ForceField") then
                    if not KnockCheck(TargetPlayer) then
                        ...
                        local strafeOffset
                        if voidhit.TargetAim.StrafeMethod == "Orbit" then
                            strafeOffset = Vector3.new(math.cos(tick()*10)*10, 0, math.sin(tick()*10)*10)
                        elseif voidhit.TargetAim.StrafeMethod == "Randomize" then
                        ...
                        if voidhit.TargetAim.Spoofer then
                            RunService:BindToRenderStep("RestoreStrafe", 199, function()
                                hrp.CFrame = SavedPosition
                                RunService:UnbindFromRenderStep("RestoreStrafe")
                            end)
                        end
                        if voidhit.TargetAim.VisualizeStrafe then
                            BodyClone:SetPrimaryPartCFrame(hrp.CFrame)
                            BodyCloneHighlight.Enabled = true
                            SetRigTransparency(BodyClone, 0)
                            SetRigColor(BodyClone, voidhit.TargetAim.VisualizeStrafeInlineColor)
                            BodyCloneHighlight.FillColor = voidhit.TargetAim.VisualizeStrafeInlineColor
                            BodyCloneHighlight.OutlineColor = voidhit.TargetAim.VisualizeStrafeOutlineColor
                        else
                            BodyCloneHighlight.Enabled = false
                        ...
                        if voidhit.TargetAim.AutoFire and MainEvent then
            ...
            if voidhit.TargetAim.Enabled and voidhit.TargetAim.Target ~= "None" and TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Head") and not KnockCheck(TargetPlayer) then
                local tool = localPlayer.Character:FindFirstChildOfClass("Tool")
                if tool and not TargetPlayer.Character:FindFirstChild("ForceField") then
                    if voidhit.TargetAim.AutoFire then
...
RunService.Heartbeat:Connect(function(dt)
	...
	local useDesync = voidhit.TargetAim.Strafe or voidhit.TargetAim.AutoStomp or voidhit.KillAura.StompAura or voidhit.TargetAim.AutoKill or headshots.AutoLoadout.Enabled or BuyingSingle or BuyingAmmo
	...
	if voidhit.TargetAim.Enabled and voidhit.TargetAim.Target ~= "None" then
	    local TargetPlayer = players:FindFirstChild(voidhit.TargetAim.Target)
	    if TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Humanoid") then
	        ...
	        if prev ~= nil and curr < prev then
	            -- Hit Sound
	            if voidhit.HitEffects.HitSounds then
	                createHitSound()
	            end
	
	            -- Hit Notification
	            if voidhit.HitEffects.HitNotifications then
	                Library:Notify("void.hit - Target Hit: " .. TargetPlayer.Name .. " - Health: " .. curr, voidhit.HitEffects.HitNotificationsTime)
	            end
	
	            -- Hit Chams
	            if voidhit.HitEffects.HitChams.Enabled then
	                HitChams(TargetPlayer)
	            end
                if voidhit.HitEffects.HitEffect.Enabled then
                    TriggerHitEffect(TargetPlayer)
                end
                if voidhit.HitEffects.HitSkeleton.Enabled then
    ...
    if voidhit.KillAura.Enabled then
        if Tool and Tool:FindFirstChild("Handle") then
			...
			local ka_target = nil
			for _, player in pairs(players:GetPlayers()) do
				if player ~= localPlayer and not voidhit.KillAura.Whitelist[player.Name] and player.Character and player.Character:FindFirstChild("Head") and not player.Character:FindFirstChild("GRABBING_CONSTRAINT") then
					if player.Character.BodyEffects:FindFirstChild("K.O") and not player.Character.BodyEffects["K.O"].Value then
						...
						if dist < closest and dist <= voidhit.KillAura.Range then
							closest = dist
							ka_target = player
						end
					end
			...
			if ka_target and ka_target.Character and ka_target.Character:FindFirstChild("Head") then
				...
				if voidhit.KillAura.Visualize then
				...
				local offset = voidhit.KillAura.Silent and Vector3.new(0, -12, 0) or Vector3.new(0, 0, 0)
    ...
    if voidhit.TargetAim.Enabled and voidhit.TargetAim.Target ~= "None" then
        ...
        if TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Humanoid") and TargetPlayer.Character:FindFirstChild("Head") and not TargetPlayer.Character:FindFirstChild("ForceField") then
            ...
            if humanoid then
                ...
                if lastHealth == nil or voidhit.TargetAim.Target ~= lastTarget then
                    lastHealth = currentHealth
                end
                if currentHealth < lastHealth then
                    if voidhit.HitEffects.HitNotifications then
                        Library:Notify("void.hit - Target Hit: " .. TargetPlayer.Name .. " - Health: " .. currentHealth, .HitEffects.HitNotificationsTime)
                    end
                    if voidhit.HitEffects.HitSounds then
                        createHitSound()
                    end
                ...
                lastHealth = currentHealth
                lastTarget = voidhit.TargetAim.Target
            end
        end
    end
    if voidhit.TargetAim.Enabled and voidhit.TargetAim.Target ~= "None" and TargetPlayer and TargetPlayer.Character and TargetPlayer.Character:FindFirstChild("Head") then
        if voidhit.TargetAim.LookAt then
        ...
        if voidhit.TargetAim.Tracer then
            ...
            if head and humanoidRootPart then
                ...
                if onScreen then
                    ...
                    local tool = localPlayer.Character:FindFirstChildOfClass("Tool")
                    if voidhit.TargetAim.TracerPosition == "Tool" and tool and tool:FindFirstChild("Handle") then
    ...
    if voidhit.TargetAim.Target ~= "None" then
        ...
        if TargetPlayer then
            ...
            Library:Notify(voidhit.TargetAim.Target .. " left the game", 3)
            voidhit.TargetAim.Target = "None"
            previousKnock = false
            previousDead = false
            lastTargetCharacter = nil
    ...
    if useDesync and hrp then
        if voidhit.TargetAim.AutoStomp and TargetPlayer and KnockCheck(TargetPlayer) and localPlayer.Character.BodyEffects.Reload.Value == false then
            ...
            if isDead == false then
                ...
                if voidhit.HitEffects.HitNotifications then
                    if tick() - t >= 3 then
                        t = tick()
                        Library:Notify("void.hit - Attempted to Stomp Target: " .. TargetPlayer.Name, voidhit.HitEffects.HitNotificationsTime)
                    end
                end
                MainEvent:FireServer("Stomp")
            end
        elseif voidhit.KillAura.StompAura and not (AutoKillActive or TargetAimActive or AutoArmorActive or AutoLoadoutActive or BuyingSingleActive or AutoHealActive) then
            local shortestDistance = math.huge
            local stompTarget
            for _, player in pairs(players:GetPlayers()) do
                if player ~= localPlayer and player.Character and player.Character:FindFirstChild("Head") and KnockCheck(player) and not player.Character:FindFirstChild("ForceField") then
                    ...
                    if distance <= voidhit.KillAura.Range and distance < shortestDistance then
                        shortestDistance = distance
                        stompTarget = player
                    end
                end
...
RunService.Heartbeat:Connect(function()
    ...
    ------------------------------------------------------------
    if voidhit.TargetAim.Enabled
        and voidhit.TargetAim.Strafe
        and voidhit.TargetAim.Target ~= "None"
        and localPlayer.Character
        and localPlayer.Character:FindFirstChild("HumanoidRootPart")
        and localPlayer.Character:FindFirstChild("Humanoid")
        and localPlayer.Character:FindFirstChild("Head") then

        local target = players:FindFirstChild(voidhit.TargetAim.Target)
        if target and target.Character and target.Character:FindFirstChild("Head")
            ...
            if ((not KnockCheck(target)) or (voidhit.TargetAim.AutoStomp and dead == false)) then
...
RunService.Heartbeat:Connect(function(dt)
    if voidhit.TargetAim.AutoKill and not (TargetAimActive or AutoArmorActive or AutoLoadoutActive or BuyingSingleActive or AutoHealActive or BuyingAmmoActive) then
        local targetName = voidhit.TargetAim.Target
...
RS.RenderStepped:Connect(function()
    if voidhit.HitboxExpander.Enabled then
        for _, Player in pairs(players:GetPlayers()) do
            if Player == Client then continue end
            local HRP = Player.Character and Player.Character:WaitForChild("HumanoidRootPart")
            if HRP then
                HRP.Size = Vector3.new(voidhit.HitboxExpander.Size, voidhit.HitboxExpander.Size, voidhit.HitboxExpander.Size)
                HRP.CanCollide = false
                if voidhit.HitboxExpander.Visualize then
                    if not highlights[Player] then
                        local Visualize = Instance.new("Highlight")
                        HRP.Transparency = 0.9
                        Visualize.Parent = HRP
                        Visualize.FillColor = voidhit.HitboxExpander.Color
                        Visualize.OutlineColor = voidhit.HitboxExpander.OutlineColor
                        Visualize.FillTransparency = voidhit.HitboxExpander.FillTransparency
                        Visualize.OutlineTransparency = voidhit.HitboxExpander.OutlineTransparency
...
local function loadDance(name)
    ...
    if animId then
        ...
        if getgenv().voidhit.Dance.Playing then
...
AnimationGroup:AddDropdown('DanceSelect', {
    ...
    Searchable = true,
    Callback = function(v)
        getgenv().voidhit.Dance.Selected = v
        loadDance(v)
    end
})
...
AnimationGroup:AddToggle('DanceToggle', {
    ...
    Default = false,
    Callback = function(v)
        getgenv().voidhit.Dance.Enabled = v
        getgenv().voidhit.Dance.Playing = v
...
getgenv().voidhit.Character = getgenv().voidhit.Character or {}
getgenv().voidhit.Character.Noclip = false

local NoclipConnection
local function toggleNoclip(bool)
    getgenv().voidhit.Character.Noclip = bool
    if bool then
...
RunService.Heartbeat:Connect(function()
    ...
    if voidhit.Desync.Enabled then
        if voidhit.Desync.Mode == "Destroy Cheaters" then
            desyncCFrame = CFrame.new(9e9, 1, 1) * oldCFrame.Rotation  -- Giữ rotation
        elseif voidhit.Desync.Mode == "Underground" then
            desyncCFrame = CFrame.new(hrp.Position - Vector3.new(0, 12, 0)) * oldCFrame.Rotation
        elseif voidhit.Desync.Mode == "Void Spam" then
        ...
        elseif voidhit.Desync.Mode == "Void" then
        ...
        elseif voidhit.Desync.Mode == "Random" then
            local amount = voidhit.Desync.RandomAmount
        ...
        elseif voidhit.Desync.Mode == "Safe Shoot" then
            desyncCFrame = CFrame.new(hrp.Position - Vector3.new(0, 5, 0)) * CFrame.Angles(math.random(0, 360), math.random(0, 360), math.rad(180))
        elseif voidhit.Desync.Mode == "Custom" then
            desyncCFrame = CFrame.new(hrp.Position - Vector3.new(voidhit.Desync.CustomX, voidhit.Desync.CustomY, voidhit.Desync.CustomZ)) * oldCFrame.Rotation
        end

        -- Apply Desync (giữ nguyên)
        hrp.CFrame = desyncCFrame
        if not voidhit.TargetAim.SpectateTarget then
	...
	elseif voidhit.TargetAim.AutoKill then
	    visualizeCFrame = killpos
	else
	    visualizeCFrame = hrp.CFrame
	end
	
	if voidhit.Desync.Visualize and voidhit.Desync.Enabled then
	...
	if voidhit.Desync.Line and voidhit.Desync.Enabled then
	...
	if voidhit.Desync.Dot and voidhit.Desync.Enabled then
	    local targetPosition = visualizeCFrame.Position
    ...
    if voidhit.Desync.Status then
        DesyncStatus.Text = "Desync: " .. (voidhit.Desync.Enabled and "TRUE" or "FALSE")
        DesyncStatus.Color = voidhit.Desync.Enabled and Color3.fromRGB(0, 255, 0) or Color3.fromRGB(255, 0, 0)
...
UserInputService.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 and voidhit.Desync.Status then
        local mousePos = UserInputService:GetMouseLocation()
...
DesyncGroup:AddToggle('DesyncEnabled', {
    ...
    Callback = function(Key)
        ...
        voidhit.Desync.Enabled = not voidhit.Desync.Enabled
        if not voidhit.Desync.Enabled then
            resetCamera()
        end
    end
})
...
DesyncGroup:AddSlider('DesyncRandomAmount', {
    ...
    Compact = false,
    Callback = function(Value)
        voidhit.Desync.RandomAmount = Value
    end
})
...
DesyncGroup:AddToggle('DesyncVisualize', {
    ...
    Default = false,
    Callback = function(Value)
        voidhit.Desync.Visualize = Value
    end
...
PlayerListGroup:AddButton('Set TargetAim', function()
    if selectedPlayer then
        voidhit.TargetAim.Target = selectedPlayer.Name
...
Promo = {
    "void.hit On Top",
    "void.hit so clean yall still losing to it..",
    "Must be rough playing without void.hit 💔",
    "cant win a hvh? maybe try /voidhit",
    "if u wanna stop losing hop in /voidhit",
    "if u cant win just get void.hit",
    "how to win a hvh? step 1: get void.hit",
    "gg/aWhK6KFHZ",
    "void.hit users eat, yall starve",
    "No void.hit? no wins. simple.",
    "You crying in chat while void.hit cooking",
    "If missing shots was a job you’d be CEO",
    "void.hit got me winning, you got excuses",
    "Keep blaming lag, we know it's skill",
    "void.hit smooth, yall movement shaky",
    "void.hit on top, your aim on vacation",
    "You fighting air while void.hit hitting heads",
    "void.hit turning bots into bosses",
    "You need void.hit more than WiFi",
    "void.hit users glide, you slide",
    "Your gameplay crying, void.hit thriving",
    "You chasing clout, void.hit chasing wins",
    "void.hit got the boost, you got excuses",
    "You pressing buttons, we pressing victory",
    "void.hit users pop off, you pop up dead",
    "Skill issue? void.hit don’t judge",
    "void.hit keeps you calm, you keep panic",
    "No void.hit? say hello to respawn",
    "void.hit players flex, you stumble",
    "Your aim wandering, void.hit laser-focused",
    "void.hit got bars, you got tears",
    "Try as you might, void.hit too strong",
    "void.hit users cook, you still reading the recipe",
    "Your shots missing, void.hit hitting",
    "void.hit above, you below",
    "We slide, you collide",
    "void.hit users shine, your screen blurry",
    "void.hit clean, your game messy",
    "You talking big, we acting bigger",
    "void.hit got moves, you got panic",
    "Without void.hit? GG for you",
    "void.hit making legends, you making mistakes",
    "You flinch, we feast",
    "void.hit players rise, you respawn",
    "We dominate, you imitate",
    "void.hit got juice, you got crumbs",
    "void.hit never sleeps, you lag",
    "You chasing shadows, we chasing wins",
    "void.hit users glide, your feet stuck",
    "No void.hit? keep dreaming",
    "void.hit built for pros, you built for bots",
    "Your keyboard crying, void.hit laughing",
    "void.hit users on top, you hitting ground",
    "You tap, we snap",
    "void.hit got focus, you got panic attacks",
    "Keep trying, we keep winning",
    "void.hit moving smooth, you jittery",
    "void.hit users dominate, you spectate",
    "You losing fast, void.hit blasting",
    "void.hit got style, your shots messy",
    "You lag, we swag",
    "void.hit players clean, you dirty",
    "Your moves weak, void.hit strong",
    "void.hit users laugh, you cry",
    "Keep missing, void.hit keep scoring",
    "You freeze, we feast",
    "void.hit got energy, you tired",
    "void.hit turning noobs into legends",
    "You flailing, we flying",
    "void.hit smooth operator, you panic button",
    "No void.hit? surrender early",
    "void.hit got tactics, you got chaos",
    "You panic, we execute",
    "void.hit users calm, you screaming",
    "void.hit gliding, you stumbling",
    "You pray, we slay",
    "void.hit on top, your score low",
    "Your shots weak, void.hit strong",
    "void.hit always moving, you stuck",
    "You spinning wheels, void.hit winning",
    "void.hit users cooking, you freezing",
    "You panic, we dominate",
    "void.hit turning losses into wins",
    "You chasing shadows, we chasing glory",
    "void.hit got juice, your hands dry",
    "You flinch, we feast",
    "void.hit players glide, you stumble",
    "Your aim shaky, void.hit steady",
    "void.hit above, you below",
    "You miss shots, we hit",
    "void.hit users rise, you respawn",
    "Your tactics weak, void.hit strong",
    "void.hit got bars, you got lag",
    "You panic, we score",
    "void.hit on top, you bottom",
    "You crying, void.hit laughing",
    "void.hit got power, you weak",
    "You tap, we dominate",
    "void.hit shining, you hiding",
    "Your game messy, void.hit clean",
    "You flail, we flex",
    "void.hit cooking, you freezing",
    "void.hit users glide, you slide",
    "Your aim wandering, void.hit laser",
    "You panic, we feast",
    "void.hit got style, your shots weak",
    "You losing, void.hit winning",
    "void.hit moving smooth, you jittery",
    "No void.hit? just GG",
    "void.hit built for winners, you built for bots",
    "Your keyboard crying, void.hit laughing",
    "You spin, we glide",
    "void.hit users on top, you bottom",
    "You press panic, we press victory",
    "void.hit got juice, you dry",
    "You flinch, we feast",
    "void.hit players dominate, you spectate",
    "You lag, we swag",
    "void.hit got tactics, you chaos",
    "No void.hit? just pray",
    "void.hit smooth operator, you panic button",
    "Your game weak, void.hit strong",
    "void.hit users clean, you messy",
    "You tap, we snap",
    "void.hit on top, your score low",
    "Your shots miss, void.hit hits",
    "void.hit always moving, you stuck",
...
getgenv().voidhit = {}
getgenv().voidhit.WatermarkEnabled = true
getgenv().voidhit.WatermarkShowFPS = true
getgenv().voidhit.WatermarkShowGameName = false
getgenv().voidhit.WatermarkShowUptime = false
getgenv().voidhit.WatermarkShowExecutor = false
getgenv().voidhit.WatermarkShowPing = true
local Stats = game:GetService("Stats")
...
WatermarkGroup:AddToggle('WatermarkEnabled', {
    ...
    Default = true,
    Callback = function(Value)
        getgenv().voidhit.WatermarkEnabled = Value
    end
})
...
WatermarkGroup:AddToggle('WatermarkShowFPS', {
    ...
    Default = true,
    Callback = function(Value)
        getgenv().voidhit.WatermarkShowFPS = Value
    end
})
...
WatermarkGroup:AddToggle('WatermarkShowGameName', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().voidhit.WatermarkShowGameName = Value
    end
})
...
WatermarkGroup:AddToggle('WatermarkShowUptime', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().voidhit.WatermarkShowUptime = Value
    end
})
...
WatermarkGroup:AddToggle('WatermarkShowExecutor', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().voidhit.WatermarkShowExecutor = Value
    end
})
...
WatermarkGroup:AddToggle('WatermarkShowPing', {
    ...
    Default = true,
    Callback = function(Value)
        getgenv().voidhit.WatermarkShowPing = Value
    end
})
...
ThemeManager:SetFolder('voidhit')
SaveManager:SetFolder('voidhit/dahood')
