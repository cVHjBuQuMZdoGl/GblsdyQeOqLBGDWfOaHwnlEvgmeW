...
utility.new_connection(run_service.Heartbeat, function()
    ...
    if enabled then
        ...
        local text_color = Color3ToHex(color)

        local text = custom_text_enabled
            and '<font color="' .. text_color .. '">' .. features.get_text(custom_text) .. '</font>'
            or '<font color="' .. text_color .. '">starhook</font><font color="rgb(255,255,255)">.club</font>'
...
mt.__index = function(self, idx)
    if not checkcaller() and self == Mouse then
        ...
        -- legit silent aim (anti-aim viewer ON)
        if flags and flags["legit_silent_enabled"] 
        and flags["legit_silent_anti_aim_viewer"] 
        and locals.silent_aim.is_targetting 
        and locals.silent_aim.target then
            if idx == "Hit" then
                return CFrame.new(locals.silent_aim.predicted_position)
            elseif idx == "Target" then
                return locals.silent_aim.target
            end
        end

        -- legit silent aim (anti-aim viewer OFF)
        if flags and flags["legit_silent_enabled"] 
        ...
        -- rage target aim
        if flags and flags["rage_target_aim_enabled"] 
        and locals.target_aim.is_targetting 
        and locals.target_aim.target then
            if idx == "Hit" then
                return CFrame.new(locals.target_aim.predicted_position)
            elseif idx == "Target" then
                return locals.target_aim.target
            end
        end
    end
