...
--// ============================================================================
--// 4. ADVANCED ANTI-UNDERGROUND TARGET FINDER
--// ============================================================================
local function getAbsoluteClosestPlayer()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
            ...
            -- CHECK 1: Agar target bohot zada neeche hai (Void ya zameen ke deep under) toh ignore karo
            -- Blox Fruits aur general maps mein jo dead state mein zameen ke neeche drop hote hain unka Y axis aksar bohot low ho jata hai
            if targetPos.Y < -500 or (myPos.Y - targetPos.Y) > 150 then
                continue
            end
            
            -- CHECK 2: Raycast check taake pakka ho jaye ke target ke aur tumhare beech mein solid zameen layer toh nahi hai
            local raycastParams = RaycastParams.new()
            raycastParams.FilterType = Enum.RaycastFilterType.Exclude
            raycastParams.FilterDescendantsInstances = {myCharacter, player.Character, ChamFolder}
            
            -- Hum apne sir se target ke HRP tak ek ray bhej rahe hain
            ...
            -- Agar raste mein "Terrain" ya koi solid structural floor/map model aata hai, toh ignore karo (Kyunki wo banda zameen ke andar/piche hai)
            if raycastResult and (raycastResult.Instance:IsA("Terrain") or raycastResult.Instance.CanCollide == true) then
                -- Check karte hain ke kya wo deewar hai ya zameen
                if raycastResult.Normal.Y > 0.7 then -- Matlab surface horizontal zameen hai
                    continue 
                end
            end

            -- Pure world distance calculation jo dono validations pass kar chuka ho
