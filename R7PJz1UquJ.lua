getgenv().ExtractCC = function()
    ...
    local MyTrail, TrailAtt0, TrailAtt1 -- Track local trail instances
    local ActiveAuraParticles = {} -- Track active aura parts
    ...
    SelfGroup:AddToggle('CharacterTrail', { Text = 'Character Trail', Default = false }):AddColorPicker('TrailColor', { Default = Color3.fromRGB(255, 255, 255) })
    
    -- Added Aura toggles and dropdown configurations
    SelfGroup:AddToggle('AuraToggle', { Text = 'Aura Enabled', Default = false })
    SelfGroup:AddDropdown('SelectedAura', { Values = { 'Glow Aura', 'Rings Aura', 'Electric Aura' }, Default = 1, Text = 'Aura Type' })

    -- [[ VISUALS TAB - NEW HIT EFFECTS GROUP ]] --
    local HitEffectsGroup = Tabs.Visuals:AddRightGroupbox('Hit Effects')

    -- 1st Toggle: Hit Effects Selection
    HitEffectsGroup:AddToggle('HitParticlesToggle', { Text = 'Hit Particles', Default = false })
    HitEffectsGroup:AddDropdown('SelectedHitEffect', { Values = { 'coom hit effects', 'nova hit effects', 'bubble hit effect' }, Default = 1, Text = 'Particle Style' })

    -- 2nd Toggle: Hit Chams Configuration
    HitEffectsGroup:AddToggle('HitChamsToggle', { Text = 'Hit Chams', Default = false }):AddColorPicker('HitChamsColor', { Default = Color3.fromRGB(255, 0, 75) })
    HitEffectsGroup:AddSlider('HitChamsDuration', { Text = 'Chams Duration', Default = 1.5, Min = 0.1, Max = 5, Rounding = 1 })
    HitEffectsGroup:AddSlider('HitChamsTransparency', { Text = 'Chams Transparency', Default = 0.4, Min = 0, Max = 1, Rounding = 2 })

    -- 3rd Toggle: Hit Camera Shake Impact Action
    HitEffectsGroup:AddToggle('HitShakeToggle', { Text = 'Hit Camera Shake', Default = false })
    HitEffectsGroup:AddDropdown('HitShakeIntensity', { Values = { 'Light', 'Medium', 'Heavy' }, Default = 2, Text = 'Shake Impact Force' })
    ...
    -- [[ DYNAMIC HIT EFFECT GENERATION SYSTEM ]] --
    local function SpawnHitEffects(position, character)
        -- Process Hit Particle Emitters
        ...
        -- Process Hit Chams (Highlight Mesh Overlay)
        ...
        -- Process Hit Camera Shake (Toggle 3 Action Item)
    ...
    function GetClosestToCrosshair(ignoreWallsAndKnock)
        ...
        for _, ent in ipairs(GetEntities()) do
            if Toggles.TeamCheck.Value and ent.Team == LocalPlayer.Team then continue end
    ...
    RunService.Heartbeat:Connect(function(dt)
        ...
        -- Self Upgraded Character Trail Logic
        if char and root then
            if Toggles.CharacterTrail.Value then
                if not MyTrail or not MyTrail.Parent or MyTrail.Parent ~= root then
                    ...
                    TrailAtt0.Name = "TrailAttachment0"
                    TrailAtt0.Position = Vector3.new(0, 1.4, 0)
                    TrailAtt0.Parent = root

                    TrailAtt1 = Instance.new("Attachment")
                    TrailAtt1.Name = "TrailAttachment1"
                    TrailAtt1.Position = Vector3.new(0, -1.4, 0)
                    TrailAtt1.Parent = root

                    MyTrail = Instance.new("Trail")
                    MyTrail.Name = "SelfMovementTrail"
                    MyTrail.Attachment0 = TrailAtt0
                    MyTrail.Attachment1 = TrailAtt1
                    MyTrail.FaceCamera = true
                    MyTrail.Lifetime = 0.5
                    
                    -- Exactly like premium stream trails: smooth taper width scale, fading transparency sequence, light glow emission
                    MyTrail.WidthScale = NumberSequence.new({
                        NumberSequenceKeypoint.new(0, 1.2),
                        NumberSequenceKeypoint.new(0.5, 0.6),
                        NumberSequenceKeypoint.new(1, 0)
                    })
                    MyTrail.Transparency = NumberSequence.new({
                        NumberSequenceKeypoint.new(0, 0.1),
                        NumberSequenceKeypoint.new(1, 1)
                    })
                    MyTrail.LightEmission = 0.85
                    MyTrail.Texture = "rbxassetid://4011115456"
                    MyTrail.Parent = root
                end
                
                if MyTrail then
                    MyTrail.Color = ColorSequence.new(Options.TrailColor.Value)
                end
            else
            ...
            if Toggles.AuraToggle.Value then
                ...
                if not ActiveAuraParticles[selected] or not ActiveAuraParticles[selected].Parent then
                    ...
                    if selected == "Glow Aura" then
                        ...
                        emitter.Color = ColorSequence.new(Color3.fromRGB(190, 0, 255))
                    ...
                    elseif selected == "Electric Aura" then
                        ...
                        emitter.Color = ColorSequence.new(Color3.fromRGB(255, 230, 0))
                        emitter.LightEmission = 0.9
                    end
        ...
        if Toggles.SilentEnabled.Value then
            ...
            if SilentTarget and SilentTarget.Character then
                ...
                if sPart then
                    ...
                    -- Hit Detection For Silent Aim Mode
                    if sHum then
                        ...
                        if currentHealth < previousHealth then
                            PlayHitSound()
                            SpawnHitEffects(sPart.Position, SilentTarget.Character)
                        end
        ...
        -- Hit Detection Sound & Visual Effects Hook (Aimlock Mode)
        if LockActive and CurrentTarget and CurrentTarget.Character then
            ...
            if hum then
                ...
                if currentHealth < previousHealth then 
                    PlayHitSound() 
                    if tPart then
                        SpawnHitEffects(tPart.Position, CurrentTarget.Character)
                    end
                end
        ...
        if LockActive then
            ...
            elseif CurrentTarget ~= TargetNotified then
                -- Target automatically shifted to a different player
                if TargetNotified then
                    Library:Notify("speared " .. TargetNotified.Name)
                end
                Library:Notify("locked on " .. CurrentTarget.Name)
    ...
    -- [[ SILENT AIM & TARGET LOCK REDIRECTION HOOKS ]] --
    local mt = getrawmetatable(game)
    local oldIndex = mt.__index
    local oldNamecall = mt.__namecall
    setreadonly(mt, false)

    -- Universal targeting calculation algorithm helper
    local function GetTargetPosition(target, partName, xPred, yPred)
        ...
        if Toggles.AntiGroundShot.Value and vel.Y < -10 then
            vel = Vector3.new(vel.X, 0, vel.Z)
        end
        
        if Toggles.ResolverToggle.Value then
            local method = Options.ResolverMethod.Value
            if method == "Velocity Null" then
                vel = Vector3.new(math.clamp(vel.X, -70, 70), math.clamp(vel.Y, -70, 70), math.clamp(vel.Z, -70, 70))
            elseif method == "LookVector" then
                vel = part.CFrame.LookVector * part.Velocity.Magnitude
            end
        end
        
        local pos = part.Position + (vel * Vector3.new(xPred, yPred, xPred)) + SharedShake
        
        if hum and (hum.FloorMaterial == Enum.Material.Air or hum:GetState() == Enum.HumanoidStateType.Freefall or hum:GetState() == Enum.HumanoidStateType.Jumping) then
    ...
    mt.__index = newcclosure(function(self, index)
        if not checkcaller() then
            -- Override Mouse tracking coordinates perfectly
            if self == Mouse and (index == "Hit" or index == "Target") then
                if Toggles.SilentEnabled.Value and SilentTarget then
                    local pos, part = GetTargetPosition(SilentTarget, Options.SilentPart.Value, tonumber(Options.SilentX.Value) or 0.1, tonumber(Options.SilentY.Value) or 0.1)
                    if pos and part then
                        return (index == "Hit" and CFrame.new(pos) or part)
                    end
                end

                if LockActive and CurrentTarget and Options.Method.Value == "Index" then
                    local pos, part = GetTargetPosition(CurrentTarget, Options.TargetPart.Value, tonumber(Options.XPred.Value) or 0.1, tonumber(Options.YPred.Value) or 0.1)
                    if pos and part then
                        return (index == "Hit" and CFrame.new(pos) or part)
                    end
                end
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        if not checkcaller() then
            -- Discover active locked targets
            local activeTarget, partName, predX, predY = nil, nil, nil, nil
            if Toggles.SilentEnabled.Value and SilentTarget then
                activeTarget = SilentTarget
                partName = Options.SilentPart.Value
                predX = tonumber(Options.SilentX.Value) or 0.1
                predY = tonumber(Options.SilentY.Value) or 0.1
            elseif LockActive and CurrentTarget then
                activeTarget = CurrentTarget
                partName = Options.TargetPart.Value
                predX = tonumber(Options.XPred.Value) or 0.1
                predY = tonumber(Options.YPred.Value) or 0.1
            end

            if activeTarget then
                local pos = GetTargetPosition(activeTarget, partName, predX, predY)
                if pos then
                    -- Intercept internal raycast calculations from camera vectors
                    if method == "Raycast" and self == workspace then
                        local origin = args[1]
                        if origin then
                            args[2] = (pos - origin).Unit * 1000 -- Overwrite direction vector directly towards target
                            return oldNamecall(self, unpack(args))
                        end
                    -- Intercept standard legacy ray methods
                    elseif method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist" then
                        local ray = args[1]
                        if ray then
                            args[1] = Ray.new(ray.Origin, (pos - ray.Origin).Unit * 1000)
                            return oldNamecall(self, unpack(args))
                        end
                    -- Intercept Namecall replication networks
                    elseif method == "FireServer" or (LockActive and Options.Method.Value == "Namecall") then
