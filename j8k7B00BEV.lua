...
local WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('faith secret | %s fps | %s ms'):format(
        math.floor(FPS),
...
LeftGroupBox:AddToggle('Enabled', {
    Text = 'Target Lock',
    Default = getgenv().paulozx.Enabled,
    Callback = function(Value)
        getgenv().paulozx.Enabled = Value
    end
})

LeftGroupBox:AddToggle('Enabled', {
    Text = 'Tracer',
    Default = getgenv().paulozx.TracerEnabled,
    Callback = function(Value)
        getgenv().paulozx.TracerEnabled = Value
    end
})

LeftGroupBox:AddToggle('NoGroundShot', {
    Text = 'No Ground Shot',
    Default = getgenv().paulozx.NoGroundShot,
  
    Callback = function(Value)
        getgenv().paulozx.NoGroundShot = Value
    end
})

LeftGroupBox:AddToggle('LookAt', {
    Text = 'Look At',
    Default = getgenv().paulozx.LookAt,
    Callback = function(Value)
        getgenv().paulozx.LookAt = Value
    end
})
...
LeftGroupBox:AddToggle('resolvert', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().paulozx.ResolverEnabled = Value
    end
})
...
Default = getgenv().paulozx.Camera,
    Callback = function(Value)
        getgenv().paulozx.Camera = Value
    end
})
...
DickHead:AddInput('Smoothness', {
    Default = tostring(getgenv().paulozx.smoothness),
    Numeric = true,
    Text = 'Camera Smoothness',
    Callback = function(Value)
        getgenv().paulozx.smoothness = tonumber(Value)
    end
})
...
sigmaox:AddToggle('Flicker', {
    Text = 'Flick',
    Default = getgenv().paulozx.Flick,
    Callback = function(Value)
        getgenv().paulozx.Flick = Value
    end
})

sigmaox:AddToggle('Auto 
Air', {
    Text = 'Enable Auto Air',
    Default = getgenv().paulozx.AutoAir,
    Callback = function(Value)
        getgenv().paulozx.AutoAir = Value
    end
})

sigmaox:AddInput('ShootDelay', {
    Default = tostring(getgenv().paulozx.ShootDelay),
    Numeric = true,
    Text = 'Shoot Delay',
    Callback = function(Value)
        getgenv().paulozx.ShootDelay = tonumber(Value)
    end
})
...
RightGroupBox:AddInput('HorizontalPrediction', {
    Default = tostring(getgenv().paulozx.HorizontalPrediction),
    Numeric = true,
    Text = 'Horizontal Prediction',
 
    Callback = function(Value)
        if not getgenv().paulozx.AutoPrediction then
            getgenv().paulozx.HorizontalPrediction = tonumber(Value)
        end
    end
})

RightGroupBox:AddInput('VerticalPrediction', {
    Default = tostring(getgenv().paulozx.VerticalPrediction),
    Numeric = true,
    Text = 'Vertical Prediction',
    Callback = function(Value)
        if not getgenv().paulozx.AutoPrediction then
            getgenv().paulozx.VerticalPrediction = tonumber(Value)
...
RightGroupBox:AddInput('Jump Offset', {
    Default = tostring(getgenv().paulozx.jumpoffset),
    Text = 'Jump Offset',
    Callback = function(Value)
        getgenv().paulozx.jumpoffset = Value
    end
})

RightGroupBox:AddToggle('AutoPrediction', {
    Text = 'Auto Prediction',
    Default = getgenv().paulozx.AutoPrediction,
    Callback = function(Value)
        getgenv().paulozx.AutoPrediction = Value
    end
})
...
RightGroupBox:AddDropdown('AutoPredMode', {
    ...
    Default = getgenv().paulozx.AutoPredMode == 'PingBased' 
and 2 or 1, -- Default to PingBased
    Text = 'Auto Prediction Mode',
    Callback = function(Value)
        getgenv().paulozx.AutoPredMode = Value
    end
})

RightGroupBox:AddDropdown('SelectedPart', {
    Values = { 'Head', 'LowerTorso', 'UpperTorso', 'HumanoidRootPart' },
    Default = getgenv().paulozx.SelectedPart == 'Head' and 1 or
              getgenv().paulozx.SelectedPart == 'LowerTorso' and 2 or
              getgenv().paulozx.SelectedPart == 'UpperTorso' and 3 or 4,
   
  Text = 'Target Part',
    Callback = function(Value)
        getgenv().paulozx.SelectedPart = Value
    end
})
...
RightGroupBox99:AddToggle('Si9282829382991lent', {
    Text = 'Silent Aim',
    Default = getgenv().paulozx.SilentAim,
    Callback = function(Value)
        getgenv().paulozx.SilentAim = Value
    end
})
...
RightGroupBox2:AddToggle('AntiEnabled', {
    Text = 'Enable Anti',
    Default = getgenv().paulozx.AntiEnabled,
    Callback = function(Value)
        getgenv().paulozx.AntiEnabled = Value
    end
})
...
RightGroupBox2:AddDropdown('SelectedMode', {
    ...
    Default = getgenv().paulozx.AntiLock == 'Predbreaker' and 1 or
  
             getgenv().paulozx.AntiLock == 'Sky' and 2 or 3,
    Text = 'Mode Selection',
    Callback = function(Value)
        getgenv().paulozx.AntiLock = Value
    end
})
...
local TargetVisualsGroup = Tabs.Visuals:AddRightGroupbox('Target Visuals')
local HitSoundsGroup = Tabs.Visuals:AddRightGroupbox('Hit Sounds')

-- ESP Toggles
ESPGroup:AddToggle('BoxESP', {
    Text = 'Box ESP (Corners & Dynamic)',
    Default = false,
    Callback = function(Value)
...
ESPGroup:AddToggle('ChamsESP', {
    Text = 'Chams ESP',
    Default = false,
    Callback = function(Value)
        getgenv().ChamsESP = Value
    end
})

ESPGroup:AddToggle('TextESP', {
    Text = 'Text ESP (Name, Health, Tool, Distance)',
    Default = false,
    Callback = function(Value)
        getgenv().TextESP = Value
    end
})
...
ESPGroup:AddToggle('HealthBar', {
    ...
    Default = false,
    Callback = function(Value)
        getgenv().HealthBar = Value
    end
})

-- Target Visuals Toggles
TargetVisualsGroup:AddToggle('TargetHighlight', {
    Text = 'Target Highlight',
    Default = false,
    Callback = function(Value)
        getgenv().TargetHighlight = Value
    end
})

TargetVisualsGroup:AddToggle('HitChams', {
    Text = 'Hit Chams',
    Default = false,
    Callback = function(Value)
        getgenv().HitChams = Value
    end
})

TargetVisualsGroup:AddToggle('HitSkeleton', {
    Text = 'Hit Skeleton',
    Default = false,
    Callback = function(Value)
        getgenv().HitSkeleton = Value
    end
})

-- Hit Sounds Implementation
local sounds = {
    ...
    Bruh = "https://github.com/CongoOhioDog/SoundS/blob/main/paulozx%20bruh%20sample.mp3?raw=true",
    BoneBreakage = "https://github.com/CongoOhioDog/SoundS/blob/main/paulozx%20bone%20breakage.mp3?raw=true",
    Fein = "https://github.com/CongoOhioDog/SoundS/blob/main/paulozx%20highly%20defined%20fein.mp3?raw=true",
    Unicorn = "https://github.com/CongoOhioDog/SoundS/blob/main/shiny%20unicorn%20for%20dh%20_%20paulozx%20audio.mp3?raw=true",
    Kitty = "https://github.com/CongoOhioDog/SoundS/blob/main/Kitty.mp3?raw=true",
    Bird = "https://github.com/CongoOhioDog/SoundS/blob/main/bird%20chirping%20for%20DH%20_%20paulozx%20audio.mp3?raw=true",
    BirthdayCake = "https://github.com/CongoOhioDog/SoundS/blob/main/Birthday%20cake%20for%20dh%20_%20paulozx.mp3?raw=true", 
    KenCarson =  "https://github.com/CongoOhioDog/SoundS/blob/main/ken_carson_-_jennifer_s_body_offici(2).mp3?raw=true"
}

-- Mock functions or framework checks for downloadSound and getAsset if required by script environment
if not downloadSound then
    if syn and syn.request then
        downloadSound = function(name, url) return url end
    else
        downloadSound = function(name, url) return url end
    end
end
...
if not getAsset then
    getAsset = function(path) return path end
end
...
local SoundNames = {"None", "RIFK7", "Bubble", "Minecraft", "Cod", "Bameware", "Neverlose", "Gamesense", "Rust", "BlackPencil", "UWU", "Plooh", "Moan", "Hentai", "Bruh", "BoneBreakage", "Fein", "Unicorn", "Kitty", "Bird", "BirthdayCake", "KenCarson"}

HitSoundsGroup:AddDropdown('HitSoundSelection', {
    Values = SoundNames,
    Default = 1,
...
ThemeManager:SetFolder('faithTheme')
SaveManager:SetFolder('faithCFG')
...
game:GetService("RunService").Heartbeat:Connect(function()
    ...
    if character and character:FindFirstChild("HumanoidRootPart") then
        ...
        if getgenv().paulozx.AntiEnabled then
            if getgenv().paulozx.AntiLock == "Predbreaker" then
                humanoidRootPart.Velocity = Vector3.new(0, 0, 0)
            elseif getgenv().paulozx.AntiLock == "Sky" then
                
humanoidRootPart.Velocity = Vector3.new(0, 100, 0)
            elseif getgenv().paulozx.AntiLock == "Ground" then
...
getgenv().paulozx.LockType = "Namecall"
getgenv().paulozx.RESOLVER = "MoveDirection"
...
local game_support = {
    ...
    [17403166075] = 
{ Remote = "MAINEVENT", Argument = "MOUSE" },
...
function SigmaOhioPlayer()
    ...
    for 
 i, v in pairs(game.Players:GetPlayers()) do
...
function LookAtPlayer(Target)
    ...
    if localHumanoidRootPart then
        if getgenv().paulozx and getgenv().paulozx.LookAt then
...
local function toggleLock()
    if enabled then
        ...
        if Plr then
          ...
          if getgenv().paulozx.TracerEnabled then
...
mt.__namecall = newcclosure(function(...)
    ...
    if getgenv().paulozx.Enabled and getgenv().paulozx.LockType == "Namecall" then
       
  if Plr and Plr.Character and method == "FireServer" and (args[2] == remoteInfo.Argument) then
            local selectedPart = getgenv().paulozx.SelectedPart
            local targetPart = Plr.Character[selectedPart]

            if targetPart then
                local velocity
                if getgenv().paulozx.ResolverEnabled then
        
             if getgenv().paulozx.RESOLVER == "MoveDirection" then
                        velocity = Plr.Character.Humanoid.MoveDirection * Plr.Character.Humanoid.WalkSpeed
                    elseif getgenv().paulozx.RESOLVER == "LookVector" then
                        velocity = targetPart.CFrame.LookVector * getgenv().paulozx.HorizontalPrediction * 1.0
 ...
 velocity = targetPart.Velocity
                ...
                local horizontalPrediction = getgenv().paulozx.HorizontalPrediction
                local verticalPrediction = getgenv().paulozx.VerticalPrediction
                local jumpOffset = getgenv().paulozx.jumpoffset or 0

                if not getgenv().paulozx.ResolverEnabled then
...
local function updatePredictionValue()
    if getgenv().paulozx.AutoPrediction then
    ...
    if ping then
            if getgenv().paulozx.AutoPredMode == "PingBased" then
                for i = 1, #predictionTable do
                    if ping < predictionTable[i][1] then
                        local value = predictionTable[i][2]
       
                     getgenv().paulozx.HorizontalPrediction = value
                        getgenv().paulozx.VerticalPrediction = value
                        break
                    end
           
     end
            elseif getgenv().paulozx.AutoPredMode == "Calculation" then
                local calculatedValue = 0.1 + (ping / 1000) * 0.32
                getgenv().paulozx.HorizontalPrediction = calculatedValue
                getgenv().paulozx.VerticalPrediction = calculatedValue
            elseif getgenv().paulozx.AutoPredMode == "AdvanceCalculation" then
                ...
                for i = 1, #predictionTable do
                    if ping < predictionTable[i][1] then
                       ...
                       getgenv().paulozx.HorizontalPrediction = advancePredictionValue
                        getgenv().paulozx.VerticalPrediction = advancePredictionValue
                        break
                    end
     
           end
            elseif getgenv().paulozx.AutoPredMode == "Blatant" then
                for i = 1, #Blatant do
...
RunService.Heartbeat:Connect(function()
    if getgenv().paulozx.Flick and Plr and Plr.Character then
       ...
       if targetPart and playerHumanoid.FloorMaterial == Enum.Material.Air then
                ...
                local targetPosition = targetPart.Position + targetPart.Velocity * getgenv().paulozx.VerticalPrediction
...
RunService.Heartbeat:Connect(function()
 ...
 if getgenv().paulozx.Camera and Plr and Plr.Character and getgenv().paulozx.SelectedPart then
        local camera = Workspace.CurrentCamera
        local selectedPart = getgenv().paulozx.SelectedPart
        local targetPart = Plr.Character[selectedPart]
     ...
     velocity = targetPart.Velocity
            ...
            local jumpOffset = getgenv().paulozx.jumpoffset or 0
            local fallOffset = getgenv().paulozx.FallOffset or 0
           ...
           local verticalVelocity = velocity.Y
            ...
            local smoothness = getgenv().paulozx.smoothness or 0.1 
...
local function ApplyAntiGround()
 ...
 if getgenv().paulozx.NoGroundShot then
                humanoidRootPart.Velocity = Vector3.new(velocity.X, 0, velocity.Z)
            end
        else
            if not getgenv().paulozx.NoGroundShot then
...
local targetSigm99928 = getgenv().paulozx.ShootDelay 
...
local function checkTarget()
    if Plr and Plr.Character then
        ...
        if humanoid and humanoidRootPart then
            ...
            if SigmaAir and getgenv().paulozx.AutoAir then
...
targetSigm99928 = getgenv().paulozx.ShootDelay

    if getgenv().paulozx.SilentAim then
