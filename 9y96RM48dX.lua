...
getgenv().BleedSettings = getgenv().BleedSettings or {
    PredictionMode = "Blatant", -- Set to "Blatant" for maximum vector projection tracking
    StandardDelayCoefficient = 0.00155, 
    StandardDelayCoefficientY = 0.00118, 
    ZeroDelayValue = 0.035,
    ZeroDelayValueY = 0.026, 
    
    -- Blatant configuration scalars
    BlatantValueX = 0.165, -- Highly amplified horizontal vector lead (X and Z alignment)
    BlatantValueY = 0.145, -- Highly amplified vertical vector lead (Y axis jump/fall tracking)
...
task.spawn(function()
    while true do
        ...
        -- [[ RESOLVE ACTIVE MODE SELECTION ]]
        local resolvedCalculatedX = liveZeroX
        local resolvedCalculatedY = liveZeroY
        local mode = getgenv().BleedSettings.PredictionMode

        if mode == "Standard" then
            resolvedCalculatedX = liveStandardX
            resolvedCalculatedY = liveStandardY
        elseif mode == "Blatant" then
            resolvedCalculatedX = getgenv().BleedSettings.BlatantValueX
            resolvedCalculatedY = getgenv().BleedSettings.BlatantValueY
        end
