...
local DesyncOffset = Vector3.new(999999, 0, 999999) -- Extreme position spoof offset
...
--// ============================================================================
--// 5. CORE ENGINES: MATRIX DESYNC & CAMERA CONTROLLER
--// ============================================================================
local randomGenerator = Random.new()

-- Advanced Matrix Desync Loop (High-Speed Network Replication Spoofing)
RunService.Heartbeat:Connect(function()
    local myCharacter = LocalPlayer.Character
    if myCharacter and myCharacter:FindFirstChild("HumanoidRootPart") and myCharacter:FindFirstChild("Humanoid") then
        if DesyncEnabled and myCharacter.Humanoid.Health > 0 then
            local hrp = myCharacter.HumanoidRootPart
            local originalCFrame = hrp.CFrame
            
            -- Server ko fake coordinate par update bhejo
            hrp.CFrame = originalCFrame * CFrame.new(DesyncOffset)
            
            -- Render frame ka wait karo packet release karne ke liye
            RunService.RenderStepped:Wait()
            
            -- Client par wapas original place par sync karo taake controls crash na hon
