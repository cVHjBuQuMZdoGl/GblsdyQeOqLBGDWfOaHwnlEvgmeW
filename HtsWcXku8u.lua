...
local Config = {
    -- Camlock Configurations (New Main Hub)
    CamlockEnabled = false,
    CamlockAntiGroundShot = false,
    CamlockResolverEnabled = false,
    CamlockPredictionX = 0.1,
    CamlockPredictionY = 0.1,
    CamlockTargetingMode = "Namecall",
    CamlockHitPartCustom = false,
    CamlockHitPart = "HumanoidRootPart",
    CamlockWallCheck = false,
    CamlockBotCheck = false,
    CamlockFriendCheck = false,
    CamlockKnockedCheck = false,
...
local function CreateCustomCamlockFloatingButton()
    local index = #SpawnedCamlockButtons + 1
    local btn = Instance.new("TextButton")
    btn.Name = "CustomCamlockFloatingToggle_" .. index
    btn.Size = UDim2.new(0, 70, 0, 70)
    btn.Position = UDim2.new(0, 40 + (index * 8), 0, 345 + (index * 4))
    btn.BackgroundColor3 = Color3.fromRGB(40, 40, 40)
    btn.BackgroundTransparency = 0.4
    btn.Text = Config.CamlockEnabled and "🔒" or "🔓"
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        btn.Text = Config.CamlockEnabled and "🔒" or "🔓"
        local toggle = Toggles and Toggles.CamlockEnabledToggle
        if toggle then toggle:SetValue(Config.CamlockEnabled) end
    end)
    
    table.insert(SpawnedCamlockButtons, btn)
end
...
-- UI Tab Registrations (Camlock assigned as the Main Tab)
local CamlockTab = Window:AddTab("Camlock")
...
-- Camlock Tab Groupboxes Hub Setup
local CamlockMainGroup = CamlockTab:AddLeftGroupbox("Camlock Configuration")
local CamlockPredictionGroup = CamlockTab:AddRightGroupbox("Prediction Configuration")
local CamlockHitpartGroup = CamlockTab:AddLeftGroupbox("Hitpart Override")
local CamlockChecksGroup = CamlockTab:AddRightGroupbox("Target Checks")
...
-- Camlock Target Visualizer (Small White Box) Setup
local CamlockVisualizer = Drawing.new("Square")
CamlockVisualizer.Thickness = 1
CamlockVisualizer.Color = Color3.fromRGB(255, 255, 255)
CamlockVisualizer.Size = Vector2.new(14, 14)
CamlockVisualizer.Filled = false
CamlockVisualizer.Visible = false
...
local CamlockCurrentTargetPart = nil
...
RunService.RenderStepped:Connect(function(dt)
    ...
    -- [[ CAMLOCK ENGINE CORE RUNTIME EXECUTION ]]
    if not Config.CamlockEnabled then
        CamlockTargetPlayer = nil
        CamlockCurrentTargetPart = nil
        CamlockVisualizer.Visible = false
        for _, button in ipairs(SpawnedCamlockButtons) do
            if button.Parent then button.Text = "🔓" end
        end
    else
        local closestCamlockDist = math.huge
        local viewportCenter = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
        local rawCamlockTarget = nil
        local rawCamlockPart = nil

        local scanningPlayers = Players:GetPlayers()
        for i = 1, #scanningPlayers do
            local targetPlr = scanningPlayers[i]
            if targetPlr ~= LocalPlayer and targetPlr.Character then
                local character = targetPlr.Character
                
                if Config.CamlockFriendCheck and IsFriend(targetPlr) then continue end
                if Config.CamlockKnockedCheck and IsKnocked(targetPlr) then continue end
                if Config.CamlockBotCheck and (targetPlr.UserId <= 0 or targetPlr.Name:lower():find("bot")) then continue end

                local scanPartName = Config.CamlockHitPartCustom and Config.CamlockHitPart or "HumanoidRootPart"
                local targetPart = character:FindFirstChild(scanPartName) or character:FindFirstChild("HumanoidRootPart") or character:FindFirstChild("Head")
                local humanoid = character:FindFirstChildOfClass("Humanoid")

                if targetPart and humanoid and humanoid.Health > 0 then
                    if Config.CamlockWallCheck and IsBehindWall(targetPart, character) then continue end
                    
                    local screenPos, onScreen = Camera:WorldToViewportPoint(targetPart.Position)
                    if onScreen then
                        local screenMagnitude = (Vector2.new(screenPos.X, screenPos.Y) - viewportCenter).Magnitude
                        if screenMagnitude < closestCamlockDist and (Config.FOVRadius == 0 or screenMagnitude <= Config.FOVRadius) then
                            closestCamlockDist = screenMagnitude
                            rawCamlockTarget = targetPlr
                            rawCamlockPart = targetPart
                        end
                    end
                end
            end
        ...
        CamlockTargetPlayer = rawCamlockTarget
        CamlockCurrentTargetPart = rawCamlockPart

        if CamlockTargetPlayer and CamlockCurrentTargetPart then
            for _, button in ipairs(SpawnedCamlockButtons) do
                if button.Parent then button.Text = "🔒" end
            end
            
            local rootPart = CamlockTargetPlayer.Character:FindFirstChild("HumanoidRootPart")
            local velocity = rootPart and rootPart.AssemblyLinearVelocity or Vector3.new(0, 0, 0)
            
            if Config.CamlockAntiGroundShot and velocity.Y < -15 then
                velocity = Vector3.new(velocity.X, 0, velocity.Z)
            end

            if Config.CamlockResolverEnabled then
                local targetHum = CamlockTargetPlayer.Character:FindFirstChildOfClass("Humanoid")
            ...
            local calculatedLockPosition = CamlockCurrentTargetPart.Position + Vector3.new(
                velocity.X * Config.CamlockPredictionX,
                velocity.Y * Config.CamlockPredictionY,
                velocity.Z * Config.CamlockPredictionX
            )

            Camera.CFrame = CFrame.new(Camera.CFrame.Position, calculatedLockPosition)

            local boxScreenPos, boxOnScreen = Camera:WorldToViewportPoint(CamlockCurrentTargetPart.Position)
            if boxOnScreen then
                CamlockVisualizer.Position = Vector2.new(boxScreenPos.X - (CamlockVisualizer.Size.X / 2), boxScreenPos.Y - (CamlockVisualizer.Size.Y / 2))
                CamlockVisualizer.Visible = true
            else
                CamlockVisualizer.Visible = false
            end
        else
    ...
    if not Config.Enabled then 
        ...
        for i = 1, #allPlayers do
            ...
            if player ~= LocalPlayer and player.Character then
                ...
                if (Config.FriendCheck and IsFriend(player)) or (Config.KoCheck and IsKnocked(player)) then 
                    continue 
                end
                ...
                if targetPart and humanoid and humanoid.Health > 0 then
                    ...
                    if onScreen then
                        ...
                        if screenMagnitude < closestDistance and (Config.FOVRadius == 0 or screenMagnitude <= Config.FOVRadius) then
                            if not (Config.WallCheck and IsBehindWall(targetPart, character)) then
...
-- Multi-Targeting Dual Index / Namecall Metamethod Hooks Optimization Engine
local OriginalIndexHook
OriginalIndexHook = hookmetamethod(game, "__index", newcclosure(function(self, Index)
    if not checkcaller() and Config.CamlockEnabled and Config.CamlockTargetingMode == "Index" and CamlockTargetPlayer and CamlockCurrentTargetPart then
        if (Index == "Hit" or Index == "Target") and self:IsA("Mouse") then
            local rootPart = CamlockTargetPlayer.Character:FindFirstChild("HumanoidRootPart")
            local velocity = rootPart and rootPart.AssemblyLinearVelocity or Vector3.new(0, 0, 0)
            if Config.CamlockResolverEnabled then
                local targetHum = CamlockTargetPlayer.Character:FindFirstChildOfClass("Humanoid")
            ...
            local customTargetPos = CamlockCurrentTargetPart.Position + Vector3.new(velocity.X * Config.CamlockPredictionX, velocity.Y * Config.CamlockPredictionY, velocity.Z * Config.CamlockPredictionX)
            if Index == "Hit" then return CFrame.new(customTargetPos) end
            if Index == "Target" then return CamlockCurrentTargetPart end
        end
    end
    return OriginalIndexHook(self, Index)
end))
...
OriginalNamecallHook = hookmetamethod(game, "__namecall", newcclosure(function(...)
    ...
    if not checkcaller() then
        -- Silent Aim Calculations Engine
        if Config.Enabled and CurrentTargetPart and (Config.CamlockTargetingMode == "Namecall" or not Config.CamlockEnabled) then
        ...
        -- Secondary Core Injection Redirect logic for Camlock Targeting Framework
        if Config.CamlockEnabled and CamlockCurrentTargetPart and Config.CamlockTargetingMode == "Namecall" then
            local rootPart = CamlockTargetPlayer.Character:FindFirstChild("HumanoidRootPart")
            local velocity = rootPart and rootPart.AssemblyLinearVelocity or Vector3.new(0, 0, 0)
            if Config.CamlockResolverEnabled then
                local targetHum = CamlockTargetPlayer.Character:FindFirstChildOfClass("Humanoid")
            ...
            local directCamlockTargetPos = CamlockCurrentTargetPart.Position + Vector3.new(velocity.X * Config.CamlockPredictionX, velocity.Y * Config.CamlockPredictionY, velocity.Z * Config.CamlockPredictionX)
            
            if method == "Raycast" and self == Workspace and args[2] and args[3] then
                args[3] = (directCamlockTargetPos - args[2]).Unit * args[3].Magnitude
                return OriginalNamecallHook(unpack(args))
            end
            if (method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist") and args[2] then
                args[2] = Ray.new(args[2].Origin, (directCamlockTargetPos - args[2].Origin).Unit * 9999)
                return OriginalNamecallHook(unpack(args))
            end
        end
    end
...
-- [[ UI INITIALIZATION CONTROLS ENGINE ]]

-- Camlock Tab Group Layout Setup
CamlockMainGroup:AddToggle("CamlockEnabledToggle", {
    Text = "Camlock Lock Engine",
    Default = false,
    Callback = function(state) Config.CamlockEnabled = state end
})

CamlockMainGroup:AddToggle("CamlockAntiGroundShotToggle", {
    Text = "Anti Ground Shot",
    Default = false,
    Callback = function(state) Config.CamlockAntiGroundShot = state end
})

CamlockMainGroup:AddToggle("CamlockResolverToggle", {
    Text = "Resolver (Move Direction)",
    Default = false,
    Callback = function(state) Config.CamlockResolverEnabled = state end
})

CamlockMainGroup:AddButton({
    Text = "Spawn Camlock Button",
    Func = CreateCustomCamlockFloatingButton
})

CamlockPredictionGroup:AddInput("CamlockPredictionXInput", {
    Text = "X Prediction Offset",
    Default = "0.1",
    Callback = function(value) Config.CamlockPredictionX = tonumber(value) or 0 end
})

CamlockPredictionGroup:AddInput("CamlockPredictionYInput", {
    Text = "Y Prediction Offset",
    Default = "0.1",
    Callback = function(value) Config.CamlockPredictionY = tonumber(value) or 0 end
})

CamlockPredictionGroup:AddDropdown("CamlockTargetingModeDropdown", {
    Values = { "Namecall", "Index" },
    Default = 1,
    Multi = false,
    Text = "Targeting Redirection Engine",
    Callback = function(value) Config.CamlockTargetingMode = value end
})

CamlockHitpartGroup:AddToggle("CamlockHitPartCustomToggle", {
    Text = "Enable Custom Hitpart",
    Default = false,
    Callback = function(state) Config.CamlockHitPartCustom = state end
})

CamlockHitpartGroup:AddDropdown("CamlockHitPartDropdown", {
    Values = { "Head", "UpperTorso", "Torso", "LowerTorso" },
    Default = 1,
    Multi = false,
    Text = "Lock Hitpart",
    Callback = function(value) Config.CamlockHitPart = value end
})

CamlockChecksGroup:AddToggle("CamlockWallCheckToggle", {
    Text = "Wall Check",
    Default = false,
    Callback = function(state) Config.CamlockWallCheck = state end
})

CamlockChecksGroup:AddToggle("CamlockBotCheckToggle", {
    Text = "Bot/NPC Check",
    Default = false,
    Callback = function(state) Config.CamlockBotCheck = state end
})

CamlockChecksGroup:AddToggle("CamlockFriendCheckToggle", {
    Text = "Friend Check",
    Default = false,
    Callback = function(state) Config.CamlockFriendCheck = state end
})

CamlockChecksGroup:AddToggle("CamlockKnockedCheckToggle", {
    Text = "Knocked Check",
    Default = false,
    Callback = function(state) Config.CamlockKnockedCheck = state end
})

-- UI Controls - Tab 2 (Silent Tab)
...
MenuGroupBox:AddButton('Unload Script', function() 
    pcall(function()
        ...
        for _, btn in ipairs(SpawnedCamlockButtons) do pcall(function() btn:Destroy() end) end
        
        FOVCircle:Destroy()
        TracerLine:Destroy()
        TargetHighlight:Destroy()
        CamlockVisualizer:Destroy()
        EspGui:Destroy()
...
RunService.RenderStepped:Connect(function()
    ...
    tbRaycastParams.FilterDescendantsInstances = { LocalPlayer.Character, CoreGui, EspGui }
