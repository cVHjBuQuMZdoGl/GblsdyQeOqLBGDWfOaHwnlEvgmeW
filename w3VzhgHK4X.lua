...
local Config = {
    ...
    -- Self Visuals Configurations
    SelfForcefield = false,
    SelfForcefieldColor = Color3.fromRGB(0, 255, 150),
    SelfTrails = false,
    SelfTrailsColor = Color3.fromRGB(255, 255, 255),
    SelfAura = "None", -- "None", "Skibidi RedRizz", "Solar Flare", "Abyssal Frost"
    ...
    NoDelayJumpEnabled = false, -- Integrated Configuration State
...
-- Robust Flight Control State Mapping Setup (PC Native Only)
...
-- [UPDATED] Custom Integrated Auto Reload Engine Framework
local function runAutoReloadScript()
    pcall(function()
        ...
        local function checkAndFire()
            -- Master Toggle verification gating
        ...
        -- Run initialization check immediately in case execution lands on 0 ammo
        checkAndFire()
    end)
end
...
function updateAura(auraName)
    ...
    local colors = {}
    if auraName == "Skibidi RedRizz" then
        colors.wave1 = Color3.fromRGB(234, 8, 255)
        colors.wave2 = Color3.fromRGB(238, 3, 255)
        colors.sparks = Color3.fromRGB(255, 21, 255)
        colors.stars = Color3.fromRGB(226, 60, 255)
    elseif auraName == "Solar Flare" then
        colors.wave1 = Color3.fromRGB(255, 60, 0)
        colors.wave2 = Color3.fromRGB(255, 150, 0)
        colors.sparks = Color3.fromRGB(255, 230, 0)
        colors.stars = Color3.fromRGB(255, 100, 0)
    elseif auraName == "Abyssal Frost" then
        colors.wave1 = Color3.fromRGB(0, 220, 255)
        colors.wave2 = Color3.fromRGB(0, 80, 255)
        colors.sparks = Color3.fromRGB(130, 255, 255)
        colors.stars = Color3.fromRGB(0, 255, 170)
    end

    createBaseEmitter("Healing Wave 1", {
        Lifetime = NumberRange.new(1.5, 1.5),
        SpreadAngle = Vector2.new(10, -10),
        LockedToPart = true,
        Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(0.1702454, 0.7, 0.014881), NumberSequenceKeypoint.new(0.2254601, 0.03125, 0.03125), NumberSequenceKeypoint.new(0.2852761, 0), NumberSequenceKeypoint.new(0.702454, 0), NumberSequenceKeypoint.new(0.8374233, 0.9125, 0.0601461), NumberSequenceKeypoint.new(1, 1)}),
        LightEmission = 0.4,
        Color = ColorSequence.new(colors.wave1),
        VelocitySpread = 10,
        Speed = NumberRange.new(3, 6),
        Brightness = 10,
        Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 3.0624998, 1.8805969), NumberSequenceKeypoint.new(0.6420546, 1.9999999, 1.7619393), NumberSequenceKeypoint.new(1, 0.7499999, 0.7499999)}),
        Rate = 20,
    ...
    createBaseEmitter("Healing Wave 2", {
        Lifetime = NumberRange.new(1.5, 1.5),
        SpreadAngle = Vector2.new(10, -10),
        LockedToPart = true,
        Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0, 1), NumberSequenceKeypoint.new(0.2254601, 0.03125, 0.03125), NumberSequenceKeypoint.new(0.6288344, 0.25625, 0.0593491), NumberSequenceKeypoint.new(0.8374233, 0.9125, 0.0601461), NumberSequenceKeypoint.new(1, 1)}),
        LightEmission = 1,
        Color = ColorSequence.new(colors.wave2),
        VelocitySpread = 10,
        Speed = NumberRange.new(3, 5),
        Brightness = 10,
        Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 3.125), NumberSequenceKeypoint.new(0.4165329, 1.3749999, 1.3749999), NumberSequenceKeypoint.new(1, 0.9375, 0.9375)}),
        Rate = 20,
    ...
    createBaseEmitter("Sparks", {
        ...
        LightEmission = 1,
        Color = ColorSequence.new(colors.sparks),
        Drag = 3,
        VelocitySpread = 180,
        Speed = NumberRange.new(5, 15),
        Brightness = 10,
        Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(0.14687, 0.4374999, 0.1875001), NumberSequenceKeypoint.new(1, 0)}),
    ...
    createBaseEmitter("Star Sparks", {
        Lifetime = NumberRange.new(1.5, 1.5),
        SpreadAngle = Vector2.new(180, -180),
        LightEmission = 1,
        Color = ColorSequence.new(colors.stars),
        Drag = 3,
        VelocitySpread = 180,
        Speed = NumberRange.new(5, 10),
        Brightness = 10,
        Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(0.1492777, 0.6874996, 0.6874996), NumberSequenceKeypoint.new(1, 0)}),
        Acceleration = Vector3.new(0, 3, 0),
        ZOffset = 2,
...
-- Monitor death/reset lifecycles to re-hook UI instances cleanly
LocalPlayer.CharacterAdded:Connect(function()
    table.clear(originalPartProperties)
    task.wait(1)
    runAutoReloadScript()
    
    -- Persistent Visuals State Reconstruction Matrix
    if Config.SelfForcefield then updateForcefield() end
    if Config.SelfTrails then updateTrails() end
    if Config.SelfAura ~= "None" then updateAura(Config.SelfAura) end
end)

-- Baseline runtime invocation check for immediate startup execution
...
local SelfVisualsGroup = VisualsTab:AddLeftGroupbox("Self Visuals") -- Dedicated Self Matrix Container Layout
    ...
    if not Config.Enabled then 
        ...
        if TargetPlayer and CurrentTargetPart then
            ...
            if rootPart then
                ...
                -- High-Performance Core Target Resolver Matrix
...
-- High-Performance Core Movement HvH Processor Matrix
RunService.Heartbeat:Connect(function(dt)
    local char = LocalPlayer.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    ...
    -- 1. CFrame Speed Matrix Integration (Reads Mobile Joystick MoveDirection)
    ...
    -- 2. Upgraded Camera-Relative Flight Controller Engine
    if Config.FlyEnabled then
        ...
        -- Translates world-space vectors dynamically into full 3D Camera space
        ...
        -- Append dedicated vertical data adjustments (Space/Control)
...
-- 3. Noclip Continuous Collisions Override Matrix
...
-- 4. Dynamic Void Invisibility Desync Engine Framework
...
-- Integrated No Delay Jump Metamethod Hook Matrix
local IsA = game.IsA
...
-- UI Controls: New Integrated Self Visuals Matrix Panel
...
SelfVisualsGroup:AddDropdown("SelfAuraDropdown", {
    Values = { "None", "Skibidi RedRizz", "Solar Flare", "Abyssal Frost" },
    Default = 1,
    Multi = false,
    Text = "Aura Matrix Visuals",
...
WeaponUtilsGroup:AddToggle("AutoReloadToggle", {
    ...
    Callback = function(state) 
        ...
        if state then
            -- Safely scan for active targets immediately on manual activation
            pcall(function()
