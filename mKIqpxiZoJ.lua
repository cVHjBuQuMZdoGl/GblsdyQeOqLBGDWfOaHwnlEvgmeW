...
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    pcall(function()
        ...
        if mouse then mouse.TargetFilter = char end
    end)
end)
...
if LocalPlayer.Character then
    task.spawn(function()
        pcall(function()
            local mouse = LocalPlayer:GetMouse()
            if mouse then mouse.TargetFilter = LocalPlayer.Character end
        end)
        task.wait(1)
        runAutoReloadScript()
    end)
...
-- Dedicated __index hook for flawless Da Hood Mouse.Hit / Mouse.Target translation
local OriginalIndexHook
OriginalIndexHook = hookmetamethod(game, "__index", newcclosure(function(self, Index)
    if not checkcaller() and Config.Enabled and CurrentTargetPart and CurrentTargetPosition ~= Vector3.new(0,0,0) then
...
OriginalNamecallHook = hookmetamethod(game, "__namecall", newcclosure(function(...)
    ...
    if not checkcaller() then
        -- Raycast Method Modification Block
        if method == "Raycast" and self == Workspace and args[2] and args[3] then
            if Config.Enabled and CurrentTargetPart and CurrentTargetPosition ~= Vector3.new(0,0,0) then
                local origin = args[2]
                args[3] = (CurrentTargetPosition - origin).Unit * args[3].Magnitude
            end
            
            -- FIX: Always inject your own character into RaycastParams exclusion to prevent hitting yourself
            local params = args[4]
            if not params then
                params = RaycastParams.new()
                params.FilterType = Enum.RaycastFilterType.Exclude
                params.FilterDescendantsInstances = {LocalPlayer.Character, CoreGui, EspGui}
                args[4] = params
            elseif typeof(params) == "RaycastParams" then
                local filter = params.FilterDescendantsInstances
                if not table.find(filter, LocalPlayer.Character) then
        ...
        -- Old Legacy Raycast Method Modification Block
        if (method == "FindPartOnRay" or method == "FindPartOnRayWithIgnoreList" or method == "FindPartOnRayWithWhitelist") and args[2] then
            ...
            -- FIX: Always inject your own character into legacy Raycast table filters
            if method == "FindPartOnRayWithIgnoreList" then
                local ignoreList = args[3]
                if type(ignoreList) == "table" then
                    if not table.find(ignoreList, LocalPlayer.Character) then
                        table.insert(ignoreList, LocalPlayer.Character)
                    end
                else
                    args[3] = {LocalPlayer.Character}
                end
            elseif method == "FindPartOnRay" then
                return OriginalNamecallHook(self, "FindPartOnRayWithIgnoreList", args[2], {LocalPlayer.Character})
            end
            return OriginalNamecallHook(unpack(args))
        end
    end
