...
local Config = {
    ...
    SelfForcefieldColor = Color3.fromRGB(255, 255, 255), -- Default to White
    SelfTrails = false,
...
--------------------------------------------------------------------------------
-- Core Functional Core Engine for Self Visuals Logic (UPDATED Matrix)
--------------------------------------------------------------------------------
...
function updateForcefield()
    ...
    if Config.SelfForcefield then
        -- Uses GetDescendants to scan structural limbs AND Accessory handles simultaneously
...
function updateTrails()
    ...
    topAttachment.Position = Vector3.new(0, 2.6, 0)
    topAttachment.Parent = rootPart

    local bottomAttachment = Instance.new("Attachment")
    bottomAttachment.Position = Vector3.new(0, -2.6, 0)
    bottomAttachment.Parent = rootPart

    -- Ultra Upgraded Glow Dynamics (100x Visual Upgrade Output)
    local trail = Instance.new("Trail")
    trail.Attachment0 = topAttachment
    trail.Attachment1 = bottomAttachment
    
    -- Dynamic Color Sequence mapping from configuration input down to pitch deep dark black contrast
    trail.Color = ColorSequence.new({
        ColorSequenceKeypoint.new(0, Config.SelfTrailsColor),
        ColorSequenceKeypoint.new(1, Color3.fromRGB(0, 0, 0))
    })
    -- Seamless alpha opacity step decay layout
    trail.Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0, 0),
        NumberSequenceKeypoint.new(0.4, 0.15),
        NumberSequenceKeypoint.new(1, 1)
    })
    -- High tier dynamic sharp sleek tapering width sequence mapping
    ...
    trail.LightEmission = 1.35
    trail.LightInfluence = 0
    trail.Texture = "rbxassetid://12523098555" -- Hyper-vibrant beam glow asset map
...
function updateAura()
    ...
    local char = getCharacter()
    local lowerTorso = char and (char:FindFirstChild("LowerTorso") or char:FindFirstChild("HumanoidRootPart"))
    ...
    -- Deconstruct HSL matrices dynamically from color-picker tracking inputs
    local h, s, v = Config.SelfAuraColor:ToHSV()
    local wave1Color = Color3.fromHSV(h, s, math.clamp(v * 1.2, 0, 1))
    local wave2Color = Color3.fromHSV((h + 0.04) % 1, s, v)
    local sparksColor = Color3.fromHSV(h, math.clamp(s * 0.7, 0, 1), 1)

    createBaseEmitter("Dynamic Wave 1", {
        ...
        LightEmission = 0.5,
        Color = ColorSequence.new(wave1Color),
        VelocitySpread = 10,
        Speed = NumberRange.new(3, 6),
        Brightness = 10,
        Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 3.3), NumberSequenceKeypoint.new(0.64, 2.1), NumberSequenceKeypoint.new(1, 0.8)}),
        Rate = 22,
    ...
    createBaseEmitter("Dynamic Wave 2", {
        Lifetime = NumberRange.new(1.5, 1.5),
        SpreadAngle = Vector2.new(10, -10),
        LockedToPart = true,
        ...
        LightEmission = 1,
        Color = ColorSequence.new(wave2Color),
        VelocitySpread = 10,
        Speed = NumberRange.new(3, 5),
        Brightness = 10,
        Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 3.2), NumberSequenceKeypoint.new(0.41, 1.4), NumberSequenceKeypoint.new(1, 0.95)}),
        Rate = 22,
    ...
    createBaseEmitter("Dynamic Sparks", {
        Lifetime = NumberRange.new(0.5, 2),
        SpreadAngle = Vector2.new(180, -180),
        LightEmission = 1,
        Color = ColorSequence.new(sparksColor),
        Drag = 3,
        VelocitySpread = 180,
        Speed = NumberRange.new(5, 15),
        Brightness = 10,
        Size = NumberSequence.new({NumberSequenceKeypoint.new(0, 0), NumberSequenceKeypoint.new(0.14, 0.45), NumberSequenceKeypoint.new(1, 0)}),
...
RunService.MainLoop:Connect(function()
...
-- UI Controls: Self Visuals Group (UPDATED)
SelfVisualsGroup:AddToggle("SelfForcefieldToggle", {
    ...
    Default = Color3.fromRGB(255, 255, 255), -- Updated White Default
...
-- [UPDATED] Dropped legacy string dropdown for unified layout state
