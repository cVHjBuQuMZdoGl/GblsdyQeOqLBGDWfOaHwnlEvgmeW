...
task.spawn(function()
    while true do
        ...
        -- [[ FIX: Added structural block so Auto-Prediction loop leaves values alone while Blatant runs ]]
...
-- [[ STRUCTURAL FIX: DECOUPLED PREDICTION LAYER ]]
...
local function calculatePrediction(targetChar, isPlayer)
    ...
    ---------------------------------------------------------
    -- BLATANT HIGH-ACCURACY ADVANCED PHYSICS MODE
    ---------------------------------------------------------
    if getgenv().BleedSettings.PredictionMode == "Blatant" then
        ...
        -- Physical deceleration and tracking checks
        ...
        -- Network lag runtime cushion multiplier
        PredPos = PredPos + (velocity * cachedLivePing)
        return PredPos
    end

    ---------------------------------------------------------
    -- DECOUPLED STANDARD MODE
    ---------------------------------------------------------
