...
-- Camlock initialized as the first tab
...
-- Layout Groupboxes: Camlock Tab
local LockMainGroup = CamLockTab:AddLeftGroupbox("Main utilities")
local LockPredictionGroup = CamLockTab:AddRightGroupbox("Prediction configuration")
local LockHitGroup = CamLockTab:AddLeftGroupbox("Target zoning")

-- Layout Groupboxes: Silent Tab
...
-- Frame-Perfect Render Pipeline Loop
RunService.RenderStepped:Connect(function(dt)
    ...
    -- 2. Performance-Scaled Active Player Full 2D Box Transformation Matrix
...
-- UI Controls: Camlock Tab Groupboxes Elements
LockMainGroup:AddButton("Spawn Target Node", function()
    -- Intended for Button Spawner Logic
end)

LockMainGroup:AddToggle("AntiGroundShotToggle", {
    Text = "Anti Ground Shot",
    Default = false,
    Callback = function(state) Config.AntiGroundShot = state end
})

LockMainGroup:AddToggle("ResolverToggle", {
    Text = "Resolver",
    Default = false,
    Callback = function(state) Config.Resolver = state end
})

LockPredictionGroup:AddInput("CamPredictionXInput", {
    Text = "Camera X Prediction",
    Default = "0.1",
    Callback = function(value) Config.CamLockPredictionX = tonumber(value) or 0 end
})

LockPredictionGroup:AddInput("CamPredictionYInput", {
    Text = "Camera Y Prediction",
    Default = "0.1",
    Callback = function(value) Config.CamLockPredictionY = tonumber(value) or 0 end
})

LockHitGroup:AddDropdown("CamHitPartDropdown", {
...
-- UI Controls: Balanced Player ESP Interface Setup
