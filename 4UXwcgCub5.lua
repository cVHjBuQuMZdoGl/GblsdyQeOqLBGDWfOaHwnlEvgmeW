...
local AimbotBtn = createButton("AIMBOT: OFF", UDim2.new(0, 20, 0, 65), function(btn)
    ...
    if not AimbotEnabled then Camera.CameraType = Enum.CameraType.Custom end
end)
...
local ClosestBtn = createButton("CLOSEST LOCK: OFF", UDim2.new(0, 20, 0, 110), function(btn)
    ...
    if not ClosestPlayerEnabled then Camera.CameraType = Enum.CameraType.Custom end
end)
...
--// ============================================================================
--// NEW FIXED REAL-DISTANCE BASED TARGET FINDER
--// ============================================================================
local function getAbsoluteClosestPlayer()
    local closestPlayer = nil
    local shortestDistance = math.huge -- Pure physical range filter
    
    -- Humare apne character ki position check karo
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
            ...
            -- Dono characters ke darmiyan asal doori (Magnitude) calculate karo
            local targetPos = player.Character.HumanoidRootPart.Position
            local actualDistance = (targetPos - myPos).Magnitude
            
            -- Jo sabse kam doori par hoga woh select hoga
    ...
    if AimbotEnabled then
        ...
        if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if ClosestPlayerEnabled then
                -- Hard Lock on actual closest
                Camera.CameraType = Enum.CameraType.Scriptable
                Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, targetPos)
            else
                -- Assist Smooth Lock on actual closest
