...
--// ============================================================================
--// 1. HARDENED MAIN-ACCOUNT STEALTH BYPASS LAYER
--// ============================================================================
local function AdvancedMainAccountBypass()
    ...
    -- High-Level Remote Interception
    mt.__namecall = _newcclosure(function(self, ...)
        ...
        if not _checkcaller() then
            ...
            -- S2 aur premium games ke common logs/telemetry remotes block karo
            if remote_name:find("ban") or remote_name:find("kick") or remote_name:find("cheat") or 
               remote_name:find("ac") or remote_name:find("detection") or remote_name:find("report") or 
               remote_name:find("telemetry") or remote_name:find("log") then
                return nil
            end
        end
    ...
    -- Hook / Upvalue Inspection Block (Environment Spoofing)
    local old_getupvalues = debug.getupvalues
    if old_getupvalues then
        hookfunction(debug.getupvalues, function(f)
            if not _checkcaller() then return {} end
            return old_getupvalues(f)
        end)
    end
    
    -- Memory aur CoreGui Scanner Bypass
...
pcall(AdvancedMainAccountBypass)
...
--// ============================================================================
--// 4. ANTI-UNDERGROUND TARGET FINDER
--// ============================================================================
local function getAbsoluteClosestPlayer()
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChild("Humanoid") and player.Character.Humanoid.Health > 0 then
            ...
            -- CHECK 1: Height filter
            if targetPos.Y < -500 or (myPos.Y - targetPos.Y) > 150 then
                continue
            end
            
            -- CHECK 2: Raycast zameen check
            ...
            local direction = targetPos - myHead.Position
            local raycastResult = workspace:Raycast(myHead.Position, direction, raycastParams)
...
--// ============================================================================
--// 5. HUMANIZED SMOOTH CAMERA CONTROLLER (Main Account Safe)
--// ============================================================================
...
RunService.RenderStepped:Connect(function()
    ...
    if AimbotEnabled then
        ...
        if target and target.Character and target.Character:FindFirstChild("HumanoidRootPart") then
            ...
            -- HUMANIZED LATENCY LAYER: Server logs mein robot detection se bachne ke liye choti si humanized variation offset
            local microJitter = Vector3.new(
                randomGenerator:NextNumber(-0.02, 0.02),
                randomGenerator:NextNumber(-0.02, 0.02),
                randomGenerator:NextNumber(-0.02, 0.02)
            )
            ...
            if ClosestPlayerEnabled then
                -- Main Account Locked: Hard instant focus but with natural smoothing
                Camera.CFrame = CFrame.lookAt(Camera.CFrame.Position, targetPos)
            else
                -- Humanized Lerp: Sabse safe tracking method
