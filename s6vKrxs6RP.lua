...
local Config = {
    -- New Camlock Configurations
    CamlockEnabled = false,
    CamlockPredictionX = 0.134,
    CamlockPredictionY = 0.134,
    AutoPredActive = false,
    CamlockHitPartEnabled = false,
    CamlockHitNearestPart = false,
    CamlockHitPart = "HumanoidRootPart",
    CamlockResolverEnabled = false,
    CamlockResolverMode = "Recalculate",
    AutoAirEnabled = false,
    AntiGroundShot = false,
    
    -- Ping Engine Default Settings
    StandardDelayCoefficient = 0.0025,
    StandardDelayCoefficientY = 0.0025,
    ZeroDelayValue = 0.134,
    ZeroDelayValueY = 0.134,
    PredictionMode = "Standard",

    -- Silent Aim Hub Configurations
    Enabled = false,
    Highlight = false,
    Tracers = false,
    ShowFOV = false,
...
-- Share Environment Reference Hook for Auto Prediction Engine Compatibility
getgenv().BleedSettings = Config
...
local function CreateCustomCamlockFloatingButton()
    ...
    btn.BackgroundTransparency = 1          
    btn.Text = CamlockTarget and "Camlock: LOCKED" or "Camlock: UNLOCKED"
    btn.Font = Enum.Font.GothamBold         
    btn.TextSize = 15
    btn.TextColor3 = Color3.fromRGB(255, 255, 255)
    btn.Parent = EspGui
    ...
    local strokeGrad = Instance.new("UIGradient")
    local crimsonColor = CamlockTarget and Color3.fromRGB(255, 0, 0) or Color3.fromRGB(130, 0, 0)
    strokeGrad.Color = ColorSequence.new({
    ...
    table.insert(SpawnedCamlockButtons, { Button = btn, StrokeGradient = strokeGrad, TextGradient = textGrad })
end
...
-- UI Window Tab Structure Realignment
...
-- Setup UI Groupboxes for Camlock Main Page
local CamlockMainGroup = MainTab:AddLeftGroupbox("Camera Lock")
local CamlockConfigGroup = MainTab:AddRightGroupbox("Camlock Configuration")

-- Old UI Groupboxes Realignment
local MainGroup = SilentTab:AddLeftGroupbox("Silent aim")
local FiltersGroup = SilentTab:AddLeftGroupbox("Checks")
local PredictionGroup = SilentTab:AddRightGroupbox("Prediction")
...
-- Explicit Config Left/Right Section Layout Generation to prevent rendering conflicts
local MenuGroupBox = ConfigTab:AddLeftGroupbox("Menu Settings")
local ThemeGroupBox = ConfigTab:AddLeftGroupbox("Theme Profiles")
local SavesGroupBox = ConfigTab:AddRightGroupbox("Configuration Manager")

local FOVCircle = Drawing.new("Circle")
FOVCircle.Thickness = 2
FOVCircle.Color = Color3.fromRGB(172, 0, 0)
...
TracerLine.Color = Color3.fromRGB(172, 0, 0)
...
RunService.RenderStepped:Connect(function(dt)
    ...
    for _, item in ipairs(SpawnedCamlockButtons) do
        if item.Button and item.Button.Parent then
            item.StrokeGradient.Rotation = rotationSpeed
            item.TextGradient.Rotation = rotationSpeed
        end
...
-- [[ HIGH-PERFORMANCE CAMERA LOCK MANAGEMENT LOOP ]]
RunService.RenderStepped:Connect(function()
    if Config.CamlockEnabled and CamlockTarget and CamlockTarget.Character then
        ...
        if hum and hum.Health > 0 and root then
            ...
            if Config.CamlockHitPartEnabled then
                if Config.CamlockHitNearestPart then
                    local closestPart = root
                    ...
                    targetPart = char:FindFirstChild(Config.CamlockHitPart) or root
                end
            end
            
            local velocity = root.AssemblyLinearVelocity
            
            -- Anti Ground Shot Clamp Realignment
            if Config.AntiGroundShot and velocity.Y < -20 then
                velocity = Vector3.new(velocity.X, 0, velocity.Z)
            end
            
            -- Camlock Resolver Processor
            if Config.CamlockResolverEnabled then
                if Config.CamlockResolverMode == "Look Vector" then
                    velocity = root.CFrame.LookVector * velocity.Magnitude
                elseif Config.CamlockResolverMode == "Move Direction" then
                    if hum and hum.MoveDirection.Magnitude > 0 then
                        velocity = hum.MoveDirection * velocity.Magnitude
                    else
                        velocity = Vector3.new(0, 0, 0)
                    end
                elseif Config.CamlockResolverMode == "Recalculate" then
                    if velocity.Magnitude > 200 then
                        velocity = root.CFrame.LookVector * math.clamp(velocity.Magnitude, 0, 50)
                    end
                end
            end
            
            local predX = Config.CamlockPredictionX or 0.134
            local predY = Config.CamlockPredictionY or 0.134
            local predictedPos = targetPart.Position + Vector3.new(velocity.X * predX, velocity.Y * predY, velocity.Z * predX)
...
-- [[ VIRTUAL AUTO AIR FIRING PROCESSOR ]]
local lastJumpTrackState = {}
RunService.Heartbeat:Connect(function()
    if Config.CamlockEnabled and Config.AutoAirEnabled and CamlockTarget and CamlockTarget.Character then
        local char = CamlockTarget.Character
        local hum = char:FindFirstChildOfClass("Humanoid")
        if hum then
            local isJumping = (hum:GetState() == Enum.HumanoidStateType.Jumping or hum.FloorMaterial == Enum.Material.Air)
            if isJumping and not lastJumpTrackState[CamlockTarget] then
                lastJumpTrackState[CamlockTarget] = true
                -- Virtual Activation to prevent joystick/camera interface locks
                local localChar = LocalPlayer.Character
                local weapon = localChar and localChar:FindFirstChildOfClass("Tool")
                if weapon then
                    weapon:Activate()
                end
            elseif not isJumping then
                lastJumpTrackState[CamlockTarget] = false
            end
        end
    else
        table.clear(lastJumpTrackState)
    end
end)
...
-- [[ Tab 1 Layout: CAMERA LOCK UTILITIES ]]
...
CamlockMainGroup:AddToggle("AutoAirToggle", {
    Text = "Auto Air",
    Default = false,
    Callback = function(state) Config.AutoAirEnabled = state end
})

CamlockMainGroup:AddToggle("AntiGroundShotToggle", {
    Text = "Anti Ground Shot",
    Default = false,
    Callback = function(state) Config.AntiGroundShot = state end
})

CamlockConfigGroup:AddInput("CamlockPredictionXInput", {
    Text = "X Prediction",
    Default = "0.134",
    Callback = function(value) Config.CamlockPredictionX = tonumber(value) or 0 end
})

CamlockConfigGroup:AddInput("CamlockPredictionYInput", {
    Text = "Y Prediction",
    Default = "0.134",
    Callback = function(value) Config.CamlockPredictionY = tonumber(value) or 0 end
})

CamlockConfigGroup:AddToggle("AutoPredActiveToggle", {
    Text = "Auto Prediction",
    Default = false,
    Callback = function(state) Config.AutoPredActive = state end
})

CamlockConfigGroup:AddToggle("CamlockHitPartToggle", {
    Text = "Hit Part",
    Default = false,
    Callback = function(state) Config.CamlockHitPartEnabled = state end
})

CamlockConfigGroup:AddToggle("CamlockHitNearestPartToggle", {
    Text = "Hit Nearest Part",
    Default = false,
    Callback = function(state) Config.CamlockHitNearestPart = state end
})

CamlockConfigGroup:AddDropdown("CamlockHitPartDropdown", {
    Values = { "HumanoidRootPart", "Head", "UpperTorso", "Torso", "LowerTorso" },
    Default = 1,
    Multi = false,
    Text = "Target Bone Specification",
    Callback = function(value) Config.CamlockHitPart = value end
})

CamlockConfigGroup:AddToggle("CamlockResolverToggle", {
    Text = "Resolver",
    Default = false,
    Callback = function(state) Config.CamlockResolverEnabled = state end
})

CamlockConfigGroup:AddDropdown("CamlockResolverModeDropdown", {
    Values = { "Recalculate", "Look Vector", "Move Direction" },
    Default = 1,
    Multi = false,
    Text = "Resolver Mode Engine",
    Callback = function(value) Config.CamlockResolverMode = value end
})


-- UI Controls - Tab 2 (Silent)
...
-- UI Controls - Remaining Tab Elements
...
MenuGroupBox:AddButton('Unload Script', function() 
    pcall(function()
        ...
        for _, item in ipairs(SpawnedCamlockButtons) do pcall(function() item.Button:Destroy() end) end
...
-- [[ THEME & CONFIG SAVE ARCHITECTURE SECTION RENDERERS ]]
ThemeManager:SetLibrary(Library)
ThemeManager:SetFolder("bleedcc")
ThemeManager:BuildConfigSection(ThemeGroupBox)

SaveManager:SetLibrary(Library)
SaveManager:SetFolder('bleedcc/configs')
SaveManager:BuildConfigSection(SavesGroupBox)
SaveManager:IgnoreThemeSettings()
SaveManager:SetIgnoreIndexes({ 'MenuKeybind' })
SaveManager:LoadAutoloadConfig()

-- [[ PING CONTROLLER UTILITIES (FROM SOURCE) ]]
...
local function generatePredictionsFromPing(pingValue)
    ...
    for pingMin = basePing, basePing + 100, 10 do
        ...
        local standardVal = (averagePing * Config.StandardDelayCoefficient) + (math.random(-5, 5) / 10000)
        standardPredictions = standardPredictions .. string.format("ping%d_%d = %.4f,\n", pingMin, pingMin + 10, standardVal)
        local zeroVal = Config.ZeroDelayValue + (math.random(-2, 2) / 10000)
...
-- [[ INTEGRATED AUTO PREDICTION MAIN PROCESS TABLE CONTROLLER ]]
task.spawn(function()
    while true do
        ...
        local liveStandardX = (currentPing * Config.StandardDelayCoefficient)
        local liveStandardY = (currentPing * Config.StandardDelayCoefficientY)
        local liveZeroX = Config.ZeroDelayValue
        local liveZeroY = Config.ZeroDelayValueY
        
        local resolvedCalculatedX = (Config.PredictionMode == "Standard") and liveStandardX or liveZeroX
        local resolvedCalculatedY = (Config.PredictionMode == "Standard") and liveStandardY or liveZeroY
        
        if Config.AutoPredActive then
            Config.CamlockPredictionX = resolvedCalculatedX
            Config.CamlockPredictionY = resolvedCalculatedY
            
            pcall(function()
                if Options and Options.CamlockPredictionXInput then
                    Options.CamlockPredictionXInput:SetValue(string.format("%.5f", resolvedCalculatedX))
                end
                if Options and Options.CamlockPredictionYInput then
                    Options.CamlockPredictionYInput:SetValue(string.format("%.5f", resolvedCalculatedY))
                end
            end)
        end
        task.wait(3) 
...
-- [[ INTEGRATED SILENT AIM TRIGGERBOT BACKEND EXECUTION LOGIC ]]
local tbLastShot = 0
