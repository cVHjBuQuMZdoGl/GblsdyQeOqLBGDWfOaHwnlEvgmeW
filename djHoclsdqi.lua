...
local Library = loadstring(game:HttpGet("https://raw.githubusercontent.com/9x6/Obsidian/main/Obsidian.lua"))()
...
local AimGroup = MainTab:AddSection("Aimlock")
local SettingsGroup = MainTab:AddSection("Aimlock Settings")
local CamlockGroup = MainTab:AddSection("Camlock Settings")
local TriggerGroup = LegitTab:AddSection("Triggerbot")
local HitboxGroup = LegitTab:AddSection("Hitbox Expander")
local StrafeGroup = RageTab:AddSection("Target Strafe")
local ReplicationGroup = RageTab:AddSection("Replication Freeze")
...
IndicatorUI.Name = "VoidHitIndicator"
...
AimGroup:AddTextBox('PredictionInput', {
    Text = 'Prediction',
    Default = '0.06',
    Numeric = true,
    Finished = true,
...
AimGroup:AddButton({
    ...
    Func = function()
        ...
        lock_button.Image = "rbxassetid://0"
...
CamlockGroup:AddTextBox('PredictionInput', {
    Text = 'Prediction',
...
CamlockGroup:AddTextBox('HorizontalPrediction', {
    Text = 'Horizontal Prediction',
    Default = tostring(getgenv().CamlockSettings.HorizontalPrediction),
    Numeric = true,
    Finished = true,
    Callback = function(val)
        local num = tonumber(val)
        if num then getgenv().CamlockSettings.HorizontalPrediction = num end
    end
})

CamlockGroup:AddTextBox('VerticalPrediction', {
    Text = 'Vertical Prediction',
    Default = tostring(getgenv().CamlockSettings.VerticalPrediction),
    Numeric = true,
    Finished = true,
    Callback = function(val)
        local num = tonumber(val)
        if num then getgenv().CamlockSettings.VerticalPrediction = num end
    end
})

CamlockGroup:AddTextBox('HorizontalStrength', {
    Text = 'Horizontal Strength',
    Default = tostring(getgenv().CamlockSettings.HorizontalStrength),
    Numeric = true,
    Finished = true,
    Callback = function(val)
        local num = tonumber(val)
        if num then getgenv().CamlockSettings.HorizontalStrength = num end
    end
})

CamlockGroup:AddTextBox('VerticalStrength', {
    Text = 'Vertical Strength',
    Default = tostring(getgenv().CamlockSettings.VerticalStrength),
    Numeric = true,
    Finished = true,
    Callback = function(val)
        local num = tonumber(val)
        if num then getgenv().CamlockSettings.VerticalStrength = num end
    end
})

CamlockGroup:AddTextBox('CameraSmoothness', {
    Text = 'Camera Smoothness',
    Default = tostring(getgenv().CamlockSettings.CameraSmoothness),
    Numeric = true,
    Finished = true,
    Callback = function(val)
        local num = tonumber(val)
        if num then getgenv().CamlockSettings.CameraSmoothness = num end
    end
})

CamlockGroup:AddTextBox('CameraShake', {
    Text = 'Camera Shake',
    Default = tostring(getgenv().CamlockSettings.CameraShake),
    Numeric = true,
    Finished = true,
    Callback = function(val)
        local num = tonumber(val)
        if num then getgenv().CamlockSettings.CameraShake = num end
    end
})

CamlockGroup:AddTextBox('JumpOffset', {
    Text = 'Jump Offset',
    Default = tostring(getgenv().CamlockSettings.JumpOffset),
    Numeric = true,
    Finished = true,
    Callback = function(val)
        local num = tonumber(val)
        if num then getgenv().CamlockSettings.JumpOffset = num end
    end
})
...
--// =========================
--// VISUALS -> 3D BOX HITBOX ESP
--// =========================

do
    local Box3DGroup = VisualTab:AddRightGroupbox('3D Box ESP')

    local Hitbox3DConfig = {
        Enabled = false,
        Filled = true,
        Thickness = 0.5,
        Transparency = 0.25,
    ...
    local Box3D_Camera = workspace.CurrentCamera
    local Box3D_Lines = {}
    local Box3D_Quads = {}

    local function Box3D_HasCharacter(Player)
        return Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
    end

    local function Box3D_DrawQuad(PosA, PosB, PosC, PosD)
        local PosAScreen, PosAVisible = Box3D_Camera:WorldToViewportPoint(PosA)
        local PosBScreen, PosBVisible = Box3D_Camera:WorldToViewportPoint(PosB)
        local PosCScreen, PosCVisible = Box3D_Camera:WorldToViewportPoint(PosC)
        local PosDScreen, PosDVisible = Box3D_Camera:WorldToViewportPoint(PosD)

        if (not PosAVisible and not PosBVisible and not PosCVisible and not PosDVisible) then return end

        local Quad = Drawing.new("Quad")
        Quad.Thickness = Hitbox3DConfig.Thickness
        Quad.Color = Hitbox3DConfig.Color
        Quad.Transparency = Hitbox3DConfig.Transparency
        Quad.ZIndex = 1
        Quad.Filled = Hitbox3DConfig.Filled
        Quad.Visible = true
        ...
        table.insert(Box3D_Quads, Quad)
    end

    local function Box3D_DrawLine(From, To)
        local FromScreen, FromVisible = Box3D_Camera:WorldToViewportPoint(From)
        local ToScreen, ToVisible = Box3D_Camera:WorldToViewportPoint(To)
        ...
        Line.Color = Hitbox3DConfig.Color
        Line.Transparency = 1
        Line.ZIndex = 1
        Line.Visible = true

        table.insert(Box3D_Lines, Line)
    end

    local function Box3D_GetCorners(Part)
    ...
    local function Box3D_DrawEsp(Player)
        local HRP = Player.Character.HumanoidRootPart
        local CubeVertices = Box3D_GetCorners({CFrame = HRP.CFrame * CFrame.new(0, -0.5, 0), Size = Vector3.new(3, 5, 3)})

        Box3D_DrawLine(CubeVertices[1], CubeVertices[2])
        Box3D_DrawLine(CubeVertices[2], CubeVertices[6])
        Box3D_DrawLine(CubeVertices[6], CubeVertices[5])
        Box3D_DrawLine(CubeVertices[5], CubeVertices[1])
        Box3D_DrawQuad(CubeVertices[1], CubeVertices[2], CubeVertices[6], CubeVertices[5])

        Box3D_DrawLine(CubeVertices[1], CubeVertices[3])
        Box3D_DrawLine(CubeVertices[2], CubeVertices[4])
        Box3D_DrawLine(CubeVertices[6], CubeVertices[8])
        Box3D_DrawLine(CubeVertices[5], CubeVertices[7])
        Box3D_DrawQuad(CubeVertices[2], CubeVertices[4], CubeVertices[8], CubeVertices[6])
        Box3D_DrawQuad(CubeVertices[1], CubeVertices[2], CubeVertices[4], CubeVertices[3])
        Box3D_DrawQuad(CubeVertices[1], CubeVertices[5], CubeVertices[7], CubeVertices[3])
        Box3D_DrawQuad(CubeVertices[5], CubeVertices[7], CubeVertices[8], CubeVertices[6])

        Box3D_DrawLine(CubeVertices[3], CubeVertices[4])
        Box3D_DrawLine(CubeVertices[4], CubeVertices[8])
        Box3D_DrawLine(CubeVertices[8], CubeVertices[7])
        Box3D_DrawLine(CubeVertices[7], CubeVertices[3])
        Box3D_DrawQuad(CubeVertices[3], CubeVertices[4], CubeVertices[8], CubeVertices[7])
    end

    local function Box3D_Clear()
        for i = 1, #Box3D_Lines do
            local Line = rawget(Box3D_Lines, i)
            if Line then Line:Remove() end
        end
        Box3D_Lines = {}
        for i = 1, #Box3D_Quads do
            local Quad = rawget(Box3D_Quads, i)
    ...
    RunService.RenderStepped:Connect(function()
        Box3D_Camera = workspace.CurrentCamera
        Box3D_Clear()
        if not Hitbox3DConfig.Enabled then return end
        local Plrs = Players:GetPlayers()
        for i = 1, #Plrs do
            local Player = rawget(Plrs, i)
            if Player ~= Players.LocalPlayer and Box3D_HasCharacter(Player) then
                Box3D_DrawEsp(Player)
            end
        end
    end)

    Box3DGroup:AddToggle('Box3D_Enable', {
        Text = '3D Box ESP',
        Default = false,
        Callback = function(v) Hitbox3DConfig.Enabled = v end
    })
    Box3DGroup:AddToggle('Box3D_Filled', {
        Text = 'Filled',
        Default = true,
        Callback = function(v) Hitbox3DConfig.Filled = v end
    })
    Box3DGroup:AddSlider('Box3D_Transparency', {
        Text = 'Fill Transparency',
        Min = 0, Max = 1, Default = 0.25, Rounding = 2,
        Callback = function(v) Hitbox3DConfig.Transparency = v end
    })
    Box3DGroup:AddSlider('Box3D_Thickness', {
        Text = 'Line Thickness',
        Min = 0.1, Max = 3, Default = 0.5, Rounding = 2,
        Callback = function(v) Hitbox3DConfig.Thickness = v end
    })
    Box3DGroup:AddLabel('Box Color'):AddColorPicker('Box3D_Color', {
        Default = Color3.fromRGB(255, 255, 255),
        Title = 'Box Color',
        Callback = function(v) Hitbox3DConfig.Color = v end
    })
end


--// =========================
--// RAGEBOT -> INFINITE RANGE
--// =========================

do
    local InfRangeGroup = ExtrasTab:AddRightGroupbox('Infinite Range')
    local InfRangeConfig = {
        Enabled = false,
        MaxRange = 100000,
    }
    ...
    RunService.RenderStepped:Connect(function()
        if not InfRangeConfig.Enabled then return end
        local character = LocalPlayer.Character
        ...
        for _, propName in pairs(rangeProps) do
            ...
            if rangeValue and rangeValue:IsA("NumberValue") then
                rangeValue.Value = InfRangeConfig.MaxRange
            end
            ...
            if config then
                ...
                if r and r:IsA("NumberValue") then
                    r.Value = InfRangeConfig.MaxRange
                end
            end
        end
    end)

    InfRangeGroup:AddToggle('InfRange_Enable', {
        Text = 'Infinite Range',
        Default = false,
        Callback = function(v) InfRangeConfig.Enabled = v end
    })
    InfRangeGroup:AddTextBox('InfRange_MaxRange', {
        Text = 'Max Range',
        Default = tostring(InfRangeConfig.MaxRange),
        Numeric = true,
        Finished = true,
        Callback = function(val)
            local num = tonumber(val)
            if num then InfRangeConfig.MaxRange = num end
        end
    })
end
...
--// =========================
--// RAGEBOT -> ANTI-VOID (+ NO FALLEN PARTS DESTROY)
--// =========================

do
    local AntiVoidGroup = ExtrasTab:AddLeftGroupbox('Anti-Void')
    local AntiVoidConfig = {
        Enabled = false,
        HeightLimit = -400,
        RegenY = 50,
    }

    local function AntiVoid_Process(objeto)
        local jogador = Players:GetPlayerFromCharacter(objeto)
        if jogador then
            local rootPart = objeto:FindFirstChild("HumanoidRootPart")
            if rootPart then
                rootPart.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
                rootPart.CFrame = CFrame.new(Vector3.new(0, AntiVoidConfig.RegenY, 0))
            end
        else
            if objeto:IsA("BasePart") and not objeto.Anchored then
                objeto.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
                objeto.Position = Vector3.new(objeto.Position.X, AntiVoidConfig.HeightLimit + 10, objeto.Position.Z)
    ...
    task.spawn(function()
        while true do
            task.wait(0.5)
            if AntiVoidConfig.Enabled then
                for _, jogador in ipairs(Players:GetPlayers()) do
                    local char = jogador.Character
                    if char then
                        local rootPart = char:FindFirstChild("HumanoidRootPart")
                        if rootPart and rootPart.Position.Y < AntiVoidConfig.HeightLimit then
                            AntiVoid_Process(char)
                        end
                    end
                end
                for _, objeto in ipairs(workspace:GetDescendants()) do
                    if objeto:IsA("BasePart") and not objeto.Anchored then
                        if objeto.Position.Y < AntiVoidConfig.HeightLimit then
                            AntiVoid_Process(objeto)
                        end
                    end
                end
            end
    ...
    AntiVoidGroup:AddToggle('AntiVoid_Enable', {
        Text = 'Anti-Void',
        Default = false,
        Callback = function(v) AntiVoidConfig.Enabled = v end
    })
    AntiVoidGroup:AddTextBox('AntiVoid_HeightLimit', {
        Text = 'Height Limit',
        Default = tostring(AntiVoidConfig.HeightLimit),
        Numeric = true,
        Finished = true,
        Callback = function(val)
            local num = tonumber(val)
            if num then AntiVoidConfig.HeightLimit = num end
        end
    })
    AntiVoidGroup:AddTextBox('AntiVoid_RegenY', {
        Text = 'Regen Height (Y)',
        Default = tostring(AntiVoidConfig.RegenY),
        Numeric = true,
        Finished = true,
        Callback = function(val)
            local num = tonumber(val)
            if num then AntiVoidConfig.RegenY = num end
        end
    })

    -- No Fallen Parts Destroy
    local FallenOriginal = workspace.FallenPartsDestroyHeight
    AntiVoidGroup:AddToggle('AntiVoid_FallenParts', {
        Text = 'No Fallen Parts Destroy',
        Default = false,
        Callback = function(v)
            pcall(function()
                if v then
                    ...
                    workspace.FallenPartsDestroyHeight = FallenOriginal
                end
            end)
        end
    })
...
--// =========================
--// RAGEBOT -> STOMP TARGET
--// =========================

do
    local StompGroup = ExtrasTab:AddRightGroupbox('Stomp')
    local StompConfig = {
        Enabled = false,
        StompTarget = false,
        Range = 12,
    ...
    local function GetMainEvent()
        ...
        return rs:FindFirstChild("MainEvent")
    end
    ...
    local function DoStomp()
        ...
        if ev then
            pcall(function() ev:FireServer("Stomp") end)
        end
    end

    local function FindKnockedTarget()
        local myChar = LocalPlayer.Character
        local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
        if not myRoot then return nil end
        local closest, closestDist = nil, StompConfig.Range
        for _, plr in ipairs(Players:GetPlayers()) do
            if plr ~= LocalPlayer and plr.Character then
    ...
    -- Stomp Target: quando o alvo travado (Target) e derrubado, TP ate ele, stompa e volta
    local stompedThisKnock = false
    task.spawn(function()
        while true do
            task.wait(0.05)
            if StompConfig.StompTarget and Target and Target.Character then
                local tChar = Target.Character
                local tRoot = tChar:FindFirstChild("HumanoidRootPart")
                local knocked = IsKnocked(tChar)
                if tRoot and knocked and not stompedThisKnock then
                    stompedThisKnock = true
                    pcall(function()
                        local myChar = LocalPlayer.Character
                        local myRoot = myChar and myChar:FindFirstChild("HumanoidRootPart")
                        if myRoot then
                ...
                elseif not knocked then
                    stompedThisKnock = false
                end
            else
                stompedThisKnock = false
            end
    ...
    task.spawn(function()
        while true do
            task.wait(0.1)
            if StompConfig.Enabled then
                pcall(function()
                    local target = FindKnockedTarget()
                    if target then DoStomp() end
                end)
            end
        end
    end)

    StompGroup:AddToggle('Stomp_Target', {
        Text = 'Stomp Target',
        Default = false,
        Callback = function(v) StompConfig.StompTarget = v end
    })
    StompGroup:AddToggle('Stomp_Enable', {
        Text = 'Auto Stomp',
        Default = false,
        Callback = function(v) StompConfig.Enabled = v end
    })
    StompGroup:AddTextBox('Stomp_Range', {
        Text = 'Stomp Range',
        Default = tostring(StompConfig.Range),
        Numeric = true,
        Finished = true,
        Callback = function(val)
            local num = tonumber(val)
            if num then StompConfig.Range = num end
        end
    })
    StompGroup:AddButton({
        Text = 'Stomp Now',
        Func = function() DoStomp() end
    })
end
