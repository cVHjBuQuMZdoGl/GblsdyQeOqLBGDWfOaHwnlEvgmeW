-- [[ STANDALONE BLEED.CC NOTIFICATION ENGINE ]]
...
-- Screen Setup & Layout Parent
local NotificationGui = UIBuilder:Create("ScreenGui", { Name = "BleedNotificationGui", ResetOnSpawn = false, ZIndexBehavior = Enum.ZIndexBehavior.Sibling, Parent = UIParent })
local NotificationContainer = UIBuilder:Create("Frame", { Name = "NotificationContainer", Size = UDim2.new(0, 180, 0, 600), Position = UDim2.new(1, -195, 0, 20), BackgroundTransparency = 1, Parent = NotificationGui })
...
-- Master Notification Core Function
local function sendNotification(messageString)
    task.spawn(function()
        ...
        local VisualCard = UIBuilder:Create("Frame", { Name = "VisualCard", Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(1.2, 0, 0, 0), BackgroundColor3 = Colors.Background, BorderSizePixel = 0, Parent = Holder })
        local CardStroke = UIBuilder:Create("UIStroke", { Color = Colors.Accent, Thickness = 1, ApplyStrokeMode = Enum.ApplyStrokeMode.Border, Parent = VisualCard })
        local Label = UIBuilder:Create("TextLabel", { Size = UDim2.new(1, -16, 1, -2), Position = UDim2.new(0, 8, 0, 0), BackgroundTransparency = 1, Text = messageString, TextColor3 = Colors.TextSecondary, Font = Enum.Font.Code, TextSize = 10, TextXAlignment = Enum.TextXAlignment.Left, Parent = VisualCard })
        local CooldownBar = UIBuilder:Create("Frame", { Name = "CooldownBar", Size = UDim2.new(1, 0, 0, 2), Position = UDim2.new(0, 0, 1, -2), BackgroundColor3 = Colors.Accent, BorderSizePixel = 0, ZIndex = 3, Parent = VisualCard })
...
getgenv().BleedNotify = sendNotification
...
local function CreateCustomCFrameFloatingButton()
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        BleedNotify("Cframe: " .. (nextState and "ON" or "OFF"))
    end)
...
local function CreateCustomFloatingButton()
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        BleedNotify("Invis: " .. (nextState and "ON" or "OFF"))
    end)
...
local function CreateCustomFlyFloatingButton()
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        BleedNotify("Fly: " .. (nextState and "ON" or "OFF"))
    end)
...
function SetInvisibleState(state)
    ...
    if hrp then
        if state then
            ...
            -- Saves exact original values and switches local character to ghost transparency
            for _, part in pairs(char:GetDescendants()) do
                if (part:IsA("BasePart") and part.Name ~= "HumanoidRootPart") or part:IsA("Decal") or part:IsA("Texture") then
                    -- FIX: Only affect parts that aren't already completely invisible (skips hidden game hitboxes)
            ...
            -- Restores exact original transparency values (avoids making inherently invisible parts solid)
            ...
            if saved then
                ...
                -- Robust fallback safety check loop
                for _, part in pairs(char:GetDescendants()) do
                    if (part:IsA("BasePart") and part.Name ~= "HumanoidRootPart") or part:IsA("Decal") then
                        -- Only default back parts that aren't meant to be hidden hitboxes
                        if part.Transparency ~= 1 then
                            part.Transparency = 0
                        end
                    end
